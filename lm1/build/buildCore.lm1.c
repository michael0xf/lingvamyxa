...
#ifndef LM_LMX_TYPEDEF_DEFINED_LmBuildOptions
#define LM_LMX_TYPEDEF_DEFINED_LmBuildOptions 1
#define LM_LMX_TYPEDEF_ID_A_LmBuildOptions 0x6e8a8214925db5a6ULL
#define LM_LMX_TYPEDEF_ID_B_LmBuildOptions 0x95a0ddb536068c67ULL
typedef struct LmBuildOptions {
    int full_build;
    int next_build;
} LmBuildOptions;
#else
#if !defined(LM_LMX_TYPEDEF_ID_A_LmBuildOptions) || !defined(LM_LMX_TYPEDEF_ID_B_LmBuildOptions) || LM_LMX_TYPEDEF_ID_A_LmBuildOptions != 0x6e8a8214925db5a6ULL || LM_LMX_TYPEDEF_ID_B_LmBuildOptions != 0x95a0ddb536068c67ULL
...
static int lm_build_write_platform_tests_script(struct LmMessageThread *lm_lmx_message_thread, FILE *file, char *output_dir, char *parser_library, char *own_library) {
    ...
    fputs("        & $make 'link' '-std=c99' '-Wall' '-Wextra' '-Wpedantic' '-Werror' '-DLM_TEST_PREDEF_DESCRIPTOR_BRANCH=1' '-Ilm1' '-Itests' $cPath $supportCSources $parserLib $ownLib '-o' $branchExePath\n", file);
    fputs("        if ($LASTEXITCODE -ne 0) { throw 'conditional predef descriptor branch link failed' }\n", file);
    fputs("        & (Resolve-Path -LiteralPath $branchExePath).Path\n", file);
