...
static LmP0Node *lm_p0_new_node(LmP0Document *document, LmP0NodeKind kind) {
    ...
    node->span = (LmP0Span *)lm_own_arena_new_zero(document->tree_arena, sizeof(*node->span));
    if (node->span == 0) {
...
static int lm_p0_starts_c_surface_atom(const char *text, size_t end, size_t start) {
    return start + 1U < end && text[start] == 'c' && text[start + 1U] == '.';
}

static int lm_p0_is_c_surface_top_boundary(char value) {
    return
        lm_p0_is_field_space(value) ||
        lm_p0_is_field_separator(value) ||
        value == '(' ||
        value == ')' ||
        value == ':' ||
        value == '#' ||
...
static size_t lm_p0_scan_c_sizeof_surface_atom(const char *text, size_t end, size_t start) {
    size_t i;
    size_t close_index;
    size_t depth;
    int raw_safe;
    ...
    if (
        start + 8U >= end ||
        memcmp(text + start, "c.sizeof", 8U) != 0 ||
        text[start + 8U] != '('
    ) {
        return start;
    }
    ...
    while (i < end) {
        ...
        if (lm_p0_starts_c_prefixed_quote(text, end, i)) {
            size_t quoted_end;

            quoted_end = lm_p0_scan_c_prefixed_quote_token(text, end, i);
            if (quoted_end > i + 1U) {
                i = quoted_end;
                continue;
            }
        ...
        if (text[i] == '\'' || text[i] == '"') {
            ...
            quoted_end = lm_p0_scan_c_quoted_token(text, end, i);
            if (quoted_end > i + 1U) {
                i = quoted_end;
                continue;
            }
    ...
    while (i < close_index) {
        if (
            text[i] == '\\' ||
            text[i] == '@' ||
            (i + 1U < close_index && text[i] == 'c' && text[i + 1U] == '.')
        ) {
            raw_safe = 0;
            break;
        }
    ...
    return raw_safe ? close_index + 1U : start;
}

static size_t lm_p0_scan_c_surface_atom(const char *text, size_t end, size_t start) {
    size_t i;
    size_t sizeof_end;
    size_t bracket_depth;

    if (!lm_p0_starts_c_surface_atom(text, end, start)) {
        return start;
    }

    sizeof_end = lm_p0_scan_c_sizeof_surface_atom(text, end, start);
    if (sizeof_end > start) {
        return sizeof_end;
    }
    ...
    while (i < end) {
        ...
        if (text[i] == '[') {
            ++bracket_depth;
        } else if (text[i] == ']' && bracket_depth > 0U) {
            --bracket_depth;
        }
        ++i;
    }
...
static size_t lm_p0_scan_compact_atom_piece(const char *text, size_t end, size_t start) {
    ...
    if (lm_p0_starts_c_surface_atom(text, end, start)) {
        return lm_p0_scan_c_surface_atom(text, end, start);
    }
...
static int lm_p0_registry_l4_join_table(
    ...
    if (
        !lm_p0_registry_identifier_value(source_table, source_name) ||
        !lm_p0_registry_identifier_value(target_table, target_name)
    ) {
        lm_p0_text_ref_delete(source_name);
        lm_p0_text_ref_delete(target_name);
        return -1;
    ...
    for (i = 0U; i < initial_count; ++i) {
        ...
        if (
            lm_p0_registry_push_row_values(
                lm_p0_text_from_cstr(target_relation),
                lm_p0_text_from_cstr(row->key),
                lm_p0_text_from_cstr(row->payload)
            ) != 0
        ) {
...
static LmL4FrameReceiver lm_p0_registry_l4_resolve_frame(
    ...
    if (strcmp(receiver_type, "l4.frame") != 0) {
        fprintf(
            stderr,
            "parser registry inconsistency: namespace.l4[\"%.*s\"] has receiver marker %s, expected l4.frame\n",
    ...
    fprintf(
        stderr,
        "parser registry inconsistency: namespace.l4[\"%.*s\"] has no direct frame receiver binding\n",
        head != 0 ? (int)head->length : 0,
        head != 0 ? head->data : ""
    );
    return 0;
...
static int lm_p0_registry_l4_dispatch_frame(
    const LmL4Loader *loader,
    void *context,
    ...
    if (receiver == 0) {
        fprintf(
            stderr,
            "parser registry error: registry body expects registered L4 receiver frames, got \"%.*s\"\n",
...
static int lm_p0_registry_load_default(void) {
    ...
    candidates[0] = override_enabled ? override_path : "lm4/parser_registry.lm4";
