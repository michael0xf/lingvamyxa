...
static int lm_trans_symbol_set_closure_call_name(LmTransSymbol *symbol, const LmP0Text *closure_call_name) {
    ...
    if (symbol == 0 || closure_call_name == 0) {
        return 1;
    }
...
static LmTransCallLowering *lm_trans_expr_call_lowering_new(void) {
    return (LmTransCallLowering *)lm_own_new_zero(sizeof(LmTransCallLowering));
}

static LmTransL4CallableType *lm_trans_expr_callable_type_new(void) {
    return (LmTransL4CallableType *)lm_own_new_zero(sizeof(LmTransL4CallableType));
}
...
static LmTransAbiParam *lm_trans_expr_abi_params_new(size_t capacity) {
    ...
    return (LmTransAbiParam *)lm_own_new_zero(sizeof(LmTransAbiParam) * capacity);
}
...
static LmTransExprPiece *lm_trans_expr_piece_new(void) {
    return (LmTransExprPiece *)lm_own_new_zero(sizeof(LmTransExprPiece));
}
...
static LmTransExprAtomLowering *lm_trans_expr_atom_lowering_new(void) {
    return (LmTransExprAtomLowering *)lm_own_new_zero(sizeof(LmTransExprAtomLowering));
}
...
static LmTransExprLoweredRange *lm_trans_expr_lowered_range_new(void) {
    ...
    if (range != 0) {
        range->pieces = lm_trans_value_stack_new(sizeof(LmTransExprPiece));
        if (range->pieces == 0) {
...
static void lm_trans_expr_lowered_range_destroy(LmTransExprLoweredRange *range) {
    if (range != 0) {
        lm_trans_value_stack_delete(&range->pieces);
        range->index = 0U;
    }
}
...
static void lm_trans_expr_stack_destroy(LmTransExprStack *stack) {
    ...
    if (stack != 0) {
        ...
        if (job != 0) {
            while (stack->jobs != 0 && lm_own_value_stack_pop(stack->jobs, job) == 0) {
                lm_trans_expr_job_destroy(job);
            }
            lm_own_delete(job, 0);
        }
        lm_trans_value_stack_delete(&stack->jobs);
    }
}

static int lm_trans_expr_stack_push(LmTransExprStack *stack, const LmTransExprJob *job) {
...
static int lm_trans_formal_param_default_fields(
    ...
    *out_first = 0;
    ...
    has_callable_type = lm_trans_l4_callable_type_from_node(node, ignored_type);
    lm_own_delete(ignored_type, 0);
...
static int lm_trans_callable_signature_matches_descriptor_name(
    ...
    if (params == 0 || descriptor_type == 0 || candidate_type == 0) {
        ...
        lm_own_delete(descriptor_type, 0);
        lm_own_delete(candidate_type, 0);
...
static int lm_trans_emit_callable_binder(
    ...
    if (descriptor_params == 0 || return_type == 0) {
        lm_own_delete(descriptor_params, 0);
        lm_own_delete(return_type, 0);
        return 1;
    }
...
static int lm_trans_cast_type_base_key(const LmP0Node *type_node, LmP0Text *out_key) {
    ...
    if (type_node->kind == LM_P0_NODE_ATOM) {
        lm_trans_strip_c_prefix(type_node->as->atom, out_key);
        return 1;
    }
...
static int lm_trans_expr_lowered_range_append_atom(
    ...
    piece->first = 0;
    piece->stop = 0;
    status = lm_own_value_stack_push(range->pieces, piece);
    lm_own_delete(piece, 0);
    return status;
}
...
static int lm_trans_expr_piece_emit_node(
    ...
    if (lm_trans_expr_stack_push_node(stack, piece->node) != 0) {
        discard = lm_trans_expr_job_new();
        if (discard != 0) {
            (void)lm_own_value_stack_pop(stack->jobs, discard);
            lm_trans_expr_job_destroy(discard);
            lm_own_delete(discard, 0);
        }
        lm_trans_expr_lowered_range_delete(lowered);
        return 1;
    }
...
static int lm_trans_emit_expr_stack_run(
    ...
    stack->jobs = lm_trans_value_stack_new(sizeof(LmTransExprJob));
    if (stack->jobs == 0) {
...
static int lm_trans_emit_type_node(FILE *file, const LmP0Node *type_node) {
    ...
    if (type_receiver_status > 0) {
        ...
        type_receiver_status = type_receiver->type_emit(file, type_node);
        lm_own_delete(type_receiver, 0);
        return type_receiver_status;
    }
...
static LmTransCDeclarator *lm_trans_statement_c_declarator_new(void) {
    return (LmTransCDeclarator *)lm_own_new_zero(sizeof(LmTransCDeclarator));
}
...
static LmTransFunctionHeader *lm_trans_statement_function_header_new(void) {
    return (LmTransFunctionHeader *)lm_own_new_zero(sizeof(LmTransFunctionHeader));
}
...
static LmTransCapture *lm_trans_statement_capture_new(void) {
    return (LmTransCapture *)lm_own_new_zero(sizeof(LmTransCapture));
}
...
static int lm_trans_emit_statement_list(
    ...
    stack->jobs = lm_trans_value_stack_new(sizeof(LmTransStatementJob));
    if (stack->jobs == 0) {
        lm_own_delete(job, 0);
        lm_own_delete(stack, 0);
        return 1;
...
static int lm_trans_collect_hoisted_functions_from_frame(
    ...
    if (function == 0 || ignored_capture == 0) {
        lm_own_delete(function, 0);
        lm_own_delete(ignored_capture, 0);
        return 1;
    }
...
static int lm_trans_emit_function(
    ...
    hoisted_functions = lm_trans_ptr_stack_new(lm_trans_hoisted_function_delete_any);
...
static int lm_trans_sort_layout_fields(
    ...
    swap = lm_own_new_zero(sizeof(fields[0]));
    if (swap == 0) {
        return 1;
    }
    ...
    for (i = 0U; i < field_count; ++i) {
        for (j = i + 1U; j < field_count; ++j) {
            if (fields[j].index < fields[i].index) {
                memcpy(swap, &fields[i], sizeof(fields[i]));
                memcpy(&fields[i], &fields[j], sizeof(fields[i]));
                memcpy(&fields[j], swap, sizeof(fields[j]));
            }
        }
    }
    lm_own_delete(swap, 0);
    return 0;
}
...
static int lm_trans_emit_layout_field(
    ...
    if (field->is_union) {
        ...
        declarator = (LmTransCDeclarator *)lm_own_new_zero(sizeof(*declarator));
        if (declarator == 0) {
            return 1;
        }
...
static int lm_trans_emit_layout_fields(
    ...
    field_capacity = 256U;
    fields = (LmTransLayoutField *)lm_own_new_zero(sizeof(fields[0]) * field_capacity);
    if (fields == 0) {
        return 1;
    }
    if (
...
static int lm_trans_collect_abi_params(
    ...
    swap = lm_own_new_zero(sizeof(params[0]));
    if (owner_text == 0 || param_text == 0 || swap == 0) {
    ...
    for (i = 0U; i < *out_count; ++i) {
        for (j = i + 1U; j < *out_count; ++j) {
            if (params[j].index < params[i].index) {
                memcpy(swap, &params[i], sizeof(params[i]));
                memcpy(&params[i], &params[j], sizeof(params[i]));
                memcpy(&params[j], swap, sizeof(params[j]));
            }
        }
    }
...
static int lm_trans_registry_push_column_metadata(
    ...
    for (index = 0U; index < column_count; ++index) {
        if (columns[index].name == 0 || !lm_trans_registry_identifier_value(columns[index].name, column_payload)) {
            status = -1;
            goto cleanup;
        }
...
static int lm_trans_l4_callable_type_from_node(
    ...
    frame = node->as->frame;
    inner = (LmTransL4CallableType *)lm_own_new_zero(sizeof(*inner));
    if (inner == 0) {
        return 0;
    }
    if (lm_trans_text_equals(frame->head, "const")) {
...
static int lm_trans_registry_materialize_fn_descriptor_param(
    ...
    param_type = (LmTransL4CallableType *)lm_own_new_zero(sizeof(*param_type));
    param_name = lm_trans_l4_text_view_new("");
    if (param_type == 0 || param_name == 0) {
        lm_own_delete(param_type, 0);
...
static int lm_trans_registry_materialize_fn_descriptor_frame(const LmP0Frame *frame) {
    ...
    function = (LmTransFunctionHeader *)lm_own_new_zero(sizeof(*function));
    return_type = (LmTransL4CallableType *)lm_own_new_zero(sizeof(*return_type));
...
static int lm_trans_registry_load_for_source(const char *source_path) {
    ...
    lm_trans_registry->loaded_paths = lm_trans_ptr_stack_new(free);
    if (
...
static int lm_trans_emit_document(const char *source_path, const char *output_path) {
    ...
    emitted = 0;
    lm_trans_declared_import_paths = lm_trans_ptr_stack_new(free);
    lm_trans_declared_import_documents = lm_trans_ptr_stack_new(lm_trans_import_document_delete);
    lm_trans_emitted_import_prelude_paths = lm_trans_ptr_stack_new(free);
    lm_trans_emitted_import_function_paths = lm_trans_ptr_stack_new(free);
    lm_trans_emitted_callable_adapters = lm_trans_ptr_stack_new(free);
    lm_trans_emitted_callable_binders = lm_trans_ptr_stack_new(free);
    lm_trans_emitted_array_value_helpers = lm_trans_ptr_stack_new(free);
    lm_trans_emitted_function_return_structs = lm_trans_ptr_stack_new(free);
    if (
