...
typedef struct LmTransRegistryColumn {
    LmP0Text name;
    LmP0Text descriptors[16];
    size_t descriptor_count;
} LmTransRegistryColumn;
...
static int lm_trans_identifier_same(LmP0Text left, LmP0Text right) {
    ...
    if (
        !lm_trans_identifier_payload(left, &left_payload) ||
        !lm_trans_identifier_payload(right, &right_payload)
    ) {
        return 0;
    }
...
static int lm_trans_registry_payload_is_null(LmP0Text atom) {
    ...
    if (!lm_trans_registry_identifier_value(atom, &payload)) {
        return 0;
    }
...
static char *lm_trans_registry_value_copy_cstr(LmP0Text value) {
    return lm_trans_text_copy_cstr(value);
}
...
static void lm_trans_p0_document_destroy_any(void *object) {
    lm_p0_document_destroy((LmP0Document *)object);
}

static int lm_trans_registry_index_row(LmTransRegistryRow *row) {
    LmTransIdentifierCard *card;
...
static int lm_trans_registry_push_row_values(
    ...
    if (lm_trans_registry_index_row(row) != 0) {
        lm_own_ptr_stack_pop(&lm_trans_registry.rows);
        lm_trans_registry_row_destroy_any(row);
        return -1;
    }
...
static LmTransRegistryRow *lm_trans_registry_lookup_row_in_identifiers(
    ...
    return (LmTransRegistryRow *)lm_own_ptr_stack_top(&relation->symbols);
}

static LmTransRegistryRow *lm_trans_registry_lookup_row_in_rows(
    LmP0Text key,
    const char *table
) {
    size_t i;
    ...
    if (!lm_trans_identifier_payload(key, &key_payload)) {
        return 0;
    }
    ...
    while (i > 0U) {
        ...
        if (
            ...
            lm_trans_text_equals(key_payload, row->key)
        ) {
            return row;
        }
    }
...
static const char *lm_trans_registry_lookup(LmP0Text key, const char *table) {
    ...
    row = lm_trans_registry_lookup_row_in_rows(key, table);
...
#if !LM_TRANS_ENABLE_LEGACY_COMPARE
    return 0;
#else
    const char *value;
...
static const char *lm_trans_namespace_registry_lookup_compare(
    const LmTransNamespace *namespace_,
    LmP0Text key,
    const char *table,
    const char *legacy_payload
    ...
    registry_payload = lm_trans_namespace_registry_lookup(namespace_, key, table);
    if (
        lm_trans_registry_compare_enabled() &&
        (
            (registry_payload == 0 && legacy_payload != 0) ||
...
static int lm_trans_registry_has_expr_emitter_binding_compare(
    LmP0Text key,
    const char *class_name,
    int legacy_result
) {
    ...
    registry_result = lm_trans_registry_has_expr_emitter_class(key, class_name);
    return lm_trans_bool_compare(class_name, key, registry_result, legacy_result);
}
...
static int lm_trans_registry_has_any_expr_emitter_binding(LmP0Text key) {
    ...
    while (i < lm_trans_registry.rows.count) {
        ...
        if (
            ...
            row->table != 0 &&
            row->payload != 0 &&
            strcmp(row->table, "expr_emitter_binding") == 0
        ) {
            if (
                lm_trans_registry_assert_payload_table_exists(row->table, lm_trans_text_from_cstr(row->key), row->payload) != 0
            ) {
                exit(2);
            }
...
static int lm_trans_text_is_operator_atom(LmP0Text text) {
    ...
    registry_result = lm_trans_registry_has_any_expr_emitter_binding(text);
    return lm_trans_bool_compare(
        "operator",
        text,
        registry_result,
        lm_trans_legacy_text_is_operator_atom(text)
    );
}
...
static FILE *lm_trans_prelude_file(FILE *fallback) {
    return lm_trans_prelude_output != 0 ? lm_trans_prelude_output : fallback;
}
...
static const char *lm_trans_symbol_class_name(const char *class_name) {
    return class_name != 0 ? class_name : "symbol";
}
...
static int lm_trans_symbol_is(const LmTransSymbol *symbol, const char *class_name) {
    return symbol != 0 && lm_trans_symbol_class_is(symbol->class_name, class_name);
}

static int lm_trans_symbol_is_executable_callable(const LmTransSymbol *symbol) {
    return
        symbol != 0 &&
        symbol->has_callable_shape &&
        (
...
static int lm_trans_legacy_is_reserved_head_name(LmP0Text name) {
    return
        ...
        lm_trans_text_equals(name, "include") ||
        lm_trans_text_equals(name, "os") ||
        lm_trans_text_equals(name, "const") ||
        lm_trans_text_equals(name, "()") ||
        lm_trans_text_equals(name, "[]") ||
        lm_trans_text_equals(name, "c.struct") ||
        lm_trans_text_equals(name, "c.union") ||
        lm_trans_text_equals(name, "c.enum") ||
...
static int lm_trans_is_reserved_head_name(LmP0Text name) {
    ...
    registry_result = namespace_class != 0;
    return lm_trans_bool_compare(
        "namespace",
        name,
        registry_result,
        lm_trans_legacy_is_reserved_head_name(name)
    );
}
...
static int lm_trans_head_binding_resolve(
    ...
    out->function_receiver_binding = lm_trans_namespace_registry_lookup_compare(
        namespace_,
        head,
        "receiver.function",
        lm_trans_legacy_function_receiver_payload(head)
    );
...
static const char *lm_trans_class_c_spelling(LmP0Text name) {
    return lm_trans_registry_lookup_compare(name, "class.spelling", lm_trans_legacy_c_spelling(name));
}
...
static int lm_trans_atom_is_prefix_expr_operator(LmP0Text text) {
    return lm_trans_registry_has_expr_emitter_binding_compare(
        text,
        "operator.prefix",
        lm_trans_legacy_atom_is_prefix_expr_operator(text)
    );
}
...
static int lm_trans_atom_is_postfix_expr_operator(LmP0Text text) {
    return lm_trans_registry_has_expr_emitter_binding_compare(
        text,
        "operator.postfix",
        lm_trans_legacy_atom_is_postfix_expr_operator(text)
    );
}
...
static int lm_trans_expected_param_is_callable_descriptor(
    ...
    if (param->kind == LM_P0_NODE_STRUCTURE) {
        ...
        if (field != 0 && field->next == 0) {
            return lm_trans_expected_param_is_callable_descriptor(field->value, namespace_);
        }
        return 0;
    }

    if (param->kind == LM_P0_NODE_ATOM) {
        symbol = lm_trans_namespace_find(namespace_, param->as.atom);
        return lm_trans_symbol_is(symbol, "callableDescriptor");
    }
...
static int lm_trans_expr_stack_try_materialize_segment(
    ...
    binding = lm_trans_registry_lookup(
        lm_trans_text_from_cstr("callable.zero-arg.value"),
    ...
    if (
        ...
        fprintf(
            stderr,
            "trans registry inconsistency: materialize.argument[callable.zero-arg.value] has unknown binding %s\n",
            binding
        );
        return 1;
    }

    return resolved.expr_segment_materializer(stack, segment, namespace_, out_consumed);
}
...
static int lm_trans_expr_stack_push_segments(
    ...
    while (index > 0U) {
        ...
        if (lm_trans_expr_stack_try_materialize_segment(stack, segment, namespace_, &materialized) != 0) {
            return 1;
        }
        if (!materialized) {
...
static int lm_trans_expr_stack_schedule_call_args(
    ...
    if (status == 0) {
        status = lm_trans_expr_stack_push_segments(stack, &segments, namespace_);
    }
...
static int lm_trans_atom_is_index_operator(LmP0Text text) {
    return lm_trans_registry_has_expr_emitter_binding_compare(
        text,
        "operator.index",
        lm_trans_legacy_atom_is_index_operator(text)
    );
}
...
static int lm_trans_emit_type_head_and_name(
    FILE *file,
    LmP0Text type_head,
    LmP0Text name
) {
    if (lm_trans_builtin_c_type_name(type_head)) {
        if (lm_trans_emit_type_name(file, type_head) != 0) {
            return 1;
        }
    } else {
        if (lm_trans_emit_name(file, type_head) != 0) {
            return 1;
        }
    }
    if (lm_trans_put(file, " ") != 0) {
...
static int lm_trans_emit_array_param(FILE *file, const LmP0Frame *frame, LmTransNamespace *namespace_) {
    ...
    while (lm_trans_array_head_next_dimension(frame->head, &head_index, &head_dimension)) {
        ...
        if (head_dimension.length != 0U) {
            ...
            lm_p0_document_destroy(dimension_document);
        } else if (dimension_field != 0 && dimension_field->value != 0) {
    ...
    if (dimension_field != 0) {
        fprintf(stderr, "trans L2 error: array parameter does not accept initializer/default fields\n");
        return 1;
    }
...
static int lm_trans_frame_positional_name_index(const LmP0Frame *frame, size_t *out_index) {
    ...
    if (lm_trans_legacy_frame_has_positional_name_argument(frame->head)) {
        *out_index = 0U;
        return 1;
    }
...
static int lm_trans_emit_array_declaration_with_qualifier(
    ...
    if (type_is_head) {
        if (lm_trans_emit_type_head_and_name(file, type_head, name_node->as.atom) != 0) {
            return 1;
        }
    } else {
        if (
        ...
        if (head_dimension.length != 0U) {
            ...
            if (parse_status != 0) {
                ...
                if (diagnostic != 0) {
                    fprintf(
                        stderr,
                        "trans L2 error: array dimension parse error %d at %zu:%zu: %s\n",
                        diagnostic->code,
                        diagnostic->line,
                        diagnostic->column,
                        diagnostic->message
                    ...
                    fprintf(stderr, "trans L2 error: array dimension parse failed\n");
                }
                if (dimension_document != 0) {
                    lm_p0_document_destroy(dimension_document);
                }
            ...
            if (
                ...
                fprintf(stderr, "trans L2 error: array dimension must not be empty\n");
                lm_p0_document_destroy(dimension_document);
                return 1;
            }
            if (lm_trans_emit_expr_fields(file, dimension_root->as.structure.first_field, namespace_) != 0) {
            ...
            if (dimension_field == 0 || dimension_field->value == 0) {
                fprintf(stderr, "trans L2 error: [] array dimension expects a size expression\n");
                return 1;
            }
            if (lm_trans_emit_expr_node(file, dimension_field->value, namespace_) != 0) {
                return 1;
    ...
    initializer_field = dimension_field;
...
static int lm_trans_emit_array_declaration_repeat_with_qualifier(
    ...
    dimension_field = name_field->next;
    head_index = 0U;
    while (lm_trans_array_head_next_dimension(template_frame->head, &head_index, &head_dimension)) {
        if (lm_trans_put(file, "[") != 0) {
            return 1;
        }
        if (head_dimension.length != 0U) {
            ...
            if (dimension_field == 0 || dimension_field->value == 0) {
                fprintf(stderr, "trans L2 error: repeated [] array dimension expects a size expression\n");
                return 1;
            }
            if (lm_trans_emit_expr_node(file, dimension_field->value, namespace_) != 0) {
                return 1;
...
static int lm_trans_emit_assignment_target(FILE *file, LmP0Text target) {
    ...
    while (i < target_payload.length) {
        if (target_payload.data[i] == '\\') {
            if (lm_trans_put(file, "->") != 0) {
                return 1;
            }
        } else if (lm_trans_write_all(file, target_payload.data + i, 1U) != 0) {
            return 1;
        }
        ++i;
    }
...
static int lm_trans_emit_registry_include_table(FILE *output);
...
static int lm_trans_atom_statement_emit_include_prelude(
    ...
    (void)indent;
    (void)namespace_;
    return lm_trans_emit_registry_include_table(lm_trans_prelude_file(file));
}
...
static int lm_trans_registry_note_callable_descriptor(
    const LmTransFunctionHeader *function
) {
    ...
    if (
        ...
        function->return_node->kind == LM_P0_NODE_ATOM &&
        lm_trans_registry_push_row_atoms(
            lm_trans_text_from_cstr("callable.result"),
            function->name,
            function->return_node->as.atom
        ) != 0
    ) {
        return 1;
    }
...
static int lm_trans_top_level_declare_function(
    ...
    if (lm_trans_registry_note_callable_descriptor(&item->function) != 0) {
        return 1;
    }
...
static int lm_trans_top_level_emit_atom_include(
    ...
    (void)namespace_;
    (void)item;
    return lm_trans_emit_registry_include_table(file);
}
...
static int lm_trans_lower_top_level_item(
    ...
    if (lm_trans_text_equals(out->frame->head, "L1")) {
        out->emit_before_functions = lm_trans_top_level_emit_l1;
        out->emits_top_level = 1;
        return 0;
    }

    if (lm_trans_text_equals(out->frame->head, "os")) {
    ...
    if (lm_trans_text_equals(out->frame->head, "include")) {
...
static int lm_trans_emit_l2_structure_with_namespace(
    ...
    while (field != 0) {
        ...
        if (item.emit_before_functions != 0) {
            if (emitted_prelude && lm_trans_put(prelude_file, "\n") != 0) {
                return 1;
            }
            status = item.emit_before_functions(prelude_file, namespace_, &item);
        } else {
            status = 0;
        }
...
static int lm_trans_declare_l2_registry_os_table(LmTransNamespace *namespace_) {
    ...
    emitted_default = 0;
    emitted = 0;
    for (i = 0U; i < lm_trans_registry.rows.count; ++i) {
        row = (LmTransRegistryRow *)lm_own_ptr_stack_at(&lm_trans_registry.rows, i);
        if (
            row != 0 &&
            row->table != 0 &&
...
static int lm_trans_registry_collect_layout_names(LmOwnPtrStack *names) {
    size_t i;
    LmTransRegistryRow *row;
    char *name;
    LmP0Text key_text;
    ...
    for (i = 0U; i < lm_trans_registry.rows.count; ++i) {
        ...
        if (
            ...
            row->key != 0 &&
            row->payload != 0 &&
            strcmp(row->table, "layout.backend") == 0 &&
            strcmp(row->payload, "c.struct") == 0
        ) {
            key_text = lm_trans_text_from_cstr(row->key);
            if (!lm_trans_registry_has(key_text, "class.present")) {
...
static char *lm_trans_layout_relation_name_new(const char *layout_name, const char *relation) {
    size_t layout_length;
    size_t relation_length;
    char *name;

    if (layout_name == 0 || relation == 0) {
        return 0;
    }

    layout_length = strlen(layout_name);
    relation_length = strlen(relation);
    name = (char *)malloc(layout_length + 1U + relation_length + 1U);
    if (name == 0) {
        return 0;
    }
    memcpy(name, layout_name, layout_length);
    name[layout_length] = '.';
    memcpy(name + layout_length + 1U, relation, relation_length + 1U);
    return name;
}
...
static int lm_trans_emit_layout_field_type(
    ...
    if (file == 0 || field == 0 || field->class_name == 0) {
        return 1;
    }

    if (field->is_const && lm_trans_put(file, "const ") != 0) {
        return 1;
    }
    if (lm_trans_emit_type_name(file, lm_trans_text_from_cstr(field->class_name)) != 0) {
        return 1;
    }
    if (field->address_depth != 0U && lm_trans_put(file, " ") != 0) {
        return 1;
    }
    for (i = 0U; i < field->address_depth; ++i) {
        if (lm_trans_put(file, "*") != 0) {
            return 1;
        }
    }
...
static int lm_trans_emit_layout_typedef(FILE *file, const char *layout_name) {
    ...
    class_table = lm_trans_layout_relation_name_new(layout_name, "field.class");
    index_table = lm_trans_layout_relation_name_new(layout_name, "field.index");
    address_depth_table = lm_trans_layout_relation_name_new(layout_name, "field.address-depth");
    const_table = lm_trans_layout_relation_name_new(layout_name, "field.const");
    if (class_table == 0 || index_table == 0 || address_depth_table == 0 || const_table == 0) {
        free(class_table);
        free(index_table);
        free(address_depth_table);
        free(const_table);
        return 1;
    }
    ...
    for (i = 0U; i < lm_trans_registry.rows.count; ++i) {
        ...
        if (
            ...
            row->key != 0 &&
            row->payload != 0 &&
            strcmp(row->table, class_table) == 0
        ) {
            ...
            index_payload = lm_trans_registry_lookup(lm_trans_text_from_cstr(row->key), index_table);
            address_depth_payload = lm_trans_registry_lookup(lm_trans_text_from_cstr(row->key), address_depth_table);
            const_payload = lm_trans_registry_lookup(lm_trans_text_from_cstr(row->key), const_table);
            if (!lm_trans_parse_size_payload(index_payload, &index)) {
            ...
            if (
                address_depth_payload != 0 &&
                !lm_trans_parse_size_payload(address_depth_payload, &fields[field_count].address_depth)
            ) {
            ...
            if (const_payload != 0) {
                if (!lm_trans_parse_size_payload(const_payload, &index)) {
...
static int lm_trans_emit_l4_layout_typedefs(FILE *file) {
    LmOwnPtrStack names;
    size_t i;
    const char *name;

    lm_own_ptr_stack_init(&names, lm_trans_free_any);
    if (lm_trans_registry_collect_layout_names(&names) != 0) {
    ...
    for (i = 0U; i < names.count; ++i) {
        ...
        if (name != 0 && lm_trans_emit_layout_typedef(file, name) != 0) {
            lm_own_ptr_stack_destroy(&names);
            return 1;
        }
    }
...
static int lm_trans_registry_collect_constant_define_names(LmOwnPtrStack *names) {
    size_t i;
    LmTransRegistryRow *row;
    char *name;
    LmP0Text key_text;
    ...
    for (i = 0U; i < lm_trans_registry.rows.count; ++i) {
        ...
        if (
            ...
            row->key != 0 &&
            row->payload != 0 &&
            strcmp(row->table, "constant.backend") == 0 &&
...
static int lm_trans_emit_l4_constant_defines(FILE *file) {
    LmOwnPtrStack names;
    size_t i;
    const char *name;
    const char *value;
    ...
    lm_own_ptr_stack_init(&names, lm_trans_free_any);
    if (lm_trans_registry_collect_constant_define_names(&names) != 0) {
        lm_own_ptr_stack_destroy(&names);
        return 1;
    }
    ...
    for (i = 0U; i < names.count; ++i) {
        name = (const char *)lm_own_ptr_stack_at(&names, i);
        value = lm_trans_registry_lookup(lm_trans_text_from_cstr(name), "constant.value");
        if (value == 0) {
...
static void lm_trans_registry_destroy(void) {
    if (lm_trans_registry.loaded) {
        ...
        lm_own_ptr_stack_destroy(&lm_trans_registry.documents);
        lm_trans_registry.loaded = 0;
    }
}

static int lm_trans_registry_row_from_frame(const LmP0Frame *frame, int allow_node_cells) {
    ...
    if (
        ...
        fprintf(stderr, "trans registry error: row expects table and key atoms plus one payload field\n");
        return -1;
    }

    if (payload_field->value->kind == LM_P0_NODE_ATOM) {
        if (
            lm_trans_registry_push_row_atoms(
        ...
        if (!allow_node_cells) {
            fprintf(stderr, "trans registry error: node row payloads are allowed only in inline L4 for now\n");
            return -1;
        }
        if (
            !lm_trans_registry_identifier_value(table_field->value->as.atom, &table_value) ||
            !lm_trans_registry_identifier_value(key_field->value->as.atom, &key_value) ||
            lm_trans_registry_push_row_node_values(table_value, key_value, payload_field->value) != 0
        ) {
            return -1;
        }
    }
...
static int lm_trans_registry_frame_single_atom(
    ...
    if (head != 0 && !lm_trans_text_equals(frame->head, head)) {
        return 0;
    }
...
static int lm_trans_registry_column_name(
    const LmP0Field *field,
    LmTransRegistryColumn *out_column
) {
    const LmP0Node *node;
    const LmP0Field *body_field;
    size_t descriptor_count;
    ...
    if (node->kind == LM_P0_NODE_FRAME) {
        if (lm_trans_registry_frame_single_atom(&node->as.frame, 0, &out_column->name) <= 0) {
            return -1;
        }
    ...
    while (body_field != 0 && lm_trans_node_is_ignored(body_field->value)) {
    ...
    body_field = body_field->next;
    while (body_field != 0) {
        if (body_field->value != 0 && !lm_trans_node_is_ignored(body_field->value)) {
...
static int lm_trans_registry_columns_from_frame(
    ...
    if (
        ...
        out_count == 0 ||
        columns_capacity == 0U ||
        !lm_trans_text_equals(frame->head, "columns")
    ) {
        return 0;
    }
    ...
    while (field != 0) {
        if (field->value != 0 && !lm_trans_node_is_ignored(field->value)) {
            status = lm_trans_registry_column_name(field, &column);
            if (status <= 0) {
                fprintf(stderr, "trans registry error: columns expects atoms or anonymous descriptor structures\n");
                return -1;
            }
            if (count >= columns_capacity) {
                fprintf(stderr, "trans registry error: table has too many columns\n");
                return -1;
            }
    ...
    if (count < 2U) {
        fprintf(stderr, "trans registry error: table expects at least two columns\n");
        return -1;
    }
    if (
        !lm_trans_registry_identifier_value(columns[0].name, &first_column) ||
        !lm_trans_text_equals(first_column, "class")
    ) {
...
static int lm_trans_registry_rows_from_frame(
    ...
    size_t column_index;
    int split_by_column;
    int first_column_is_class_typed;
    int cell_column_is_class_typed;

    if (frame == 0 || !lm_trans_text_equals(frame->head, "rows")) {
        return 0;
    }
    if (columns == 0 || column_count < 2U) {
        fprintf(stderr, "trans registry error: rows require at least two columns\n");
        return -1;
    }
    ...
    first_column_is_class_typed = lm_trans_registry_column_is_class_typed(&columns[0]);
    field = frame->body.first_field;
    while (field != 0) {
        if (field->value != 0 && !lm_trans_node_is_ignored(field->value)) {
            ...
            if (column_index == 0U) {
                ...
                if (
                    first_column_is_class_typed &&
                    lm_trans_registry_note_class_present(key_node->as.atom) != 0
                ) {
                    return -1;
                }
            } else {
                cell_node = field->value;
                cell_column_is_class_typed = lm_trans_registry_column_is_class_typed(&columns[column_index]);
                if (
                    key_node == 0 ||
                ...
                if (
                    cell_column_is_class_typed &&
                    cell_node->kind == LM_P0_NODE_ATOM &&
                    lm_trans_registry_payload_is_null(cell_node->as.atom) == 0 &&
                    lm_trans_registry_note_class_present(cell_node->as.atom) != 0
                ) {
                    return -1;
                }
            }
...
static int lm_trans_registry_validate_table_trailer(
    ...
    if (
        !lm_trans_registry_identifier_value(actual, &actual_payload) ||
        !lm_trans_registry_identifier_value(table_name, &expected_payload) ||
...
static int lm_trans_registry_table_from_frame(const LmP0Frame *frame, int allow_node_cells) {
    const LmP0Field *field;
    const LmP0Node *node;
    LmTransRegistryColumn columns[128];
    LmP0Text table_name;
    size_t column_count;
    int have_name;
    int have_columns;
    ...
    while (field != 0) {
        ...
        if (node != 0 && !lm_trans_node_is_ignored(node)) {
            ...
            if (status > 0) {
                ...
                if (!have_name) {
                    fprintf(stderr, "trans registry error: table columns must appear after name\n");
                    return -1;
                }
                status = lm_trans_registry_columns_from_frame(
                    &node->as.frame,
                    columns,
                ...
                if (lm_trans_registry_push_column_metadata(table_name, columns, column_count) != 0) {
                    fprintf(stderr, "trans registry error: cannot store table column metadata\n");
                    return -1;
                }
                have_columns = 1;
                ...
                status = lm_trans_registry_rows_from_frame(
                    &node->as.frame,
                    table_name,
                    columns,
                    column_count,
...
static int lm_trans_registry_load_rows(const LmP0Structure *structure, int allow_node_cells) {
    ...
    while (field != 0) {
        ...
        if (!lm_trans_node_is_ignored(node)) {
            ...
            status = lm_trans_registry_row_from_frame(&node->as.frame, allow_node_cells);
            if (status <= 0) {
                if (status < 0) {
                    return 1;
                }
                status = lm_trans_registry_table_from_frame(&node->as.frame, allow_node_cells);
                if (status <= 0) {
                    if (status == 0) {
                        fprintf(stderr, "trans registry error: registry body expects table or row frames\n");
                    }
...
static int lm_trans_registry_seen_table_add(LmOwnPtrStack *seen, LmP0Text table_name) {
    LmP0Text payload;
    char *name;
    size_t i;
    char *existing;
    ...
    if (!lm_trans_registry_identifier_value(table_name, &payload)) {
        return -1;
    }
    ...
    while (i < seen->count) {
        ...
        if (existing != 0 && lm_trans_text_equals(payload, existing)) {
            return 1;
        }
        ++i;
    }

    name = lm_trans_text_copy_cstr(payload);
    if (name == 0) {
        return -1;
    }
...
static int lm_trans_registry_check_table_frame_unique(
    const LmP0Frame *frame,
    LmOwnPtrStack *seen
) {
    const LmP0Field *field;
    ...
    status = lm_trans_registry_seen_table_add(seen, table_name);
    if (status < 0) {
        fprintf(stderr, "trans registry error: cannot record table name\n");
        return -1;
    }
    if (status > 0) {
        if (lm_trans_registry_identifier_value(table_name, &payload)) {
            fprintf(
                stderr,
                "trans registry error: duplicate table in one L4 schema: %.*s\n",
                (int)payload.length,
                payload.data
            );
        } else {
            fprintf(stderr, "trans registry error: duplicate table in one L4 schema\n");
        }
        return -1;
    }
...
static int lm_trans_registry_check_duplicate_tables_in_rows(
    const LmP0Structure *structure,
    LmOwnPtrStack *seen
) {
    const LmP0Field *field;
    ...
    while (field != 0) {
        ...
        if (
            !lm_trans_node_is_ignored(node) &&
            node->kind == LM_P0_NODE_FRAME &&
            lm_trans_text_equals(node->as.frame.head, "table") &&
            lm_trans_registry_check_table_frame_unique(&node->as.frame, seen) != 0
        ) {
            return -1;
        }
        field = field->next;
...
static int lm_trans_registry_check_duplicate_tables_in_root(
    const LmP0Node *root,
    int implicit_l4
) {
    const LmP0Field *field;
    ...
    while (field != 0 && status == 0) {
        node = field->value;
        if (!lm_trans_node_is_ignored(node) && node->kind == LM_P0_NODE_FRAME) {
            if (
                lm_trans_text_equals(node->as.frame.head, "L4") ||
                lm_trans_text_equals(node->as.frame.head, "registry")
            ) {
                status = lm_trans_registry_check_duplicate_tables_in_rows(&node->as.frame.body, &seen);
            } else if (implicit_l4 && lm_trans_text_equals(node->as.frame.head, "table")) {
                status = lm_trans_registry_check_table_frame_unique(&node->as.frame, &seen);
            }
        }
        field = field->next;
    }
...
static int lm_trans_registry_load_root(const LmP0Node *root, int implicit_l4, int allow_node_cells) {
    ...
    if (lm_trans_registry_check_duplicate_tables_in_root(root, implicit_l4) != 0) {
        return 1;
    }
    ...
    while (field != 0) {
        ...
        if (!lm_trans_node_is_ignored(node)) {
            if (node->kind != LM_P0_NODE_FRAME) {
                fprintf(stderr, "trans registry error: root fields must be L4/registry frames, or table/row frames in .lm4 files\n");
                return 1;
            }
            if (
                lm_trans_text_equals(node->as.frame.head, "L4") ||
                ...
                if (lm_trans_registry_load_rows(&node->as.frame.body, allow_node_cells) != 0) {
                    return 1;
                }
                if (lm_trans_validate_end_trailer(&node->as.frame) != 0) {
                    return 1;
                ...
                loaded = 1;
            } else if (implicit_l4 && lm_trans_text_equals(node->as.frame.head, "row")) {
                if (lm_trans_registry_row_from_frame(&node->as.frame, allow_node_cells) <= 0) {
                    return 1;
                }
                loaded = 1;
            } else if (implicit_l4 && lm_trans_text_equals(node->as.frame.head, "table")) {
                if (lm_trans_registry_table_from_frame(&node->as.frame, allow_node_cells) <= 0) {
                    return 1;
                }
                loaded = 1;
            } else {
...
static int lm_trans_registry_load_file_path(
    ...
    status = lm_trans_registry_load_root(
        ...
        lm_trans_path_has_extension(registry_path, ".lm4"),
        1
    );
    if (status != 0) {
        lm_p0_document_destroy(document);
    ...
    if (lm_own_ptr_stack_push(&lm_trans_registry.documents, document) != 0) {
        lm_p0_document_destroy(document);
        return 1;
    }
...
static int lm_trans_registry_candidate_path(
    ...
    if (strncmp(candidate_name, "lm2/", 4U) == 0) {
        ...
        strcpy(registry_path, candidate_name);
        return 0;
    }
...
static int lm_trans_registry_load_for_source(const char *source_path) {
    ...
    lm_own_ptr_stack_init(&lm_trans_registry.documents, lm_trans_p0_document_destroy_any);
...
static int lm_trans_emit_document(const char *source_path, const char *output_path) {
    ...
    if (lm_trans_registry_load_inline_root(root) != 0) {
        lm_p0_document_destroy(document);
        lm_trans_registry_destroy();
        return 1;
    }
    ...
    emitted = 0;
    status = lm_trans_emit_l4_layout_typedefs(prelude_output);
    if (status == 0) {
        status = lm_trans_emit_l4_constant_defines(prelude_output);
    }
    if (status == 0) {
