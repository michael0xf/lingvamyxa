...
static int lm_trans_head_binding_resolve(
    ...
    out->function_receiver_binding = lm_trans_registry_lookup_compare(
        head,
        "receiver.function",
    ...
    out->statement_receiver_binding = lm_trans_registry_lookup(head, "receiver.statement");
...
static int lm_trans_namespace_declare_registry_class_cstr(
    ...
    text = lm_trans_text_from_cstr(name);
    existing = lm_trans_namespace_find(namespace_, text);
    if (lm_trans_symbol_is(existing, "class")) {
        return 0;
    }

    symbol = lm_trans_symbol_new(text, "class", namespace_->depth);
    if (symbol == 0) {
        return 1;
    }
    if (lm_trans_symbol_set_owned_name(symbol, text) != 0) {
        lm_trans_symbol_destroy(symbol);
        return 1;
    }
    if (lm_own_ptr_stack_push(&namespace_->items, symbol) != 0) {
...
static int lm_trans_namespace_declare_registry_table_classes(
    LmTransNamespace *namespace_
) {
    size_t i;
    LmTransRegistryRow *row;
    ...
    for (i = 0U; i < lm_trans_registry.rows.count; ++i) {
        ...
        if (
            ...
            row->table != 0 &&
            row->key != 0 &&
            strcmp(row->table, "class.present") == 0
        ) {
            if (lm_trans_namespace_declare_registry_class_cstr(namespace_, row->key) != 0) {
                return 1;
            }
        }
    }
...
static int lm_trans_emit_l2_structure_with_namespace(
    ...
    if (declare_items) {
        if (lm_trans_namespace_declare_registry_table_classes(namespace_) != 0) {
            return 1;
        }
