...
int l1_hdr_register_path(const char * in_path, int depth);
int l1_path_is_absolute(const char * path);
char * l1_getcwd(char * buffer, size_t size);
...
int l1_c_atom_nested_receiver(const LmP0Text * text, size_t start)
{
    size_t i;
    size_t k;
    int depth;
...
int l1_c_ident(FILE * out, const LmP0Text * text, const char * path, const LmP0Node * node)
{
    ...
    if (start == 2U && l1_c_atom_nested_receiver(text, start) != 0) {
...
int l1_emit_define_repl_structure(FILE * out, const LmP0Structure * body, const char * path, const LmP0Node * owner)
{
    LmP0Field * field;
    const LmP0Node * node;
    const LmP0Text * atom;
    ...
    node = field -> value;
    if (node == 0) {
    return l1_error(path, owner, "define receiver expects atom tokens");
    }
    if (first == 0) {
    if (l1_write_cstr(out, " ") != 0) {
    return 1;
    ...
    first = 0;
    if (node -> kind == LM_P0_NODE_STRUCTURE) {
    if (l1_emit_define_repl_structure(out, node->as->structure, path, node) != 0) {
    return 1;
    }
    }
    else {
...
int l1_emit_define(FILE * out, const LmP0Frame * frame, const char * path, const LmP0Node * owner, int ifndef_default)
{
    ...
    if (field != 0 && field -> value != 0 && field -> value -> kind == LM_P0_NODE_FRAME) {
    if (ifndef_default != 0 || l1_hdr_emitting == 0) {
    return l1_error(path, owner, "function-like macro");
    }
    call = field -> value -> as -> frame;
    atom = call -> head;
    ...
    param = 0;
    if (call -> body != 0) {
    param = call -> body -> first_field;
    }
    while (param != 0) {
    if (l1_node_ignored(param->value) == 0) {
    if (param -> value == 0 || param -> value -> kind != LM_P0_NODE_ATOM) {
    return l1_error(path, param->value, "function-like macro parameter expects an identifier");
    }
    atom = param -> value -> as -> atom;
    if (atom == 0 || atom -> data == 0 || atom -> length == 0U || atom -> data[0] == 34 || atom -> data[0] == 39) {
    return l1_error(path, param->value, "function-like macro parameter expects an identifier");
    }
    start = 0U;
    ...
    if (n == 0U || l1_ident_is_reserved(atom) != 0) {
    return l1_error(path, param->value, "function-like macro parameter expects an identifier");
    }
    if (first_param == 0) {
    if (l1_write_cstr(out, ", ") != 0) {
    ...
    if (node -> kind == LM_P0_NODE_STRUCTURE && l1_hdr_emitting != 0 && ifndef_default == 0) {
    if (l1_write_cstr(out, " ") != 0) {
    return 1;
    }
    if (l1_emit_define_repl_structure(out, node->as->structure, path, node) != 0) {
...
int l1_hdr_include_path(char * dst, size_t cap, const char * in_path, const LmP0Node * node)
{
    ...
    if (l1_unit_id_from_source(dst, cap, in_path, in_path, node) != 0) {
    return 1;
    }
    n = strlen(dst);
    if (n < 6U || l1_path_is_h_lm1(dst) == 0) {
    return l1_error(in_path, node, "generated header name too long");
    }
    memcpy(dst + n - 6U, ".lm1.h", 6U);
    return 0;
}
int l1_hdr_register_unit(const LmP0Node * root, const char * path, int depth)
{
    LmP0Field * field;
    LmP0Field * inner;
    const LmP0Node * node;
    ...
    while (field != 0) {
    node = field -> value;
    if (l1_node_ignored(node) == 0 && node != 0 && node -> kind == LM_P0_NODE_FRAME && l1_text_eq(node->as->frame->head, "predef")) {
    inner = 0;
    if (node -> as -> frame -> body != 0) {
    inner = node -> as -> frame -> body -> first_field;
    }
    while (inner != 0) {
...
int l1_hdr_mark_unit_emitted(const LmP0Node * root, const char * path)
{
    ...
    name = 0;
    if (l1_text_eq(node->as->frame->head, "struct") || l1_text_eq(node->as->frame->head, "enum") || l1_text_eq(node->as->frame->head, "fnptr")) {
    name = l1_hdr_frame_name(node->as->frame);
    }
    if (l1_text_eq(node->as->frame->head, "type")) {
    name = l1_hdr_frame_name(node->as->frame);
    }
    if (name != 0) {
    if (l1_ident_to_buf(nbuf, 64U, name) != 0) {
    return l1_error(path, node, "type name too long");
    }
    if (l1_hdr_name_list_add(l1_hdr_emitted, &l1_hdr_emittedn, 2048U, nbuf) != 0) {
    return l1_error(path, node, "too many header types");
    }
    }
    }
    field = field -> next;
...
int l1_hdr_register_path(const char * in_path, int depth)
{
    ...
    const LmP0Node * root;
    int status;
    if (in_path == 0 || l1_path_is_h_lm1(in_path) == 0) {
    return l1_error(in_path, 0, "header unit predef expects a header source");
    }
...
int l1_hdr_field_byval_name(const LmP0Node * node, char * buf, size_t cap)
{
    ...
    if (l1_hdr_kind_of_cstr(buf) != 115 && l1_hdr_kind_of_cstr(buf) != 102) {
    return 0;
    }
    return 1;
}
int l1_emit_hdr_field_decl(FILE * out, const LmP0Node * node, const char * path, int with_indent, int force_const)
{
    ...
    hi = 0U;
    if (head != 0 && head -> data != 0) {
    while (hi + 1U < head -> length && head -> data[hi] == 91 && head -> data[hi + 1U] == 93) {
    rank = rank + 1;
    hi = hi + 2U;
    }
    }
...
int l1_hdr_check_item(const LmP0Node * node, const char * path)
{
    ...
    if (field == 0 || field -> value == 0 || field -> value -> kind != LM_P0_NODE_ATOM) {
    return l1_error(path, node, "type alias missing name");
    }
    ...
    if (field == 0) {
    return l1_error(path, node, "type alias missing type");
    }
    while (field != 0) {
    if (l1_node_ignored(field->value) == 0) {
    if (field -> value == 0 || field -> value -> kind != LM_P0_NODE_ATOM) {
    return l1_error(path, field->value, "type alias expects type name atoms");
    }
    if (l1_is_type_name(field->value->as->atom) == 0) {
    return l1_error(path, field->value, "type alias expects type name atoms");
    }
    }
    field = field -> next;
    }
    return l1_hdr_type_add(name, path, node, 97);
    }
    ...
    if (l1_text_eq(frame->head, "predef")) {
    return 0;
    }
    if (l1_text_eq(frame->head, "external") || l1_text_eq(frame->head, "import") || l1_text_eq(frame->head, "os") || l1_text_eq(frame->head, "ifdef")) {
    return l1_error(path, node, "executable body");
    }
    return l1_error(path, node, "unsupported header unit form");
}
...
int l1_emit_hdr_type_alias(FILE * out, const LmP0Frame * frame, const char * path, const LmP0Node * node)
{
    LmP0Field * field;
    const LmP0Text * name;
    const LmP0Node * tnode;
    if (frame == 0 || frame -> body == 0) {
    return l1_error(path, node, "type alias missing name");
    }
    field = frame -> body -> first_field;
    ...
    if (l1_node_ignored(field->value) == 0) {
    tnode = field -> value;
    if (tnode == 0 || tnode -> kind != LM_P0_NODE_ATOM) {
    return l1_error(path, tnode, "type alias expects type name atoms");
    }
    if (l1_write_cstr(out, " ") != 0) {
    return 1;
    }
    if (l1_write_type_spelling(out, tnode->as->atom, path, tnode) != 0) {
    return 1;
    }
    }
    field = field -> next;
...
int l1_hdr_emit_predef_includes(FILE * out, const LmP0Frame * frame, const char * path)
{
    LmP0Field * field;
    const LmP0Node * node;
    char imported[1040];
...
int l1_emit_header_unit(FILE * out, const LmP0Node * root, const char * in_path, const char * unit_id)
{
    ...
    if (l1_text_eq(node->as->frame->head, "predef")) {
    if (l1_hdr_emit_predef_includes(out, node->as->frame, in_path) != 0) {
    return 1;
    }
    }
    ...
    while (field != 0) {
    node = field -> value;
    if (l1_node_ignored(node) == 0 && node != 0 && node -> kind == LM_P0_NODE_FRAME && l1_text_eq(node->as->frame->head, "type")) {
    if (l1_emit_hdr_type_alias(out, node->as->frame, in_path, node) != 0) {
    return 1;
    }
    }
    field = field -> next;
