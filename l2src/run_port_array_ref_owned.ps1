# Runtime-module parity runner: lmx_array_ref_owned (review-chat lane, 2026-09-14).
#
...
# ABI names: a unit's open path builds its root arrays through
# lmx_array_ref_new_positive_owned, so a wrapper under a real name would
# re-enter l2_library_open before the unit's graph exists. The unit drops the
# lmx_ prefix, the SELFTEST is redirected onto the two public names with -D,
# the runtime keeps the handwritten module under the real names, and nm
# proves the generated object defines the unit names and none of the ABI
# names.
#
...
# selftest's main, with tracking still off: an empty LMX Array (descriptor,
# no backing), a positive one (zeroed backing), a scalar type refused, then
# disposed.
#
...
$out = Join-Path $baseline "build/port_array_ref_owned/$stamp"
...
$redirects = @(
    @{ abi = 'lmx_array_ref_new_owned'; unit = 'array_ref_new_owned' }
    @{ abi = 'lmx_array_ref_new_positive_owned'; unit = 'array_ref_new_positive_owned' }
)
...
$referenceNames = @('lmx_msg_blocks', 'lmx_owned_ranges', 'lmx_msg_storage', 'lmx_array_ref_owned')
...
# ---------------------------------------------------------------------------
# 2. Reference run, linked as l2src/run_array_ref_owned.ps1 links it.
# ---------------------------------------------------------------------------
$selfSrc = 'l2src/tests/lmx_array_ref_owned_selftest.lm1'
...
if ($refOut -notmatch 'array_ref_owned checks=(\d+) failures=0 allocations=(\d+) releases=(\d+)') { throw "reference selftest did not report zero failures: $refOut" }
...
$lm2 = 'l2src/lmx_array_ref_owned.lm2'
...
# realloc; the element stride is sizeof a pointer probe, never a literal.
...
if ($genText -notmatch 'c\.sizeof\(backing\)') { throw 'the element stride is not sizeof the pointer probe' }
...
int array_ref_selftest_main(void);
int main(void) {
    ...
    empty = lmx_array_ref_new_owned(LMX_TYPE_ARRAY_OF_LMX, 0, &blocks, &ranges);
    if (empty == 0 || empty->len != 0 || empty->data != 0) { fprintf(stderr, "warm-up: empty array\n"); return 90; }
    three = lmx_array_ref_new_owned(LMX_TYPE_ARRAY_OF_LMX, 3, &blocks, &ranges);
    if (three == 0 || three->len != 3 || three->data == 0) { fprintf(stderr, "warm-up: positive array\n"); return 91; }
    cells = (void **)three->data;
    if (cells[0] != 0 || cells[1] != 0 || cells[2] != 0) { fprintf(stderr, "warm-up: backing not zeroed\n"); return 92; }
    if (lmx_array_ref_new_owned(LMX_TYPE_ARRAY_OF_INT, 1, &blocks, &ranges) != 0) { fprintf(stderr, "warm-up: scalar type not refused\n"); return 93; }
    if (lmx_array_ref_new_positive_owned(LMX_TYPE_ARRAY_OF_LMX, 0, &blocks, &ranges) != 0) { fprintf(stderr, "warm-up: positive constructor accepted zero\n"); return 94; }
    lmx_msg_blocks_dispose_all(&blocks);
    fprintf(stderr, "warm-up ok\n");
    return array_ref_selftest_main();
}
'@.Replace("`r`n", "`n") | Set-Content -LiteralPath $driverC -Encoding ascii -NoNewline

$selfDrivenObj = Join-Path $out 'selftest_driven.o'
Step 'selftest_driven_compile' (Invoke-Native ("gcc $cflags $redirect -Dmain=array_ref_selftest_main -I " + (Q $hdrs) + ' -c ' + (Q $selfC) + ' -o ' + (Q $selfDrivenObj)) (Join-Path $out 'selftest_driven.gcc.log')) (Join-Path $out 'selftest_driven.gcc.log')
...
Write-Output "lmx_array_ref_owned parity PASS: $refChecks checks, 0 failures, $($ev.reference.allocations) counted allocations released, reference and generated agree on both runs, selftest redirected onto the unit's names; evidence $evPath"
