...
param([ValidateSet('1','2','both')][string]$Part='both',
...
if ($Part -eq '2' -or $Part -eq 'both') {
    ...
    function New-GatesCopy([string]$fromText,[string]$dest){
        $lines=$fromText -split "`r?`n"
        $out=New-Object System.Collections.Generic.List[string]
        $done=$false
        foreach($l in $lines){
            $out.Add($l)
            if(-not $done -and $l -match '^\$gates = @\('){ foreach($r in $injected){ $out.Add($r) }; $done=$true }
        }
    ...
    try {
        New-GatesCopy (Get-Content -LiteralPath (Join-Path $PSScriptRoot 'run_gates.ps1') -Raw) $fixedCopy
        New-GatesCopy ((& git -C $repo show ("{0}:l2src/run_gates.ps1" -f $base)) -join "`n") $baseCopy
