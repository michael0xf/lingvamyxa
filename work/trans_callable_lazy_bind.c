...
static int callSource(IntSource source);
static int callAdd(AddX add);
int main(void);

static int five(void);
static int lm_trans_bind_five_as_IntSource_0_call(void *lm_env) {
    (void)lm_env;
    return five();
}
static IntSource lm_trans_bind_five_as_IntSource_0(void) {
    IntSource lm_result;
    lm_result = (IntSource)lm_own_new_zero(sizeof(*lm_result));
    if (lm_result == 0) {
        return 0;
    }
    lm_result->call = lm_trans_bind_five_as_IntSource_0_call;
    lm_result->env = 0;
    lm_result->destroy = 0;
    return lm_result;
}
...
int main(void) {
    return callSource(lm_trans_bind_five_as_IntSource_0()) != 5 || callAdd(lm_trans_bind_addBase_as_AddX_1(4)) != 7 || callAdd(lm_trans_bind_addBase_as_AddX_1(4)) != 7;
}
