# Runtime-module parity runner: lmx_msg_roots_stale (review-chat lane, 2026-09-14).
#
...
# real symbol: the caller is the executor after an ownership transfer
# (lmx_message_exec.c), never a unit's open path. The handwritten object is
# kept OUT of the parity link.
#
# The selftest counts releases by compiling the MODULE with
# -Dfree=lmx_stale_test_free (a fault substitution scoped to the helper
# object, as l2src/run_lmx_msg_roots_stale.ps1 does); the generated unit is
# compiled the same way, so its releases are the counted ones. A C driver
# opens the unit with a refused call first and zeroes the counters.
#
...
$out = Join-Path $baseline "build/port_msg_roots_stale/$stamp"
...
$fault = '-Dfree=lmx_stale_test_free'
...
foreach ($source in $sources) {
    ...
    $objByName[$stem] = $obj
    if ($stem -ne 'lmx_msg_roots_stale') { $objs += $obj }
}
...
# The reference's module object with the fault substitution, as the
# existing runner builds it.
$faultObj = Join-Path $out 'native_fault.o'
Step 'native_fault_compile' (Invoke-Native ("gcc $cflags $fault -I " + (Q $hdrs) + ' -c ' + (Q $moduleC['lmx_msg_roots_stale']) + ' -o ' + (Q $faultObj)) (Join-Path $out 'native_fault.gcc.log')) (Join-Path $out 'native_fault.gcc.log')

$selfSrc = 'l2src/tests/lmx_msg_roots_stale_selftest.lm1'
...
if ($refOut -notmatch 'stale checks=(\d+) failures=0 frees=(\d+) mask=(\d+)') { throw "reference selftest did not report zero failures: $refOut" }
$refChecks = [int]$Matches[1]
if ([int]$Matches[2] -lt 1) { throw 'the free substitution is not in force: the reference counted no releases' }
$ev.reference = [ordered]@{ exit = $refExit; stdout = $refOut.Trim(); checks = $refChecks; frees = [int]$Matches[2] }
...
$lm2 = 'l2src/lmx_msg_roots_stale.lm2'
...
if ($genText -notmatch 'sub: lmx_msg_roots_drop_stale \(@: LmxMsg m\)') { throw 'the public signature is missing or changed' }
# One classification per root through the Message's own ranges, one release
# per dropped node, nothing allocated.
...
if (([regex]::Matches($methodText, 'lmx_owned_ranges_find\(')).Count -ne 1) { throw 'drop_stale does not classify through exactly one find' }
if (([regex]::Matches($methodText, 'c\.free\(')).Count -ne 1) { throw 'drop_stale does not release exactly at one site' }
if ($methodText -match 'c\.(malloc|calloc|realloc)\(') { throw 'drop_stale allocates; the handwritten module does not' }
...
if ($nmText -notmatch '(?m)^\S+\s+T\s+_?lmx_msg_roots_drop_stale\s*$') { throw 'the generated object does not define lmx_msg_roots_drop_stale' }
...
extern int frees;
extern int released_mask;
int stale_selftest_main(void);
int main(void) {
    /* First call: the library unit opens; a null Message is a no-op. */
    lmx_msg_roots_drop_stale(0);
    fprintf(stderr, "warm-up frees=%d\n", frees);
    frees = 0;
    released_mask = 0;
    fprintf(stderr, "warm-up ok\n");
    return stale_selftest_main();
}
'@.Replace("`r`n", "`n") | Set-Content -LiteralPath $driverC -Encoding ascii -NoNewline

$selfDrivenObj = Join-Path $out 'selftest_driven.o'
Step 'selftest_driven_compile' (Invoke-Native ("gcc $cflags -Dmain=stale_selftest_main -I " + (Q $hdrs) + ' -c ' + (Q $selfC) + ' -o ' + (Q $selfDrivenObj)) (Join-Path $out 'selftest_driven.gcc.log')) (Join-Path $out 'selftest_driven.gcc.log')
...
Write-Output "lmx_msg_roots_stale parity PASS: $refChecks checks, 0 failures, $($ev.reference.frees) counted releases, reference and generated agree on both runs, real symbol from the generated unit; evidence $evPath"
