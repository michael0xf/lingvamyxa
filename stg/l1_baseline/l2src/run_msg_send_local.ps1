...
param(
    [string]$CoreCommit = '896ad97f17a3131b952fbc55a7baf963a32b1f05',
...
try {
    ...
    $modules = @()
    foreach ($name in @('lmx_msg_blocks', 'lmx_owned_ranges', 'lmx_msg_storage', 'lmx_msg_path_storage', 'lmx_msg_slots', 'lmx_msg_mail_chain')) {
    ...
    foreach ($level in @($Optimization | ForEach-Object { $_.ToUpperInvariant() } | Select-Object -Unique)) {
        ...
        if ($result -notmatch '(?m)^send local checks=136 failures=0 owned_frees=1\s*$') { throw "Unexpected test result: $result" }
