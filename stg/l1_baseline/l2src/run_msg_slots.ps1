...
$run = Join-Path $repo "build/codex/msg_slots/$runId"
...
$ownNames = @('lmx_msg_slots.h.lm1', 'lmx_msg_slots.lm1', 'tests/lmx_msg_slots_selftest.lm1')
...
$evidence = [ordered]@{ compiler = $compiler; compilerSHA256 = $pin; requestedCore = $CoreCommit; sources = $hashes; stages = @(); result = 'RUNNING' }
function Invoke-SlotsStage([string]$Name, [string]$Tool, [string[]]$NativeArgs) {
...
try {
    ...
    Invoke-SlotsStage 'resolve_core' $git @('rev-parse', '--verify', '--end-of-options', "$CoreCommit^{commit}")
    ...
    foreach ($name in @('lmx_msg_blocks', 'lmx_owned_ranges', 'lmx_msg_storage', 'lmx_msg_path_storage', 'lmx_msg_slots')) {
        Invoke-SlotsStage "header_$name" $compiler @("l2src/$name.h.lm1", (Join-Path $headers "l2src/$name.lm1.h"))
    }
    $module = Join-Path $run 'lmx_msg_slots.c'
    $test = Join-Path $run 'lmx_msg_slots_selftest.c'
    Invoke-SlotsStage 'module' $compiler @('l2src/lmx_msg_slots.lm1', $module)
    Invoke-SlotsStage 'test' $compiler @('l2src/tests/lmx_msg_slots_selftest.lm1', $test)
    $gcc = (Get-Command gcc -ErrorAction Stop).Source
    $nm = (Get-Command nm -ErrorAction Stop).Source
    $flags = @('-std=c99', '-Wall', '-Wextra', '-Wpedantic', '-Werror', '-I', $headers, '-I', $stageWorkingDir)
    Invoke-SlotsStage 'gcc_version' $gcc @('--version')
    foreach ($level in @('O0', 'O2')) {
        $obj = Join-Path $run "slots_$level.o"
        $exe = Join-Path $run "slots_$level.exe"
        Invoke-SlotsStage "object_$level" $gcc ($flags + @("-$level", '-c', $module, '-o', $obj))
        Invoke-SlotsStage "imports_$level" $nm @('-u', $obj)
        ...
        Invoke-SlotsStage "symbols_$level" $nm @('--defined-only', $obj)
        ...
        Invoke-SlotsStage "compile_$level" $gcc ($flags + @("-$level", $obj, $test, '-o', $exe))
        Invoke-SlotsStage "run_$level" $exe @()
        $result = Get-Content -LiteralPath (Join-Path $run "run_$level.stdout.txt") -Raw
        if ($result -notmatch '(?m)^slots checks=278 failures=0\s*$') { throw "Unexpected result: $result" }
