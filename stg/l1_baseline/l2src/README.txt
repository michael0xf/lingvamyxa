...
Hosted C entry for simple programs is only `fn: main () int` or
`fn: main (int: NAME; @@: char NAME2) int` (any source names; emitted
...
directives. Simple main may declare `int`, `char`, `size_t`, and
`@: T` locals (optional `0` init on pointer/`int`) and call
