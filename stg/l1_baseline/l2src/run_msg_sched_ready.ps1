...
$run = Join-Path $repo "build/codex/msg_sched_ready/$runId"
...
$ownNames = @('lmx_msg_sched_ready.h.lm1', 'lmx_msg_sched_ready.lm1', 'tests/lmx_msg_sched_ready_selftest.lm1')
...
function Invoke-ReadyStage([string]$Name, [string]$Tool, [string[]]$NativeArgs) {
...
try {
    ...
    Invoke-ReadyStage 'resolve_core' $git @('rev-parse', '--verify', '--end-of-options', "$CoreCommit^{commit}")
    ...
    Invoke-ReadyStage 'archive_core' $git (@('archive', '--format=zip', "--output=$archive", $revision, '--') + $paths)
    ...
    foreach ($name in @('lmx_msg_blocks', 'lmx_owned_ranges', 'lmx_msg_storage', 'lmx_msg_path_storage', 'lmx_msg_sched_ready')) {
        Invoke-ReadyStage "header_$name" $compiler @("l2src/$name.h.lm1", (Join-Path $headers "l2src/$name.lm1.h"))
    }
    $module = Join-Path $run 'lmx_msg_sched_ready.c'
    $test = Join-Path $run 'lmx_msg_sched_ready_selftest.c'
    Invoke-ReadyStage 'module' $compiler @('l2src/lmx_msg_sched_ready.lm1', $module)
    Invoke-ReadyStage 'test' $compiler @('l2src/tests/lmx_msg_sched_ready_selftest.lm1', $test)
    $gcc = (Get-Command gcc -ErrorAction Stop).Source
    $nm = (Get-Command nm -ErrorAction Stop).Source
    $flags = @('-std=c99', '-Wall', '-Wextra', '-Wpedantic', '-Werror', '-I', $headers, '-I', $stageWorkingDir)
    Invoke-ReadyStage 'gcc_version' $gcc @('--version')
    foreach ($level in @($Optimization | ForEach-Object { $_.ToUpperInvariant() } | Select-Object -Unique)) {
        $obj = Join-Path $run "sched_ready_$level.o"
        $exe = Join-Path $run "sched_ready_$level.exe"
        Invoke-ReadyStage "object_$level" $gcc ($flags + @("-$level", '-c', $module, '-o', $obj))
        Invoke-ReadyStage "imports_$level" $nm @('-u', $obj)
        ...
        Invoke-ReadyStage "symbols_$level" $nm @('--defined-only', $obj)
        ...
        Invoke-ReadyStage "compile_$level" $gcc ($flags + @("-$level", $obj, $test, '-o', $exe))
        Invoke-ReadyStage "run_$level" $exe @()
        $result = Get-Content -LiteralPath (Join-Path $run "run_$level.stdout.txt") -Raw
        if ($result -notmatch '(?m)^sched_ready checks=20 failures=0\s*$') { throw "Unexpected result: $result" }
