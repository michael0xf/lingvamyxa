...
$run = Join-Path $repo "build/codex/chars_owned/$runId"
$headers = Join-Path $run 'headers'
New-Item -ItemType Directory -Path (Join-Path $headers 'l2src') -Force | Out-Null
$names = @('lmx_msg_blocks','lmx_owned_ranges','lmx_msg_storage','lmx_chars_owned')
$sources = @($PSCommandPath, (Join-Path $PSScriptRoot 'lmx.h'), (Join-Path $PSScriptRoot 'tests/lmx_chars_owned_selftest.lm1'))
...
function Invoke-CharsStage([string]$Name, [string]$Tool, [string[]]$NativeArgs) {
    $options = @{}
...
try {
    $objects = @()
    foreach ($name in $names) {
        Invoke-CharsStage "header_$name" $compiler @("l2src/$name.h.lm1", (Join-Path $headers "l2src/$name.lm1.h"))
        $module = Join-Path $run "$name.c"
        $obj = Join-Path $run "$name.o"
        Invoke-CharsStage "translate_$name" $compiler @("l2src/$name.lm1",$module)
        Invoke-CharsStage "compile_$name" $gcc ($flags + @('-c',$module,'-o',$obj))
        $objects += $obj
    }
    $charObject = Join-Path $run 'lmx_chars_owned.o'
    Invoke-CharsStage 'imports' $nm @('-u',$charObject)
    foreach ($line in (Get-Content -LiteralPath (Join-Path $run 'imports.stdout.txt'))) {
        if ($line.Trim() -and $line -notmatch '\bU\s+(malloc|free|lmx_msg_storage_move_all)\s*$') { throw "Unexpected character dependency: $line" }
    }
    Invoke-CharsStage 'symbols' $nm @('--defined-only',$charObject)
    foreach ($line in (Get-Content -LiteralPath (Join-Path $run 'symbols.stdout.txt'))) {
        if ($line -match '^\s*[0-9a-fA-F]+\s+[bBdDgGsSC]\s+' -and $line -notmatch '\s+\.(bss|data|sbss|sdata)\s*$') { throw "Global character storage: $line" }
    }
    $test = Join-Path $run 'lmx_chars_owned_selftest.c'
    $exe = Join-Path $run 'lmx_chars_owned_selftest.exe'
    Invoke-CharsStage 'translate_test' $compiler @('l2src/tests/lmx_chars_owned_selftest.lm1',$test)
    Invoke-CharsStage 'link_test' $gcc ($flags + $objects + @($test,'-Wl,--wrap=malloc','-Wl,--wrap=free','-o',$exe))
    Invoke-CharsStage 'run_test' $exe @()
    $result = Get-Content -LiteralPath (Join-Path $run 'run_test.stdout.txt') -Raw
    if ($result -notmatch '^chars_owned checks=\d+ failures=0 allocations=(\d+) releases=(\d+)\s*$' -or $Matches[1] -ne $Matches[2]) { throw "Unexpected test result: $result" }
