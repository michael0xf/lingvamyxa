# Runtime-module parity runner: lmx_msg_sched_ready (review-chat lane, 2026-09-14).
#
...
# names by real symbol: the only runtime caller is the executor's scheduling
# (lmx_message_exec.c), and a unit's open path creates its Message without
# scheduling anything. The handwritten object is kept OUT of the parity link.
#
# The selftest keeps no allocation counters. A C driver opens the unit with
# real calls first: a zeroed LmxMsg enqueued, found at the head and marked
# queued, dequeued back, the list empty again.
#
...
$out = Join-Path $baseline "build/port_msg_sched_ready/$stamp"
...
# ---------------------------------------------------------------------------
# 1. Support objects: the full runtime set; the handwritten module built too,
#    used by the reference only.
# ---------------------------------------------------------------------------
...
$referenceNames = @('lmx_msg_blocks', 'lmx_owned_ranges', 'lmx_msg_storage', 'lmx_msg_path_storage', 'lmx_msg_sched_ready')
...
foreach ($source in $sources) {
    ...
    $objByName[$stem] = $obj
    if ($stem -eq 'lmx_msg_sched_ready') { $nativeObj = $obj } else { $objs += $obj }
}
if (-not $nativeObj) { throw 'the native sched_ready object was not built' }
$objList = ($objs | ForEach-Object { Q $_ }) -join ' '
$referenceObjs = ($referenceNames | ForEach-Object { Q $objByName[$_] }) -join ' '

# ---------------------------------------------------------------------------
# 2. Reference run, linked as l2src/run_msg_sched_ready.ps1 links it.
# ---------------------------------------------------------------------------
$selfSrc = 'l2src/tests/lmx_msg_sched_ready_selftest.lm1'
...
if ($refOut -notmatch 'sched_ready checks=(\d+) failures=0') { throw "reference selftest did not report zero failures: $refOut" }
...
$lm2 = 'l2src/lmx_msg_sched_ready.lm2'
...
# Pure list operations: no allocator, no traversal except unlink's scan.
if ($genText -match 'c\.(malloc|calloc|realloc|free)\(') { throw 'the unit allocates or frees; the handwritten module does not' }
if (([regex]::Matches($genText, '(?m)^\s*while:')).Count -ne 1) { throw 'the unit does not have exactly one loop (unlink)' }
...
foreach ($sym in @('lmx_msg_sched_ready_enqueue', 'lmx_msg_sched_ready_dequeue', 'lmx_msg_sched_ready_unlink')) {
...
int main(void) {
    ...
    lmx_msg_sched_ready_enqueue(&head, &tail, &m);
    if (head != &m || tail != &m || m.sched_queued != 1) { fprintf(stderr, "warm-up: enqueue\n"); return 90; }
    if (lmx_msg_sched_ready_dequeue(&head, &tail) != &m || head != 0 || tail != 0 || m.sched_queued != 0) { fprintf(stderr, "warm-up: dequeue\n"); return 91; }
    if (lmx_msg_sched_ready_dequeue(&head, &tail) != 0) { fprintf(stderr, "warm-up: empty dequeue\n"); return 92; }
    fprintf(stderr, "warm-up ok\n");
    return ready_selftest_main();
}
'@.Replace("`r`n", "`n") | Set-Content -LiteralPath $driverC -Encoding ascii -NoNewline

$selfDrivenObj = Join-Path $out 'selftest_driven.o'
Step 'selftest_driven_compile' (Invoke-Native ("gcc $cflags -Dmain=ready_selftest_main -I " + (Q $hdrs) + ' -c ' + (Q $selfC) + ' -o ' + (Q $selfDrivenObj)) (Join-Path $out 'selftest_driven.gcc.log')) (Join-Path $out 'selftest_driven.gcc.log')
...
Write-Output "lmx_msg_sched_ready parity PASS: $refChecks checks, 0 failures, reference and generated agree on both runs, real symbols from the generated unit; evidence $evPath"
