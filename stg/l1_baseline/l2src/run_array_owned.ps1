...
try {
    ...
    $charObject = Join-Path $run 'lmx_array_owned.o'
    ...
    $test = Join-Path $run 'lmx_array_owned_selftest.c'
    $exe = Join-Path $run 'lmx_array_owned_selftest.exe'
    Invoke-ArrayStage 'translate_test' $compiler @('l2src/tests/lmx_array_owned_selftest.lm1',$test)
    ...
    if ($result -notmatch '^array_owned checks=\d+ failures=0 allocations=(\d+) releases=(\d+)\s*$' -or $Matches[1] -ne $Matches[2]) { throw "Unexpected test result: $result" }
