...
# Staleness: run_seed and run_gen write build\l1trans\<gen>\l1trans.sources.txt when they build the exe,
# one "path SHA256" line per input (l1src\l1trans.lm1 and the C the exe was compiled from). Every
# hash is taken from the files now and compared with the record; nothing is pinned here and no
# timestamp is read. A missing record, a missing file or a changed hash is red and names it.
$record = Join-Path $bin "l1trans.sources.txt"
if (-not (Test-Path -LiteralPath $record)) { throw "stale check: no build record $record for $l1trans (build it with run_seed or run_gen)" }
$recorded = @(Get-Content -LiteralPath $record | Where-Object { $_.Trim().Length -gt 0 })
if ($recorded.Count -lt 2) { throw "stale check: $record names $($recorded.Count) inputs, expected the source and the C" }
$sawSource = $false
foreach ($entry in $recorded) {
    if ($entry -notmatch '^(\S+)\s+([0-9A-F]{64})$') { throw "stale check: bad line in ${record}: $entry" }
    $inputPath = $Matches[1]
    $want = $Matches[2]
    if ($inputPath -eq "l1src\l1trans.lm1") { $sawSource = $true }
    if (-not (Test-Path -LiteralPath $inputPath)) { throw "stale check: $inputPath named in $record is missing" }
    $now = (Get-FileHash -LiteralPath $inputPath -Algorithm SHA256).Hash
    if ($now -ne $want) { throw "stale l1trans.exe: $inputPath changed since $gen was built (recorded $want, now $now)" }
}
if (-not $sawSource) { throw "stale check: $record does not name l1src\l1trans.lm1" }
Set-Content -LiteralPath $script:smokeLog -Value "$(Get-Date -Format o) smoke start record=$record inputs=$($recorded.Count) hash_exe=$((Get-FileHash $l1trans).Hash)"
...
Invoke-Translate "tests\l1\fn_close_ret_nested_if.lm1" "$obj\fn_close_ret_nested_if.c"
Assert-CHas "$obj\fn_close_ret_nested_if.c" "return a;"
...
# gen0 is gcc of the committed lm1\build\l1trans.lm1.c (run_seed), no longer the lm2 seed,
# so these checks run on every generation.
...
Invoke-Translate "tests\l1\sizeof_str_colon.lm1" "$obj\sizeof_str_colon.c"
Assert-CHas "$obj\sizeof_str_colon.c" 'sizeof(("x:y"))'
Assert-CHas "$obj\sizeof_str_colon.c" 'sizeof(("cast:"))'
Invoke-CcRun "$obj\sizeof_str_colon.c" "$bin\sizeof_str_colon.exe" 0 $null
Invoke-Translate "tests\l1\sizeof_char_colon.lm1" "$obj\sizeof_char_colon.c"
Assert-CHas "$obj\sizeof_char_colon.c" "sizeof((':'"
Invoke-CcRun "$obj\sizeof_char_colon.c" "$bin\sizeof_char_colon.exe" 0 $null
Invoke-Translate "tests\l1\sizeof_ternary.lm1" "$obj\sizeof_ternary.c"
Assert-CHas "$obj\sizeof_ternary.c" "sizeof((1 ? 2 : 3))"
Assert-CHas "$obj\sizeof_ternary.c" "sizeof((1 ? a : b))"
Invoke-CcRun "$obj\sizeof_ternary.c" "$bin\sizeof_ternary.exe" 0 $null
