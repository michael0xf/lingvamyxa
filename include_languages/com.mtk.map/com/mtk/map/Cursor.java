...
public class Cursor implements LiveVector.Element, Const, ISrc, IPointer{
    ...
    public boolean next(){
        ...
        if (leaf == null)
            return false;
        // Porting (Lingvamyxa MIX): same-page reverse uses the leaf monitor.
        // Crossing pages locks lower address first (this leaf, then next).
        int index = this.index;
        if (index < leaf.size() - 1 ) {
            leaf.reverse(index, index + 1);
            return true;
        }
        Leaf next = dropEmptyForward(leaf.getNext());
        if (next == null)
            return false;
        final Leaf n = next;
        final int idx = index;
        Branch.lockLowerThenHigher(leaf, n, new Runnable() {
            public void run() {
                if (n.size() == 0)
                    return;
                Object o = n.elementAt(0);
                leaf.setElementAt(o, idx);
                n.setElementAt(Cursor.this, 0);
            }
        });
        return  true;
    }
    ...
    public boolean prev(){
        ...
        if (leaf == null)
            return false;
        // Porting: crossing to prev must lock the lower-address leaf first.
        int index = this.index;
        if (index > 0 ) {
            leaf.reverse(index, index - 1);
            return true;
        }
        Leaf prev = dropEmptyBackward(leaf.getPrev());
        if (prev == null)
            return false;
        final Leaf p = prev;
        Branch.lockLowerThenHigher(p, leaf, new Runnable() {
            public void run() {
                if (p.size() == 0)
                    return;
                int i = p.size() - 1;
                ...
                p.setElementAt(Cursor.this, i);
            }
        });
        return true;
    }

    // Porting: empty-page delete uses Leaf.remove() chain order (prev, this, next).
    static Leaf dropEmptyForward(Leaf next) {
        while (next != null) {
            if (next.size() != 0)
                return next;
            Leaf toRemove = next;
    ...
    static Leaf dropEmptyBackward(Leaf prev) {
        while (prev != null) {
            if (prev.size() != 0)
                return prev;
            Leaf toRemove = prev;
    ...
    */
    ...
    public  void goTo(Cursor to){
        ...
        if (to != null){
            ...
            if (to.isDeleted())
                return;
            moveToLeaf(l, to.index + 1);
        }
            
    }

    // Porting: two-page cursor move locks lower-address leaf first.
    void moveToLeaf(final Leaf dest, final int at) {
        final Leaf from = this.leaf;
        if (from == dest || from == null) {
            remove();
            dest.insertElementAt(this, at);
            return;
        }
        Object lower = from;
        Object higher = dest;
        if (from.getAddress(0) > dest.getAddress(0)) {
            lower = dest;
            higher = from;
        }
        Branch.lockLowerThenHigher(lower, higher, new Runnable() {
            public void run() {
                remove();
                dest.insertElementAt(Cursor.this, at);
            }
        });
    }
