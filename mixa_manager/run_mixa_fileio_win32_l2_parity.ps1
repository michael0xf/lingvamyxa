# Real L1-vs-L2 parity gate for mixa_fileio_win32 (dependency-order
# continuation after mixa_dir_win32). No dedicated ABI probe pair: this
# module declares no new struct of its own beyond what mixa_fileio_
# win32.h.lm1/mixa_fileio.h.lm1 already declare (MixaFio/
# MixaFioFaultVTable), reached via the REAL headers predef'd directly,
# identically on both the real oracle and this L2 port.
#
# Dependency-linking note: mixa_fileio_win32.lm1 predefs ONLY its own
# real header plus mixa_fileio.h.lm1 (both header-only, no bodies) --
# this module sits at the bottom of the dependency stack (windows.h
# direct Win32 calls), so BOTH the oracle side and the L2 side compile
# a single real/generated object with no separate real dependency
# objects needed at all.
#
#   1. ALWAYS builds and runs the ORACLE-side harness (a real write-
#      then-read round trip, a real CREATE_NEW existing-destination
#      refusal, a real missing-source refusal, a real chunked multi-
#      read reassembly, the deterministic fault hook proven on both
#      write and read with real partial data left on disk, and
#      argument negatives).
#   2. Attempts to translate the COMPLETE mixa_fileio_win32.lm2.
...
#
# mixa_fileio_win32.h.lm1/.lm1 and mixa_fileio.h.lm1 are the parity
...
$BaseDir = Join-Path $RepoRoot "build\mixa\claude\mixa_fileio_win32_l2_parity"
...
Invoke-HeaderTrans "mixa_manager\mixa_fileio_win32_l2.h.lm1" "mixa_fileio_win32_l2.lm1.h"
...
$hExit = Invoke-Cmd $L1Trans "mixa_manager\tests\mixa_fileio_win32_parity_harness.lm1 `"$harnessC`"" $hLog1 $hLog2
Pop-Location
...
# ---- Step 3: ALWAYS build + run the ORACLE-side harness. mixa_fileio_
# win32.lm1 predefs ONLY headers (see top comment) -- no separate real
# dependency objects needed. ----
$oracleC = Join-Path $RunDir "mixa_fileio_win32_oracle.c"
$ocLog1 = Join-Path $RunDir "oracle_trans_stdout.log"
$ocLog2 = Join-Path $RunDir "oracle_trans_stderr.log"
Push-Location $RepoRoot
$ocExit = Invoke-Cmd $L1Trans "mixa_manager\mixa_fileio_win32.lm1 `"$oracleC`"" $ocLog1 $ocLog2
Pop-Location
if ($ocExit -ne 0) { Get-Content $ocLog2; throw "oracle mixa_fileio_win32.lm1 translation failed" }

$oracleO = Join-Path $RunDir "mixa_fileio_win32_oracle.o"
...
if ($occExit -ne 0) { Get-Content $occLog2; throw "oracle mixa_fileio_win32.lm1 compile failed" }
...
# ---- Step 4: attempt the COMPLETE mixa_fileio_win32.lm2 translation. ----
Push-Location $RepoRoot
$fwSrc = "mixa_manager\mixa_fileio_win32.lm2"
$fwOut = Join-Path $RunDir "mixa_fileio_win32_l2.lm1"
$fwStdout = Join-Path $RunDir "fw_stdout.log"
$fwStderr = Join-Path $RunDir "fw_stderr.log"
$FwExit = Invoke-Cmd "`"$l2exe`"" "`"$fwSrc`" `"$fwOut`"" $fwStdout $fwStderr
Pop-Location

$FwStdoutText = if (Test-Path -LiteralPath $fwStdout) { Get-Content -LiteralPath $fwStdout -Raw } else { "" }
$FwStderrText = if (Test-Path -LiteralPath $fwStderr) { Get-Content -LiteralPath $fwStderr -Raw } else { "" }
$FwSourceHash = (Get-FileHash -LiteralPath (Join-Path $RepoRoot $fwSrc) -Algorithm SHA256).Hash
...
$KnownBarrier = ($FwStderrText -match "unknown foreign type") -or ($FwStderrText -match "incompatible entry signature")

if ($FwExit -ne 0 -and $KnownBarrier) {
    ...
    Push-Location $RepoRoot
    $l2FwC = Join-Path $RunDir "mixa_fileio_win32_l2.c"
    $l2ccLog1 = Join-Path $RunDir "l2fw_trans_stdout.log"
    $l2ccLog2 = Join-Path $RunDir "l2fw_trans_stderr.log"
    $l2ccExit = Invoke-Cmd $L1Trans "`"$fwOut`" `"$l2FwC`"" $l2ccLog1 $l2ccLog2
    Pop-Location
    if ($l2ccExit -ne 0) {
        ...
        $l2FwO = Join-Path $RunDir "mixa_fileio_win32_l2.o"
        $l2occLog1 = Join-Path $RunDir "l2fw_compile_stdout.log"
        $l2occLog2 = Join-Path $RunDir "l2fw_compile_stderr.log"
        $l2occExit = Invoke-Cmd "gcc" "$GccStd -I `"$RepoRoot`" -I `"$RunDir\headers`" -c `"$l2FwC`" -o `"$l2FwO`"" $l2occLog1 $l2occLog2
        if ($l2occExit -ne 0) {
            ...
            $l2olLog1 = Join-Path $RunDir "l2fw_link_stdout.log"
            $l2olLog2 = Join-Path $RunDir "l2fw_link_stderr.log"
            $l2olExit = Invoke-Cmd "gcc" "$GccStd -I `"$RepoRoot`" -I `"$RunDir\headers`" `"$harnessO`" `"$l2FwO`" -o `"$l2Exe`"" $l2olLog1 $l2olLog2
            if ($l2olExit -ne 0) {
...
FileioWin32-L2-Header: mixa_manager\mixa_fileio_win32_l2.h.lm1
FileioWin32-L2-Source: $fwSrc
FileioWin32-L2-Source-Sha256: $FwSourceHash
Harness-Source-Sha256: $((Get-FileHash -LiteralPath (Join-Path $RepoRoot "mixa_manager\tests\mixa_fileio_win32_parity_harness.lm1") -Algorithm SHA256).Hash)
Runner-Sha256: $RunnerHash
Oracle-Run-Exit-Code: $oracleRunExit
Oracle-Trace:
$OracleTrace
FileioWin32-Translate-Exit-Code: $FwExit
FileioWin32-Translate-Stdout:
$FwStdoutText
FileioWin32-Translate-Stderr:
$FwStderrText
L2-Trace:
$L2TraceText
...
switch ($Verdict) {
    "EXPECTED_CORE_BARRIER" {
        "EXPECTED_CORE_BARRIER: full mixa_fileio_win32.lm2 translation stopped at an already-known barrier. This is NOT a pass -- the integrated frontend is not yet on main. The oracle-side harness DID run (see Oracle-Trace above)."
    }
    "UNEXPECTED_FAILURE" {
    ...
    "PASS" {
        "PASS: full mixa_fileio_win32.lm2 translated, built, and ran; its trace is byte-identical to the L1 oracle's own trace."
    }
}
