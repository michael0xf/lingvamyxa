# File content read/write seam selftest runner (ticket 20260912-092435):
# real on-disk fixtures under this run's own directory, real Win32
# refusals (missing source, existing destination under CREATE_NEW), a
# chunked read/write content-equality check, and the deterministic fault
# hook that the first copy operation needs for partial-write/read
# cleanup checks. No user files.
...
$BaseDir = Join-Path $RepoRoot "build\mixa\claude\fileio"
...
try {
    ...
    $TransOut = Join-Path $RunDir "mixa_fileio_selftest.c"
    $ExeOut = Join-Path $RunDir "mixa_fileio_selftest.exe"
