# Real L1-vs-L2 parity gate for mixa_share (share chain ticket
# 20260913-192700, module 1 of 6). No dedicated ABI probe pair: this
# module declares no new struct of its own beyond what mixa_share.h.
# lm1 already declares (MixaShareRequest/MixaShareVTable), reached via
# the REAL header predef'd directly, identically on both the real
# oracle and this L2 port.
#
# Dependency-linking note: mixa_share.lm1 predefs ONLY mixa_share_win32.
# h.lm1 and mixa_share.h.lm1 (both header-only, no bodies -- the win32
# adapter's own body lives in the separate, not-yet-ported mixa_share_
# win32.lm1) -- so BOTH the oracle side and the L2 side compile a
# single real/generated object with no separate real dependency objects
# needed at all. This harness drives its own deterministic fake
# backend (mirroring the existing accepted mixa_share_selftest.lm1),
# never the real win32 adapter.
#
#   1. ALWAYS builds and runs the ORACLE-side harness (ClearShell-
#      parity title derivation, single/multi-file shares driven to
#      completion, cancellation, a restricted backend, an incomplete
#      vtable, a missing file and a soft-skipped directory, a backend
#      refusing begin() for a missing context, destroy-before-
#      completion leaving no leaked registration, a rejected double
#      begin(), and every terminal event rejecting a further begin()).
#   2. Attempts to translate the COMPLETE mixa_share.lm2.
...
#
# mixa_share.h.lm1/.lm1 and mixa_share_win32.h.lm1 are the parity
...
$BaseDir = Join-Path $RepoRoot "build\mixa\claude\mixa_share_l2_parity"
...
Invoke-HeaderTrans "mixa_manager\mixa_share_l2.h.lm1" "mixa_share_l2.lm1.h"
...
$hExit = Invoke-Cmd $L1Trans "mixa_manager\tests\mixa_share_parity_harness.lm1 `"$harnessC`"" $hLog1 $hLog2
Pop-Location
...
# ---- Step 3: ALWAYS build + run the ORACLE-side harness. mixa_share.
# lm1 predefs ONLY headers (see top comment) -- no separate real
# dependency objects needed. ----
$oracleC = Join-Path $RunDir "mixa_share_oracle.c"
$ocLog1 = Join-Path $RunDir "oracle_trans_stdout.log"
$ocLog2 = Join-Path $RunDir "oracle_trans_stderr.log"
Push-Location $RepoRoot
$ocExit = Invoke-Cmd $L1Trans "mixa_manager\mixa_share.lm1 `"$oracleC`"" $ocLog1 $ocLog2
Pop-Location
if ($ocExit -ne 0) { Get-Content $ocLog2; throw "oracle mixa_share.lm1 translation failed" }

$oracleO = Join-Path $RunDir "mixa_share_oracle.o"
...
if ($occExit -ne 0) { Get-Content $occLog2; throw "oracle mixa_share.lm1 compile failed" }
...
# ---- Step 4: attempt the COMPLETE mixa_share.lm2 translation. ----
Push-Location $RepoRoot
$shSrc = "mixa_manager\mixa_share.lm2"
$shOut = Join-Path $RunDir "mixa_share_l2.lm1"
$shStdout = Join-Path $RunDir "sh_stdout.log"
$shStderr = Join-Path $RunDir "sh_stderr.log"
$ShExit = Invoke-Cmd "`"$l2exe`"" "`"$shSrc`" `"$shOut`"" $shStdout $shStderr
Pop-Location

$ShStdoutText = if (Test-Path -LiteralPath $shStdout) { Get-Content -LiteralPath $shStdout -Raw } else { "" }
$ShStderrText = if (Test-Path -LiteralPath $shStderr) { Get-Content -LiteralPath $shStderr -Raw } else { "" }
$ShSourceHash = (Get-FileHash -LiteralPath (Join-Path $RepoRoot $shSrc) -Algorithm SHA256).Hash
...
$KnownBarrier = ($ShStderrText -match "unknown foreign type") -or ($ShStderrText -match "incompatible entry signature")

if ($ShExit -ne 0 -and $KnownBarrier) {
    ...
    Push-Location $RepoRoot
    $l2ShC = Join-Path $RunDir "mixa_share_l2.c"
    $l2ccLog1 = Join-Path $RunDir "l2sh_trans_stdout.log"
    $l2ccLog2 = Join-Path $RunDir "l2sh_trans_stderr.log"
    $l2ccExit = Invoke-Cmd $L1Trans "`"$shOut`" `"$l2ShC`"" $l2ccLog1 $l2ccLog2
    Pop-Location
    if ($l2ccExit -ne 0) {
        ...
        $l2ShO = Join-Path $RunDir "mixa_share_l2.o"
        $l2occLog1 = Join-Path $RunDir "l2sh_compile_stdout.log"
        $l2occLog2 = Join-Path $RunDir "l2sh_compile_stderr.log"
        $l2occExit = Invoke-Cmd "gcc" "$GccStd -I `"$RepoRoot`" -I `"$RunDir\headers`" -c `"$l2ShC`" -o `"$l2ShO`"" $l2occLog1 $l2occLog2
        if ($l2occExit -ne 0) {
            ...
            $l2olLog1 = Join-Path $RunDir "l2sh_link_stdout.log"
            $l2olLog2 = Join-Path $RunDir "l2sh_link_stderr.log"
            $l2olExit = Invoke-Cmd "gcc" "$GccStd -I `"$RepoRoot`" -I `"$RunDir\headers`" `"$harnessO`" `"$l2ShO`" -o `"$l2Exe`"" $l2olLog1 $l2olLog2
            if ($l2olExit -ne 0) {
...
Share-L2-Header: mixa_manager\mixa_share_l2.h.lm1
Share-L2-Source: $shSrc
Share-L2-Source-Sha256: $ShSourceHash
Harness-Source-Sha256: $((Get-FileHash -LiteralPath (Join-Path $RepoRoot "mixa_manager\tests\mixa_share_parity_harness.lm1") -Algorithm SHA256).Hash)
Runner-Sha256: $RunnerHash
Oracle-Run-Exit-Code: $oracleRunExit
Oracle-Trace:
$OracleTrace
Share-Translate-Exit-Code: $ShExit
Share-Translate-Stdout:
$ShStdoutText
Share-Translate-Stderr:
$ShStderrText
L2-Trace:
$L2TraceText
...
switch ($Verdict) {
    "EXPECTED_CORE_BARRIER" {
        "EXPECTED_CORE_BARRIER: full mixa_share.lm2 translation stopped at an already-known barrier. This is NOT a pass -- the integrated frontend is not yet on main. The oracle-side harness DID run (see Oracle-Trace above)."
    }
    "UNEXPECTED_FAILURE" {
    ...
    "PASS" {
        "PASS: full mixa_share.lm2 translated, built, and ran; its trace is byte-identical to the L1 oracle's own trace."
    }
}
