# Real L1-vs-L2 parity gate for mixa_audio_launch (ticket 20260913-
# 232000/231500 Part 2, module 4 of 8). The real mixa_audio_launch.lm1
# predefs the full body of mixa_audio_panel.lm1 directly (which in turn
# embeds mixa_app_window/mixa_audio_button/mixa_audio/mixa_audio_win32),
# so neither side of this harness needs a separate real dependency
# object -- each side's own single translated .c already contains
# everything.
#
#   1. ALWAYS builds and runs the ORACLE-side harness (deterministic
#      fake audio device; snapshot built from fake path STRINGS only,
#      never touching the filesystem, matching the real selftest's own
#      reasoning that the scanning half is proven separately).
#   2. Attempts to translate the COMPLETE mixa_audio_launch.lm2.
...
#
# mixa_audio_launch.lm1/.h.lm1 and the real, unmodified chain it predefs
# (mixa_audio_panel, mixa_app_window, mixa_audio_button, mixa_audio,
# mixa_audio_win32) are the parity oracle and are never touched. Nothing
# under stg/l1_baseline is modified, only read. Every input is built
# fresh in a unique run directory -- no stale objects.
param()
...
$BaseDir = Join-Path $RepoRoot "build\mixa\claude\mixa_audio_launch_l2_parity"
...
# ---- Step 0: translate the real headers (matching run_audio_launch_
# selftest.ps1's own list), then the L2 header unit. ----
...
Invoke-HeaderTrans "mixa_manager\mixa_audio_panel.h.lm1" "mixa_audio_panel.lm1.h"
Invoke-HeaderTrans "mixa_manager\mixa_audio_launch.h.lm1" "mixa_audio_launch.lm1.h"
Invoke-HeaderTrans "mixa_manager\mixa_audio_launch_l2.h.lm1" "mixa_audio_launch_l2.lm1.h"
...
$hExit = Invoke-Cmd $L1Trans "mixa_manager\tests\mixa_audio_launch_parity_harness.lm1 `"$harnessC`"" $hLog1 $hLog2
Pop-Location
...
$oracleC = Join-Path $RunDir "mixa_audio_launch_oracle.c"
$ocLog1 = Join-Path $RunDir "oracle_trans_stdout.log"
$ocLog2 = Join-Path $RunDir "oracle_trans_stderr.log"
Push-Location $RepoRoot
$ocExit = Invoke-Cmd $L1Trans "mixa_manager\mixa_audio_launch.lm1 `"$oracleC`"" $ocLog1 $ocLog2
Pop-Location
if ($ocExit -ne 0) { Get-Content $ocLog2; throw "oracle mixa_audio_launch.lm1 translation failed" }

$oracleO = Join-Path $RunDir "mixa_audio_launch_oracle.o"
...
if ($occExit -ne 0) { Get-Content $occLog2; throw "oracle mixa_audio_launch.lm1 compile failed" }
...
# ---- Step 4: attempt the COMPLETE mixa_audio_launch.lm2 translation. ----
Push-Location $RepoRoot
$alSrc = "mixa_manager\mixa_audio_launch.lm2"
...
$AlStdoutText = if (Test-Path -LiteralPath $alStdout) { Get-Content -LiteralPath $alStdout -Raw } else { "" }
$AlStderrText = if (Test-Path -LiteralPath $alStderr) { Get-Content -LiteralPath $alStderr -Raw } else { "" }
$AlSourceHash = (Get-FileHash -LiteralPath (Join-Path $RepoRoot $alSrc) -Algorithm SHA256).Hash
...
$KnownBarrier = ($AlStderrText -match "unknown foreign type") -or ($AlStderrText -match "incompatible entry signature") -or ($AlStderrText -match "unsupported own array declaration")

if ($AlExit -ne 0 -and $KnownBarrier) {
    ...
    Push-Location $RepoRoot
    $l2AlC = Join-Path $RunDir "mixa_audio_launch_l2.c"
    $l2ccLog1 = Join-Path $RunDir "l2al_trans_stdout.log"
    $l2ccLog2 = Join-Path $RunDir "l2al_trans_stderr.log"
    $l2ccExit = Invoke-Cmd $L1Trans "`"$alOut`" `"$l2AlC`"" $l2ccLog1 $l2ccLog2
    Pop-Location
    if ($l2ccExit -ne 0) {
        ...
        $l2AlO = Join-Path $RunDir "mixa_audio_launch_l2.o"
        $l2occLog1 = Join-Path $RunDir "l2al_compile_stdout.log"
        $l2occLog2 = Join-Path $RunDir "l2al_compile_stderr.log"
        $l2occExit = Invoke-Cmd "gcc" "$GccStd -I `"$RepoRoot`" -I `"$RunDir\headers`" -I `"$L2RuntimeHeaderRoot`" -I `"$L2RuntimeHeaderRoot\stg\l1_baseline`" -I `"$L1Root`" -c `"$l2AlC`" -o `"$l2AlO`"" $l2occLog1 $l2occLog2
        if ($l2occExit -ne 0) {
            ...
            $l2olLog1 = Join-Path $RunDir "l2al_link_stdout.log"
            $l2olLog2 = Join-Path $RunDir "l2al_link_stderr.log"
            $l2olExit = Invoke-Cmd "gcc" "$GccStd -I `"$RepoRoot`" -I `"$RunDir\headers`" `"$harnessO`" `"$l2AlO`" $L2RuntimeObjList -o `"$l2Exe`"" $l2olLog1 $l2olLog2
            if ($l2olExit -ne 0) {
...
AudioLaunch-L2-Header: mixa_manager\mixa_audio_launch_l2.h.lm1
AudioLaunch-L2-Source: $alSrc
AudioLaunch-L2-Source-Sha256: $AlSourceHash
Harness-Source-Sha256: $((Get-FileHash -LiteralPath (Join-Path $RepoRoot "mixa_manager\tests\mixa_audio_launch_parity_harness.lm1") -Algorithm SHA256).Hash)
Runner-Sha256: $RunnerHash
Oracle-Run-Exit-Code: $oracleRunExit
Oracle-Trace:
$OracleTrace
AudioLaunch-Translate-Exit-Code: $AlExit
AudioLaunch-Translate-Stdout:
$AlStdoutText
AudioLaunch-Translate-Stderr:
$AlStderrText
L2-Trace:
$L2TraceText
...
switch ($Verdict) {
    "EXPECTED_CORE_BARRIER" {
        "EXPECTED_CORE_BARRIER: full mixa_audio_launch.lm2 translation stopped at an already-known barrier. This is NOT a pass -- the integrated frontend is not yet on main. The oracle-side harness DID run (see Oracle-Trace above)."
    }
    "UNEXPECTED_FAILURE" {
    ...
    "PASS" {
        "PASS: full mixa_audio_launch.lm2 translated, built, and ran; its trace is byte-identical to the L1 oracle's own trace."
    }
}
