# Process marker selftest runner (ticket 20260912-145000): PROCESS_SEAM
# 4.1's in-band marker -- identity generation, the real shell wrapper
# (a temp .bat, see mixa_process_marker.lm1's own comment for why a
# one-liner delayed-expansion trick was tried and measured broken before
# choosing this), and the incremental streaming parser. Real child
# processes for the end-to-end scenarios; pure in-process byte-feeding
# for the parser's own edge cases. No visible windows (CREATE_NO_WINDOW
# is unconditional in mixa_process_win32.lm1's own spawn()). Fixtures
# live under this run's own directory; every mutation stays inside that
# root. No user files.
...
$RunDir = ""
$LogDir = ""
$TestSource = Join-Path $RepoRoot "mixa_manager\tests\mixa_process_marker_selftest.lm1"
$TransOut = ""
$ExeOut = ""
$TransStdout = ""
$TransStderr = ""
...
$BaseDir = Join-Path $RepoRoot "build\mixa\claude\process_marker"
...
try {
    ...
    $HashFiles = @{
        "marker_header" = "mixa_manager\mixa_process_marker.h.lm1"
        "marker_impl" = "mixa_manager\mixa_process_marker.lm1"
