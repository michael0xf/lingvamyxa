# Copy Here key-event selftest runner (ticket 20260912-124720): drives
# mixa_fm_copy_here_key_event with a real MixaEvent round-tripped through
# the headless backend's own push/poll seam (mixa_headless_push_event +
# mixa_backend_poll), not a hand-filled local struct. Links the portable
# headless backend stack (event_fifo + backend_table + backend_headless +
# backend_ctors_headless) alongside the pinned stable translator's own
# fm_copy chain, the same way run_mixa.ps1 links mixa_pump_selftest.
...
$BaseDir = Join-Path $RepoRoot "build\mixa\claude\fm_copy_here_key"
