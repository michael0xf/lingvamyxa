# Real L1-vs-L2 parity gate for mixa_share_button (share chain ticket
# 20260913-192700, module 3 of 6). No dedicated ABI probe pair: this
# module declares no new struct of its own beyond what mixa_share_
# button.h.lm1 already declares (MixaShareSendCtx), reached via the
# REAL header predef'd directly, identically on both the real oracle
# and this L2 port.
#
# Dependency-linking note: mixa_share_button.lm1 predefs mixa_share_
# button.h.lm1 (header) + mixa_share_action.lm1 (body) directly, and
# mixa_share_action.lm1 itself further predefs the FULL bodies of
# mixa_selection.lm1 and mixa_share.lm1 in its own single translation
# unit -- so the ORACLE side (translating mixa_share_button.lm1 alone)
# already embeds everything transitively and needs no separate real
# objects. The L2 header predefs ONLY mixa_share_button.h.lm1 (header-
# only), plus a forward-declared mixa_share_action_send (that function
# has no L1 header of its own anywhere in this codebase) -- so the L2
# side separately links the real, unmodified mixa_share_action.lm1
# (which itself already embeds selection+share, no dedup risk since
# this is the only place those bodies are linked on the L2 side).
#
#   1. ALWAYS builds and runs the ORACLE-side harness (a real hit
#      driving a real selection through the Share seam via this thin
#      adapter, reporting sent_count/skipped_dirs/last_status/out_r
#      back through the ctx struct exactly as mixa_share_action_send
#      itself reports; and a null ctx rejected with ERR_ARG).
#   2. Attempts to translate the COMPLETE mixa_share_button.lm2.
...
#
# mixa_share_button.h.lm1/.lm1 and every real dependency (mixa_share_
# action.lm1, mixa_share.h.lm1/.lm1, mixa_share_win32.h.lm1, mixa_
# selection.h/.lm1) are the parity oracle (or real, unmodified
...
$BaseDir = Join-Path $RepoRoot "build\mixa\claude\mixa_share_button_l2_parity"
...
Invoke-HeaderTrans "mixa_manager\mixa_share_button.h.lm1" "mixa_share_button.lm1.h"
Invoke-HeaderTrans "mixa_manager\mixa_share_button_l2.h.lm1" "mixa_share_button_l2.lm1.h"
...
$hExit = Invoke-Cmd $L1Trans "mixa_manager\tests\mixa_share_button_parity_harness.lm1 `"$harnessC`"" $hLog1 $hLog2
Pop-Location
...
# button.lm1's own translation unit already embeds mixa_share_action/
# mixa_selection/mixa_share (see top comment) -- no separate real
# objects needed. ----
$oracleC = Join-Path $RunDir "mixa_share_button_oracle.c"
$ocLog1 = Join-Path $RunDir "oracle_trans_stdout.log"
$ocLog2 = Join-Path $RunDir "oracle_trans_stderr.log"
Push-Location $RepoRoot
$ocExit = Invoke-Cmd $L1Trans "mixa_manager\mixa_share_button.lm1 `"$oracleC`"" $ocLog1 $ocLog2
Pop-Location
if ($ocExit -ne 0) { Get-Content $ocLog2; throw "oracle mixa_share_button.lm1 translation failed" }

$oracleO = Join-Path $RunDir "mixa_share_button_oracle.o"
...
if ($occExit -ne 0) { Get-Content $occLog2; throw "oracle mixa_share_button.lm1 compile failed" }
...
# ---- Step 4: attempt the COMPLETE mixa_share_button.lm2 translation. ----
Push-Location $RepoRoot
$sbSrc = "mixa_manager\mixa_share_button.lm2"
$sbOut = Join-Path $RunDir "mixa_share_button_l2.lm1"
$sbStdout = Join-Path $RunDir "sb_stdout.log"
$sbStderr = Join-Path $RunDir "sb_stderr.log"
$SbExit = Invoke-Cmd "`"$l2exe`"" "`"$sbSrc`" `"$sbOut`"" $sbStdout $sbStderr
Pop-Location

$SbStdoutText = if (Test-Path -LiteralPath $sbStdout) { Get-Content -LiteralPath $sbStdout -Raw } else { "" }
$SbStderrText = if (Test-Path -LiteralPath $sbStderr) { Get-Content -LiteralPath $sbStderr -Raw } else { "" }
$SbSourceHash = (Get-FileHash -LiteralPath (Join-Path $RepoRoot $sbSrc) -Algorithm SHA256).Hash
...
$KnownBarrier = ($SbStderrText -match "unknown foreign type") -or ($SbStderrText -match "incompatible entry signature") -or ($SbStderrText -match "unsupported own array declaration")

if ($SbExit -ne 0 -and $KnownBarrier) {
    ...
    Push-Location $RepoRoot
    $l2SbC = Join-Path $RunDir "mixa_share_button_l2.c"
    $l2ccLog1 = Join-Path $RunDir "l2sb_trans_stdout.log"
    $l2ccLog2 = Join-Path $RunDir "l2sb_trans_stderr.log"
    $l2ccExit = Invoke-Cmd $L1Trans "`"$sbOut`" `"$l2SbC`"" $l2ccLog1 $l2ccLog2
    Pop-Location
    if ($l2ccExit -ne 0) {
        ...
        $l2SbO = Join-Path $RunDir "mixa_share_button_l2.o"
        $l2occLog1 = Join-Path $RunDir "l2sb_compile_stdout.log"
        $l2occLog2 = Join-Path $RunDir "l2sb_compile_stderr.log"
        $l2occExit = Invoke-Cmd "gcc" "$GccStd -I `"$RepoRoot`" -I `"$RunDir\headers`" -c `"$l2SbC`" -o `"$l2SbO`"" $l2occLog1 $l2occLog2
        if ($l2occExit -ne 0) {
            Get-Content $l2occLog2
            $Verdict = "UNEXPECTED_FAILURE"
            $ExitCode = 1
        } else {
            $shareActionO = Build-RealDep "mixa_share_action" "mixa_manager\mixa_share_action.lm1"
            $l2Exe = Join-Path $RunDir "parity_l2.exe"
            $l2olLog1 = Join-Path $RunDir "l2sb_link_stdout.log"
            $l2olLog2 = Join-Path $RunDir "l2sb_link_stderr.log"
            $l2olExit = Invoke-Cmd "gcc" "$GccStd -I `"$RepoRoot`" -I `"$RunDir\headers`" `"$harnessO`" `"$l2SbO`" `"$shareActionO`" -o `"$l2Exe`"" $l2olLog1 $l2olLog2
            if ($l2olExit -ne 0) {
...
ShareButton-L2-Header: mixa_manager\mixa_share_button_l2.h.lm1
ShareButton-L2-Source: $sbSrc
ShareButton-L2-Source-Sha256: $SbSourceHash
Harness-Source-Sha256: $((Get-FileHash -LiteralPath (Join-Path $RepoRoot "mixa_manager\tests\mixa_share_button_parity_harness.lm1") -Algorithm SHA256).Hash)
Runner-Sha256: $RunnerHash
Oracle-Run-Exit-Code: $oracleRunExit
Oracle-Trace:
$OracleTrace
ShareButton-Translate-Exit-Code: $SbExit
ShareButton-Translate-Stdout:
$SbStdoutText
ShareButton-Translate-Stderr:
$SbStderrText
L2-Trace:
$L2TraceText
...
switch ($Verdict) {
    "EXPECTED_CORE_BARRIER" {
        "EXPECTED_CORE_BARRIER: full mixa_share_button.lm2 translation stopped at an already-known barrier. This is NOT a pass -- the integrated frontend is not yet on main. The oracle-side harness DID run (see Oracle-Trace above)."
    }
    "UNEXPECTED_FAILURE" {
    ...
    "PASS" {
        "PASS: full mixa_share_button.lm2 translated, built, and ran; its trace is byte-identical to the L1 oracle's own trace."
    }
}
