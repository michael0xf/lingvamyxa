...
# Library wrappers execute through the real Message runtime. The shared
# lib_l2_runtime_support.ps1 builds that runtime object set fresh into
# this invocation's directory (same reasoning: never depend on stale
# build/l2 state), including the L2-native units (e.g. lmx_root_record)
# a bare .lm1 list can't cover.
...
if ($SelExit -ne 0 -and $KnownBarrier) {
    ...
    if ($l2scExit -ne 0) {
        ...
        $l2occExit = Invoke-Cmd "gcc" "$GccStd -I `"$RepoRoot`" -I `"$L1Root`" -I `"$HeaderRoot`" -I `"$($L2Rt.HeaderRoot)`" -Dmalloc=test_malloc -Dcalloc=test_calloc -c `"$l2SelC`" -o `"$l2SelO`"" $l2occLog1 $l2occLog2
        if ($l2occExit -ne 0) {
            ...
            $l2olExit = Invoke-Cmd "gcc" "$GccStd -I `"$RepoRoot`" -I `"$L1Root`" -I `"$HeaderRoot`" `"$harnessO`" `"$l2SelO`" `"$faultO`" $($L2Rt.ObjList) -o `"$l2Exe`"" $l2olLog1 $l2olLog2
            if ($l2olExit -ne 0) {
