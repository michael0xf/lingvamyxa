# File-manager navigation/list model selftest runner (ticket
# 20260912-075000): real, test-owned directory trees enumerated through
# the accepted mixa_dir seam, with an independent mixa_dir oracle proving
# observed order and an injected allocator proving out-of-memory refusal
# and exact disposal at every owned allocation step. Fixtures live under
# this run's own directory and are rebuilt from scratch each run, so no
# scenario depends on leftovers from a previous one -- the test also
# creates and deletes files mid-run to exercise refresh, and every path
# it touches is inside that run directory.
...
$RunDir = ""
$LogDir = ""
$TestSource = Join-Path $RepoRoot "mixa_manager\tests\mixa_file_manager_selftest.lm1"
$TransOut = ""
$ExeOut = ""
$TransStdout = ""
$TransStderr = ""
...
$BaseDir = Join-Path $RepoRoot "build\mixa\claude\file_manager"
...
try {
    ...
    $TransOut = Join-Path $RunDir "mixa_file_manager_selftest.c"
    $ExeOut = Join-Path $RunDir "mixa_file_manager_selftest.exe"
