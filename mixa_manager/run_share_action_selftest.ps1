# Selection/Send button glue selftest runner (ticket 20260912-045346's
# "proceed directly to selection/button/nested-failure UI" next step).
# Patterned from run_share_selftest.ps1 with the same stable-compiler pin.
...
$LogDir = ""
$FixtureDir = ""
$TestSource = Join-Path $RepoRoot "mixa_manager\tests\mixa_share_action_selftest.lm1"
$TransOut = ""
$ExeOut = ""
$TransStdout = ""
$TransStderr = ""
...
$BaseDir = Join-Path $RepoRoot "build\mixa\claude\share_action"
...
try {
    ...
    # same two-step pattern as run_share_selftest.ps1's own headers.
    ...
    # Translate test (predefs headers + mixa_selection.lm1 + mixa_share.lm1
    # controller + mixa_share_action.lm1 glue + fake backend + main, all in
    # one TU -- same single-file pattern as run_share_selftest.ps1).
    $TransOut = Join-Path $RunDir "mixa_share_action_selftest.c"
    $ExeOut = Join-Path $RunDir "mixa_share_action_selftest.exe"
    ...
    $ShareImplHashFile = Join-Path $LogDir "share_impl_hash.txt"
    $SelectionImplHashFile = Join-Path $LogDir "selection_impl_hash.txt"
    $ActionImplHashFile = Join-Path $LogDir "action_impl_hash.txt"
    ...
    try {
        ...
        $ShareImplPath = Join-Path $RepoRoot "mixa_manager\mixa_share.lm1"
        if (Test-Path -LiteralPath $ShareImplPath -PathType Leaf) {
            Set-Content -LiteralPath $ShareImplHashFile -Value ((Get-FileHash -LiteralPath $ShareImplPath -Algorithm SHA256).Hash)
        }
        $SelectionImplPath = Join-Path $RepoRoot "mixa_manager\mixa_selection.lm1"
        if (Test-Path -LiteralPath $SelectionImplPath -PathType Leaf) {
            Set-Content -LiteralPath $SelectionImplHashFile -Value ((Get-FileHash -LiteralPath $SelectionImplPath -Algorithm SHA256).Hash)
        }
        $ActionImplPath = Join-Path $RepoRoot "mixa_manager\mixa_share_action.lm1"
        if (Test-Path -LiteralPath $ActionImplPath -PathType Leaf) {
            Set-Content -LiteralPath $ActionImplHashFile -Value ((Get-FileHash -LiteralPath $ActionImplPath -Algorithm SHA256).Hash)
        }
...
Share-Impl-Hash-File: $ShareImplHashFile
Selection-Impl-Hash-File: $SelectionImplHashFile
Action-Impl-Hash-File: $ActionImplHashFile
