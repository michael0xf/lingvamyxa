# Real L1-vs-L2 parity gate for mixa_calculator_syntax (ticket
# 20260913-231500 Part 2, module 1 of 8). Fully self-contained: no OS or
...
#   2. Attempts to translate the COMPLETE mixa_calculator_syntax.lm2.
...
#
# mixa_calculator_syntax.lm1 and mixa_calculator_syntax.h.lm1 are the
...
$BaseDir = Join-Path $RepoRoot "build\mixa\claude\mixa_calculator_syntax_l2_parity"
...
# ---- Step 0: translate the real L1 header, then the L2 header unit. ----
Invoke-HeaderTrans "mixa_manager\mixa_calculator_syntax.h.lm1" "mixa_calculator_syntax.lm1.h"
Invoke-HeaderTrans "mixa_manager\mixa_calculator_syntax_l2.h.lm1" "mixa_calculator_syntax_l2.lm1.h"
...
$hExit = Invoke-Cmd $L1Trans "mixa_manager\tests\mixa_calculator_syntax_parity_harness.lm1 `"$harnessC`"" $hLog1 $hLog2
Pop-Location
...
$oracleC = Join-Path $RunDir "mixa_calculator_syntax_oracle.c"
$ocLog1 = Join-Path $RunDir "oracle_trans_stdout.log"
$ocLog2 = Join-Path $RunDir "oracle_trans_stderr.log"
Push-Location $RepoRoot
$ocExit = Invoke-Cmd $L1Trans "mixa_manager\mixa_calculator_syntax.lm1 `"$oracleC`"" $ocLog1 $ocLog2
Pop-Location
if ($ocExit -ne 0) { Get-Content $ocLog2; throw "oracle mixa_calculator_syntax.lm1 translation failed" }

$oracleO = Join-Path $RunDir "mixa_calculator_syntax_oracle.o"
...
if ($occExit -ne 0) { Get-Content $occLog2; throw "oracle mixa_calculator_syntax.lm1 compile failed" }
...
# ---- Step 4: attempt the COMPLETE mixa_calculator_syntax.lm2 translation. ----
Push-Location $RepoRoot
$csSrc = "mixa_manager\mixa_calculator_syntax.lm2"
$csOut = Join-Path $RunDir "mixa_calculator_syntax_l2.lm1"
$csStdout = Join-Path $RunDir "cs_stdout.log"
$csStderr = Join-Path $RunDir "cs_stderr.log"
$CsExit = Invoke-Cmd "`"$l2exe`"" "`"$csSrc`" `"$csOut`"" $csStdout $csStderr
Pop-Location

$CsStdoutText = if (Test-Path -LiteralPath $csStdout) { Get-Content -LiteralPath $csStdout -Raw } else { "" }
$CsStderrText = if (Test-Path -LiteralPath $csStderr) { Get-Content -LiteralPath $csStderr -Raw } else { "" }
$CsSourceHash = (Get-FileHash -LiteralPath (Join-Path $RepoRoot $csSrc) -Algorithm SHA256).Hash
...
$KnownBarrier = ($CsStderrText -match "unknown foreign type") -or ($CsStderrText -match "incompatible entry signature") -or ($CsStderrText -match "unsupported own array declaration")

if ($CsExit -ne 0 -and $KnownBarrier) {
    ...
    Push-Location $RepoRoot
    $l2CsC = Join-Path $RunDir "mixa_calculator_syntax_l2.c"
    $l2ccLog1 = Join-Path $RunDir "l2cs_trans_stdout.log"
    $l2ccLog2 = Join-Path $RunDir "l2cs_trans_stderr.log"
    $l2ccExit = Invoke-Cmd $L1Trans "`"$csOut`" `"$l2CsC`"" $l2ccLog1 $l2ccLog2
    Pop-Location
    if ($l2ccExit -ne 0) {
        ...
        $l2CsO = Join-Path $RunDir "mixa_calculator_syntax_l2.o"
        $l2occLog1 = Join-Path $RunDir "l2cs_compile_stdout.log"
        $l2occLog2 = Join-Path $RunDir "l2cs_compile_stderr.log"
        $l2occExit = Invoke-Cmd "gcc" "$GccStd -I `"$RepoRoot`" -I `"$RunDir\headers`" -c `"$l2CsC`" -o `"$l2CsO`"" $l2occLog1 $l2occLog2
        if ($l2occExit -ne 0) {
            ...
            $l2olLog1 = Join-Path $RunDir "l2cs_link_stdout.log"
            $l2olLog2 = Join-Path $RunDir "l2cs_link_stderr.log"
            $l2olExit = Invoke-Cmd "gcc" "$GccStd -I `"$RepoRoot`" -I `"$RunDir\headers`" `"$harnessO`" `"$l2CsO`" -o `"$l2Exe`"" $l2olLog1 $l2olLog2
            if ($l2olExit -ne 0) {
...
CalculatorSyntax-L2-Header: mixa_manager\mixa_calculator_syntax_l2.h.lm1
CalculatorSyntax-L2-Source: $csSrc
CalculatorSyntax-L2-Source-Sha256: $CsSourceHash
Harness-Source-Sha256: $((Get-FileHash -LiteralPath (Join-Path $RepoRoot "mixa_manager\tests\mixa_calculator_syntax_parity_harness.lm1") -Algorithm SHA256).Hash)
Runner-Sha256: $RunnerHash
Oracle-Run-Exit-Code: $oracleRunExit
Oracle-Trace:
$OracleTrace
CalculatorSyntax-Translate-Exit-Code: $CsExit
CalculatorSyntax-Translate-Stdout:
$CsStdoutText
CalculatorSyntax-Translate-Stderr:
$CsStderrText
L2-Trace:
$L2TraceText
...
switch ($Verdict) {
    "EXPECTED_CORE_BARRIER" {
        "EXPECTED_CORE_BARRIER: full mixa_calculator_syntax.lm2 translation stopped at an already-known barrier. This is NOT a pass -- the integrated frontend is not yet on main. The oracle-side harness DID run (see Oracle-Trace above)."
    }
    "UNEXPECTED_FAILURE" {
    ...
    "PASS" {
        "PASS: full mixa_calculator_syntax.lm2 translated, built, and ran; its trace is byte-identical to the L1 oracle's own trace."
    }
}
