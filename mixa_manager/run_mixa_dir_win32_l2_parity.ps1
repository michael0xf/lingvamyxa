# Real L1-vs-L2 parity gate for mixa_dir_win32 (dependency-order
# continuation after mixa_selection_walk). No dedicated ABI probe pair:
# this module declares no new struct of its own beyond what mixa_dir_
# win32.h.lm1/mixa_dir.h.lm1 already declare (MixaDir/MixaDirEntry),
# reached via the REAL headers predef'd directly, identically on both
# the real oracle and this L2 port.
#
# Dependency-linking note: mixa_dir_win32.lm1 predefs ONLY its own real
# header plus mixa_dir.h.lm1 (both header-only, no bodies) -- this
# module sits at the bottom of the dependency stack (windows.h direct
# Win32 calls), so BOTH the oracle side and the L2 side compile a
# single real/generated object with no separate real dependency
# objects needed at all.
#
#   1. ALWAYS builds and runs the ORACLE-side harness (a real fixture
#      tree: files, a subdirectory, an empty directory, a non-empty
#      directory, argument negatives, a real rename/remove sequence,
#      and a test-owned invalid enumeration handle proving ERR_IO).
#   2. Attempts to translate the COMPLETE mixa_dir_win32.lm2.
...
#
# mixa_dir_win32.h.lm1/.lm1 and mixa_dir.h.lm1 are the parity oracle
# (or real, unmodified dependencies) and are never touched. Nothing
...
$BaseDir = Join-Path $RepoRoot "build\mixa\claude\mixa_dir_win32_l2_parity"
...
Invoke-HeaderTrans "mixa_manager\mixa_dir_win32_l2.h.lm1" "mixa_dir_win32_l2.lm1.h"
...
$hExit = Invoke-Cmd $L1Trans "mixa_manager\tests\mixa_dir_win32_parity_harness.lm1 `"$harnessC`"" $hLog1 $hLog2
Pop-Location
...
# ---- Step 3: ALWAYS build + run the ORACLE-side harness. mixa_dir_
# win32.lm1 predefs ONLY headers (see top comment) -- no separate real
# dependency objects needed. ----
$oracleC = Join-Path $RunDir "mixa_dir_win32_oracle.c"
$ocLog1 = Join-Path $RunDir "oracle_trans_stdout.log"
$ocLog2 = Join-Path $RunDir "oracle_trans_stderr.log"
Push-Location $RepoRoot
$ocExit = Invoke-Cmd $L1Trans "mixa_manager\mixa_dir_win32.lm1 `"$oracleC`"" $ocLog1 $ocLog2
Pop-Location
if ($ocExit -ne 0) { Get-Content $ocLog2; throw "oracle mixa_dir_win32.lm1 translation failed" }

$oracleO = Join-Path $RunDir "mixa_dir_win32_oracle.o"
...
if ($occExit -ne 0) { Get-Content $occLog2; throw "oracle mixa_dir_win32.lm1 compile failed" }
...
# ---- Step 4: attempt the COMPLETE mixa_dir_win32.lm2 translation. ----
Push-Location $RepoRoot
$dwSrc = "mixa_manager\mixa_dir_win32.lm2"
$dwOut = Join-Path $RunDir "mixa_dir_win32_l2.lm1"
$dwStdout = Join-Path $RunDir "dw_stdout.log"
$dwStderr = Join-Path $RunDir "dw_stderr.log"
$DwExit = Invoke-Cmd "`"$l2exe`"" "`"$dwSrc`" `"$dwOut`"" $dwStdout $dwStderr
Pop-Location

$DwStdoutText = if (Test-Path -LiteralPath $dwStdout) { Get-Content -LiteralPath $dwStdout -Raw } else { "" }
$DwStderrText = if (Test-Path -LiteralPath $dwStderr) { Get-Content -LiteralPath $dwStderr -Raw } else { "" }
$DwSourceHash = (Get-FileHash -LiteralPath (Join-Path $RepoRoot $dwSrc) -Algorithm SHA256).Hash
...
$KnownBarrier = ($DwStderrText -match "unknown foreign type") -or ($DwStderrText -match "incompatible entry signature")

if ($DwExit -ne 0 -and $KnownBarrier) {
    ...
    Push-Location $RepoRoot
    $l2DwC = Join-Path $RunDir "mixa_dir_win32_l2.c"
    $l2ccLog1 = Join-Path $RunDir "l2dw_trans_stdout.log"
    $l2ccLog2 = Join-Path $RunDir "l2dw_trans_stderr.log"
    $l2ccExit = Invoke-Cmd $L1Trans "`"$dwOut`" `"$l2DwC`"" $l2ccLog1 $l2ccLog2
    Pop-Location
    if ($l2ccExit -ne 0) {
        ...
        $l2DwO = Join-Path $RunDir "mixa_dir_win32_l2.o"
        $l2occLog1 = Join-Path $RunDir "l2dw_compile_stdout.log"
        $l2occLog2 = Join-Path $RunDir "l2dw_compile_stderr.log"
        $l2occExit = Invoke-Cmd "gcc" "$GccStd -I `"$RepoRoot`" -I `"$RunDir\headers`" -c `"$l2DwC`" -o `"$l2DwO`"" $l2occLog1 $l2occLog2
        if ($l2occExit -ne 0) {
            ...
            $l2olLog1 = Join-Path $RunDir "l2dw_link_stdout.log"
            $l2olLog2 = Join-Path $RunDir "l2dw_link_stderr.log"
            $l2olExit = Invoke-Cmd "gcc" "$GccStd -I `"$RepoRoot`" -I `"$RunDir\headers`" `"$harnessO`" `"$l2DwO`" -o `"$l2Exe`"" $l2olLog1 $l2olLog2
            if ($l2olExit -ne 0) {
...
DirWin32-L2-Header: mixa_manager\mixa_dir_win32_l2.h.lm1
DirWin32-L2-Source: $dwSrc
DirWin32-L2-Source-Sha256: $DwSourceHash
Harness-Source-Sha256: $((Get-FileHash -LiteralPath (Join-Path $RepoRoot "mixa_manager\tests\mixa_dir_win32_parity_harness.lm1") -Algorithm SHA256).Hash)
Runner-Sha256: $RunnerHash
Oracle-Run-Exit-Code: $oracleRunExit
Oracle-Trace:
$OracleTrace
DirWin32-Translate-Exit-Code: $DwExit
DirWin32-Translate-Stdout:
$DwStdoutText
DirWin32-Translate-Stderr:
$DwStderrText
L2-Trace:
$L2TraceText
...
switch ($Verdict) {
    "EXPECTED_CORE_BARRIER" {
        "EXPECTED_CORE_BARRIER: full mixa_dir_win32.lm2 translation stopped at an already-known barrier. This is NOT a pass -- the integrated frontend is not yet on main. The oracle-side harness DID run (see Oracle-Trace above)."
    }
    "UNEXPECTED_FAILURE" {
    ...
    "PASS" {
        "PASS: full mixa_dir_win32.lm2 translated, built, and ran; its trace is byte-identical to the L1 oracle's own trace."
    }
}
