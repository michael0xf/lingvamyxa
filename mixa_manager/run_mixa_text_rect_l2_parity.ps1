# Real L1-vs-L2 parity gate for mixa_text_rect (ticket 20260913-100000).
# Same structure as run_mixa_tiles_l2_parity.ps1 (093000). Per this
# ticket's own instruction 2 ("reuse the proven MixaCell/MixaTextRect
# declaration strategy from commit 414f7a7f where compatible; do not
# create a second conflicting layout"), this reuses mixa_tiles_l2.h.lm1
# AND its own ABI probe pair directly -- no new struct header or ABI
# probes were created for this module.
#   0. ABI parity gate: reuses mixa_tiles_abi_probe_real.c/_l2.c (same
#      MixaCell/MixaTextRect structs, already verified byte-identical
#      for the tiles port). Hard failure (ABI_MISMATCH, exit 1) on any
#      difference.
#   1. ALWAYS builds and runs the ORACLE-side harness.
#   2. Attempts to translate the COMPLETE mixa_text_rect.lm2.
#      - fails with an already-known barrier diagnostic -> EXPECTED_
#        CORE_BARRIER (exit 2). NOT a pass -- the integrated frontend
#        with custom-struct-pointer/[]: size_t support is not yet on
#        main (ticket 20260913-093000/-100000's own own finding).
...
#
# mixa_text_rect.lm1/mixa_core.h are the parity oracle and are never
...
$BaseDir = Join-Path $RepoRoot "build\mixa\claude\mixa_text_rect_l2_parity"
...
# ---- Step 0: translate the shared header-unit once (reused from the
# mixa_tiles port, 414f7a7f). ----
...
# ---- Step 2: build the harness object. ----
Push-Location $RepoRoot
...
$hExit = Invoke-Cmd $L1Trans "mixa_manager\tests\mixa_text_rect_parity_harness.lm1 `"$harnessC`"" $hLog1 $hLog2
Pop-Location
...
$oracleC = Join-Path $RunDir "mixa_text_rect_oracle.c"
$ocLog1 = Join-Path $RunDir "oracle_trans_stdout.log"
$ocLog2 = Join-Path $RunDir "oracle_trans_stderr.log"
Push-Location $RepoRoot
$ocExit = Invoke-Cmd $L1Trans "mixa_manager\mixa_text_rect.lm1 `"$oracleC`"" $ocLog1 $ocLog2
Pop-Location
if ($ocExit -ne 0) { Get-Content $ocLog2; throw "oracle mixa_text_rect.lm1 translation failed" }

$oracleO = Join-Path $RunDir "mixa_text_rect_oracle.o"
...
if ($occExit -ne 0) { Get-Content $occLog2; throw "oracle mixa_text_rect.lm1 compile failed" }
...
# ---- Step 4: attempt the COMPLETE mixa_text_rect.lm2 translation. ----
Push-Location $RepoRoot
$trSrc = "mixa_manager\mixa_text_rect.lm2"
$trOut = Join-Path $RunDir "mixa_text_rect_l2.lm1"
$trStdout = Join-Path $RunDir "tr_stdout.log"
$trStderr = Join-Path $RunDir "tr_stderr.log"
$TrExit = Invoke-Cmd "`"$l2exe`"" "`"$trSrc`" `"$trOut`"" $trStdout $trStderr
Pop-Location

$TrStdoutText = if (Test-Path -LiteralPath $trStdout) { Get-Content -LiteralPath $trStdout -Raw } else { "" }
$TrStderrText = if (Test-Path -LiteralPath $trStderr) { Get-Content -LiteralPath $trStderr -Raw } else { "" }
$TrSourceHash = (Get-FileHash -LiteralPath (Join-Path $RepoRoot $trSrc) -Algorithm SHA256).Hash
...
$KnownBarrier = ($TrStderrText -match "unknown foreign type") -or ($TrStderrText -match "incompatible entry signature")

if ($TrExit -ne 0 -and $KnownBarrier) {
    ...
    Push-Location $RepoRoot
    $l2TrC = Join-Path $RunDir "mixa_text_rect_l2.c"
    $l2tcLog1 = Join-Path $RunDir "l2tr_trans_stdout.log"
    $l2tcLog2 = Join-Path $RunDir "l2tr_trans_stderr.log"
    $l2tcExit = Invoke-Cmd $L1Trans "`"$trOut`" `"$l2TrC`"" $l2tcLog1 $l2tcLog2
    Pop-Location
    if ($l2tcExit -ne 0) {
        ...
        $l2TrO = Join-Path $RunDir "mixa_text_rect_l2.o"
        $l2occLog1 = Join-Path $RunDir "l2tr_compile_stdout.log"
        $l2occLog2 = Join-Path $RunDir "l2tr_compile_stderr.log"
        $l2occExit = Invoke-Cmd "gcc" "$GccStd -I `"$RepoRoot`" -I `"$RunDir\headers`" -c `"$l2TrC`" -o `"$l2TrO`"" $l2occLog1 $l2occLog2
        if ($l2occExit -ne 0) {
            ...
            $l2olLog1 = Join-Path $RunDir "l2tr_link_stdout.log"
            $l2olLog2 = Join-Path $RunDir "l2tr_link_stderr.log"
            $l2olExit = Invoke-Cmd "gcc" "$GccStd -I `"$RepoRoot`" -I `"$RunDir\headers`" `"$harnessO`" `"$l2TrO`" -o `"$l2Exe`"" $l2olLog1 $l2olLog2
            if ($l2olExit -ne 0) {
...
TextRect-L2-Header (reused from 414f7a7f): mixa_manager\mixa_tiles_l2.h.lm1
TextRect-L2-Source: $trSrc
TextRect-L2-Source-Sha256: $TrSourceHash
Harness-Source-Sha256: $((Get-FileHash -LiteralPath (Join-Path $RepoRoot "mixa_manager\tests\mixa_text_rect_parity_harness.lm1") -Algorithm SHA256).Hash)
Runner-Sha256: $RunnerHash
Oracle-Run-Exit-Code: $oracleRunExit
Oracle-Trace:
$OracleTrace
TextRect-Translate-Exit-Code: $TrExit
TextRect-Translate-Stdout:
$TrStdoutText
TextRect-Translate-Stderr:
$TrStderrText
L2-Trace:
$L2TraceText
...
switch ($Verdict) {
    "EXPECTED_CORE_BARRIER" {
        "EXPECTED_CORE_BARRIER: full mixa_text_rect.lm2 translation stopped at an already-known barrier. This is NOT a pass -- Codex's integrated frontend is not yet on main. The oracle-side harness DID run (see Oracle-Trace above)."
    }
    "UNEXPECTED_FAILURE" {
    ...
    "PASS" {
        "PASS: full mixa_text_rect.lm2 translated, built, and ran; its trace is byte-identical to the L1 oracle's own trace."
    }
}
