# Real L1-vs-L2 parity gate for mixa_selection_walk (dependency-order
# continuation after ticket 20260913-175200's own mixa_remove_confirm).
# No dedicated ABI probe pair: this module declares no new struct of
# its own (MixaSelWalkSink/MixaSelWalkAllocVTable are already real,
# unmodified declarations reached via the REAL mixa_selection_walk.h.
# lm1 predef'd directly, identically on both the real oracle and this
# L2 port).
#
# Dependency-linking note: mixa_selection_walk.lm1 predefs the FULL
...
# selection_walk.h.lm1 (header-only, no bodies), so the L2 side
...
#
#   1. ALWAYS builds and runs the ORACLE-side harness (real nested
#      directory trees, real lazy visit-order proof by position, real
#      allocation-fault sweep across every owned step, real root-array
#      consolidation during the scan, a real UTF-8 directory name).
#   2. Attempts to translate the COMPLETE mixa_selection_walk.lm2.
...
#
# mixa_selection_walk.h.lm1/.lm1 and every real dependency (mixa_dir_
...
$BaseDir = Join-Path $RepoRoot "build\mixa\claude\mixa_selection_walk_l2_parity"
...
Invoke-HeaderTrans "mixa_manager\mixa_selection_walk_l2.h.lm1" "mixa_selection_walk_l2.lm1.h"
...
$hExit = Invoke-Cmd $L1Trans "mixa_manager\tests\mixa_selection_walk_parity_harness.lm1 `"$harnessC`"" $hLog1 $hLog2
Pop-Location
...
# selection_walk.lm1's own translation unit already embeds mixa_dir_
# win32/mixa_selection (see top comment) -- no separate real objects
# needed. ----
$oracleC = Join-Path $RunDir "mixa_selection_walk_oracle.c"
$ocLog1 = Join-Path $RunDir "oracle_trans_stdout.log"
$ocLog2 = Join-Path $RunDir "oracle_trans_stderr.log"
Push-Location $RepoRoot
$ocExit = Invoke-Cmd $L1Trans "mixa_manager\mixa_selection_walk.lm1 `"$oracleC`"" $ocLog1 $ocLog2
Pop-Location
if ($ocExit -ne 0) { Get-Content $ocLog2; throw "oracle mixa_selection_walk.lm1 translation failed" }

$oracleO = Join-Path $RunDir "mixa_selection_walk_oracle.o"
...
if ($occExit -ne 0) { Get-Content $occLog2; throw "oracle mixa_selection_walk.lm1 compile failed" }
...
# ---- Step 4: attempt the COMPLETE mixa_selection_walk.lm2 translation. ----
Push-Location $RepoRoot
$swSrc = "mixa_manager\mixa_selection_walk.lm2"
...
$KnownBarrier = ($SwStderrText -match "unknown foreign type") -or ($SwStderrText -match "incompatible entry signature")
...
if ($SwExit -ne 0 -and $KnownBarrier) {
    ...
    Push-Location $RepoRoot
    $l2SwC = Join-Path $RunDir "mixa_selection_walk_l2.c"
...
SelectionWalk-L2-Header: mixa_manager\mixa_selection_walk_l2.h.lm1
SelectionWalk-L2-Source: $swSrc
SelectionWalk-L2-Source-Sha256: $SwSourceHash
Harness-Source-Sha256: $((Get-FileHash -LiteralPath (Join-Path $RepoRoot "mixa_manager\tests\mixa_selection_walk_parity_harness.lm1") -Algorithm SHA256).Hash)
Runner-Sha256: $RunnerHash
Oracle-Run-Exit-Code: $oracleRunExit
Oracle-Trace:
$OracleTrace
SelectionWalk-Translate-Exit-Code: $SwExit
SelectionWalk-Translate-Stdout:
$SwStdoutText
SelectionWalk-Translate-Stderr:
$SwStderrText
L2-Trace:
$L2TraceText
...
switch ($Verdict) {
    "EXPECTED_CORE_BARRIER" {
        "EXPECTED_CORE_BARRIER: full mixa_selection_walk.lm2 translation stopped at an already-known barrier. This is NOT a pass -- the integrated frontend is not yet on main. The oracle-side harness DID run (see Oracle-Trace above)."
    }
    "UNEXPECTED_FAILURE" {
    ...
    "PASS" {
        "PASS: full mixa_selection_walk.lm2 translated, built, and ran; its trace is byte-identical to the L1 oracle's own trace."
    }
}
