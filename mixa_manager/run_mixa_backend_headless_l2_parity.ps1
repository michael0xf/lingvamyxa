# Real L1-vs-L2 parity gate for mixa_backend_headless (share-and-
# backends ticket 20260913-192700, module 6 of 6, the LAST module in
# the standing list). No dedicated ABI probe pair, and no L1 header
# unit to predef at all: mixa_backend_headless.h has no .h.lm1
# counterpart anywhere in this codebase -- real, plain, hand-written C.
# MixaHeadless's own fields are dereferenced via `\` with no L1-level
# struct: declaration, proven safe by the established "`\` defers to
# gcc" finding.
#
# No dedicated selftest exists for this module alone in the accepted
...
# others (all already re-verified clean by run_mixa.ps1's own
# regression sweep, unaffected by this port since it touches no real
# file). This gate exercises the module's own real contract directly.
#
#   1. ALWAYS builds and runs the ORACLE-side harness (open() argument/
#      odd-cell/overflow validation; a real present()/frame_at() pixel
#      round trip with a too-small-byte-count refusal; poll() on an
#      empty inbox vs a real pushed event in FIFO order; a real RESIZE
#      event actually reallocating the frame buffer, proven by writing
#      and reading the new frame; a real clipboard set/get round trip
#      including the short-buffer-reports-required-length contract and
#      the unset-clipboard-empty case; push_event's own real pixel-to-
#      half-cell conversion for MOUSE and MOUSE_WHEEL; glyph() MISSING
#      for every valid layer and refusal for an invalid one; and the
#      singleton table constructor's identical-pointer-across-calls
#      contract).
#   2. Attempts to translate the COMPLETE mixa_backend_headless.lm2.
...
#
# mixa_backend_headless.h/.lm1 and every real dependency (mixa_event_
# fifo, mixa_backend_ctors_headless) are the parity oracle (or real,
...
$BaseDir = Join-Path $RepoRoot "build\mixa\claude\mixa_backend_headless_l2_parity"
...
# ---- Step 0: translate the L2 header (mixa_backend_headless.h is
# plain C, no other real headers to translate). ----
Invoke-HeaderTrans "mixa_manager\mixa_backend_headless_l2.h.lm1" "mixa_backend_headless_l2.lm1.h"
...
$hExit = Invoke-Cmd $L1Trans "mixa_manager\tests\mixa_backend_headless_parity_harness.lm1 `"$harnessC`"" $hLog1 $hLog2
Pop-Location
...
# oracle and L2 sides -- mixa_backend_headless.lm1 predefs nothing). ----
$eventFifoO = Build-RealDep "mixa_event_fifo" "mixa_manager\mixa_event_fifo.lm1"

# ---- Step 4: ALWAYS build + run the ORACLE-side harness. ----
$oracleC = Join-Path $RunDir "mixa_backend_headless_oracle.c"
$ocLog1 = Join-Path $RunDir "oracle_trans_stdout.log"
$ocLog2 = Join-Path $RunDir "oracle_trans_stderr.log"
Push-Location $RepoRoot
$ocExit = Invoke-Cmd $L1Trans "mixa_manager\mixa_backend_headless.lm1 `"$oracleC`"" $ocLog1 $ocLog2
Pop-Location
if ($ocExit -ne 0) { Get-Content $ocLog2; throw "oracle mixa_backend_headless.lm1 translation failed" }

$oracleO = Join-Path $RunDir "mixa_backend_headless_oracle.o"
...
if ($occExit -ne 0) { Get-Content $occLog2; throw "oracle mixa_backend_headless.lm1 compile failed" }
...
$olExit = Invoke-Cmd "gcc" "$GccStd -I `"$RepoRoot`" -I `"$RunDir\headers`" `"$harnessO`" `"$oracleO`" `"$eventFifoO`" -o `"$oracleExe`"" $olLog1 $olLog2
...
# ---- Step 5: attempt the COMPLETE mixa_backend_headless.lm2 translation. ----
Push-Location $RepoRoot
$bhSrc = "mixa_manager\mixa_backend_headless.lm2"
$bhOut = Join-Path $RunDir "mixa_backend_headless_l2.lm1"
$bhStdout = Join-Path $RunDir "bh_stdout.log"
$bhStderr = Join-Path $RunDir "bh_stderr.log"
$BhExit = Invoke-Cmd "`"$l2exe`"" "`"$bhSrc`" `"$bhOut`"" $bhStdout $bhStderr
Pop-Location

$BhStdoutText = if (Test-Path -LiteralPath $bhStdout) { Get-Content -LiteralPath $bhStdout -Raw } else { "" }
$BhStderrText = if (Test-Path -LiteralPath $bhStderr) { Get-Content -LiteralPath $bhStderr -Raw } else { "" }
$BhSourceHash = (Get-FileHash -LiteralPath (Join-Path $RepoRoot $bhSrc) -Algorithm SHA256).Hash
...
$KnownBarrier = ($BhStderrText -match "unknown foreign type") -or ($BhStderrText -match "incompatible entry signature") -or ($BhStderrText -match "unsupported own array declaration")

if ($BhExit -ne 0 -and $KnownBarrier) {
    ...
    Push-Location $RepoRoot
    $l2BhC = Join-Path $RunDir "mixa_backend_headless_l2.c"
    $l2ccLog1 = Join-Path $RunDir "l2bh_trans_stdout.log"
    $l2ccLog2 = Join-Path $RunDir "l2bh_trans_stderr.log"
    $l2ccExit = Invoke-Cmd $L1Trans "`"$bhOut`" `"$l2BhC`"" $l2ccLog1 $l2ccLog2
    Pop-Location
    if ($l2ccExit -ne 0) {
        ...
        $l2BhO = Join-Path $RunDir "mixa_backend_headless_l2.o"
        $l2occLog1 = Join-Path $RunDir "l2bh_compile_stdout.log"
        $l2occLog2 = Join-Path $RunDir "l2bh_compile_stderr.log"
        $l2occExit = Invoke-Cmd "gcc" "$GccStd -I `"$RepoRoot`" -I `"$RunDir\headers`" -c `"$l2BhC`" -o `"$l2BhO`"" $l2occLog1 $l2occLog2
        if ($l2occExit -ne 0) {
            ...
            $l2olLog1 = Join-Path $RunDir "l2bh_link_stdout.log"
            $l2olLog2 = Join-Path $RunDir "l2bh_link_stderr.log"
            $l2olExit = Invoke-Cmd "gcc" "$GccStd -I `"$RepoRoot`" -I `"$RunDir\headers`" `"$harnessO`" `"$l2BhO`" `"$eventFifoO`" -o `"$l2Exe`"" $l2olLog1 $l2olLog2
            if ($l2olExit -ne 0) {
...
BackendHeadless-L2-Header: mixa_manager\mixa_backend_headless_l2.h.lm1
BackendHeadless-L2-Source: $bhSrc
BackendHeadless-L2-Source-Sha256: $BhSourceHash
Harness-Source-Sha256: $((Get-FileHash -LiteralPath (Join-Path $RepoRoot "mixa_manager\tests\mixa_backend_headless_parity_harness.lm1") -Algorithm SHA256).Hash)
Runner-Sha256: $RunnerHash
Oracle-Run-Exit-Code: $oracleRunExit
Oracle-Trace:
$OracleTrace
BackendHeadless-Translate-Exit-Code: $BhExit
BackendHeadless-Translate-Stdout:
$BhStdoutText
BackendHeadless-Translate-Stderr:
$BhStderrText
L2-Trace:
$L2TraceText
...
switch ($Verdict) {
    "EXPECTED_CORE_BARRIER" {
        "EXPECTED_CORE_BARRIER: full mixa_backend_headless.lm2 translation stopped at an already-known barrier. This is NOT a pass -- the integrated frontend is not yet on main. The oracle-side harness DID run (see Oracle-Trace above)."
    }
    "UNEXPECTED_FAILURE" {
    ...
    "PASS" {
        "PASS: full mixa_backend_headless.lm2 translated, built, and ran; its trace is byte-identical to the L1 oracle's own trace."
    }
}
