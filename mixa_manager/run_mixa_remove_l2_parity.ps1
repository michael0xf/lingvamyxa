# Real L1-vs-L2 parity gate for mixa_remove (ticket 20260913-171800)
# -- the direct dependency mixa_fm_remove (already ported, ticket
# 20260913-161000) wires to the file-manager view.
#
# Dependency-linking note: mixa_remove.lm1 predefs the FULL body of
# mixa_selection_walk.lm1 directly (which itself predefs the full
# bodies of mixa_dir_win32.lm1 and mixa_selection.lm1) -- so the
# ORACLE side's own translation unit already embeds all three and
# needs no separate real objects. The L2 header instead predefs ONLY
# mixa_remove.h.lm1 (header-only, no bodies), so the L2 side separately
# links the real, unmodified mixa_selection_walk.lm1 (which embeds
# dir_win32/selection the same way) for those symbols.
#
#   1. ALWAYS builds and runs the ORACLE-side harness (a real nested
#      tree with strict postorder removal, real read-only/enumeration/
#      allocation faults, real cancellation between roots, a real
#      Unicode file name).
#   2. Attempts to translate the COMPLETE mixa_remove.lm2.
...
#
# mixa_remove.h.lm1/.lm1 and every real dependency (mixa_selection_
# walk.h.lm1/.lm1, mixa_dir_win32.h.lm1/.lm1, mixa_selection.h/.lm1,
# mixa_dir.h.lm1) are the parity oracle (or real, unmodified
...
$BaseDir = Join-Path $RepoRoot "build\mixa\claude\mixa_remove_l2_parity"
...
Invoke-HeaderTrans "mixa_manager\mixa_remove_l2.h.lm1" "mixa_remove_l2.lm1.h"
...
$hExit = Invoke-Cmd $L1Trans "mixa_manager\tests\mixa_remove_parity_harness.lm1 `"$harnessC`"" $hLog1 $hLog2
Pop-Location
...
# remove.lm1's own translation unit already embeds mixa_selection_walk
# /mixa_dir_win32/mixa_selection (see top comment) -- no separate real
# objects needed. ----
$oracleC = Join-Path $RunDir "mixa_remove_oracle.c"
$ocLog1 = Join-Path $RunDir "oracle_trans_stdout.log"
$ocLog2 = Join-Path $RunDir "oracle_trans_stderr.log"
Push-Location $RepoRoot
$ocExit = Invoke-Cmd $L1Trans "mixa_manager\mixa_remove.lm1 `"$oracleC`"" $ocLog1 $ocLog2
Pop-Location
if ($ocExit -ne 0) { Get-Content $ocLog2; throw "oracle mixa_remove.lm1 translation failed" }

$oracleO = Join-Path $RunDir "mixa_remove_oracle.o"
...
if ($occExit -ne 0) { Get-Content $occLog2; throw "oracle mixa_remove.lm1 compile failed" }
...
# ---- Step 4: attempt the COMPLETE mixa_remove.lm2 translation. ----
Push-Location $RepoRoot
$rmSrc = "mixa_manager\mixa_remove.lm2"
$rmOut = Join-Path $RunDir "mixa_remove_l2.lm1"
$rmStdout = Join-Path $RunDir "rm_stdout.log"
$rmStderr = Join-Path $RunDir "rm_stderr.log"
$RmExit = Invoke-Cmd "`"$l2exe`"" "`"$rmSrc`" `"$rmOut`"" $rmStdout $rmStderr
Pop-Location

$RmStdoutText = if (Test-Path -LiteralPath $rmStdout) { Get-Content -LiteralPath $rmStdout -Raw } else { "" }
$RmStderrText = if (Test-Path -LiteralPath $rmStderr) { Get-Content -LiteralPath $rmStderr -Raw } else { "" }
$RmSourceHash = (Get-FileHash -LiteralPath (Join-Path $RepoRoot $rmSrc) -Algorithm SHA256).Hash
...
# "unknown foreign type" covers every prior module's own first-custom-
# struct-parameter variant; "incompatible entry signature" additionally
# covers this module's own NEW first-encountered trigger shape -- a
# bare fnptr-TYPEDEF parameter (MixaRemoveNodeFn), not a struct at all
# -- isolated precisely at mixa_remove_invoke's own first parameter,
# see the port note.
$KnownBarrier = ($RmStderrText -match "unknown foreign type") -or ($RmStderrText -match "incompatible entry signature")

if ($RmExit -ne 0 -and $KnownBarrier) {
    ...
    Push-Location $RepoRoot
    $l2RmC = Join-Path $RunDir "mixa_remove_l2.c"
    $l2ccLog1 = Join-Path $RunDir "l2rm_trans_stdout.log"
    $l2ccLog2 = Join-Path $RunDir "l2rm_trans_stderr.log"
    $l2ccExit = Invoke-Cmd $L1Trans "`"$rmOut`" `"$l2RmC`"" $l2ccLog1 $l2ccLog2
    Pop-Location
    if ($l2ccExit -ne 0) {
        ...
        $l2RmO = Join-Path $RunDir "mixa_remove_l2.o"
        $l2occLog1 = Join-Path $RunDir "l2rm_compile_stdout.log"
        $l2occLog2 = Join-Path $RunDir "l2rm_compile_stderr.log"
        $l2occExit = Invoke-Cmd "gcc" "$GccStd -I `"$RepoRoot`" -I `"$RunDir\headers`" -c `"$l2RmC`" -o `"$l2RmO`"" $l2occLog1 $l2occLog2
        if ($l2occExit -ne 0) {
            ...
            $l2olLog1 = Join-Path $RunDir "l2rm_link_stdout.log"
            $l2olLog2 = Join-Path $RunDir "l2rm_link_stderr.log"
            $l2olExit = Invoke-Cmd "gcc" "$GccStd -I `"$RepoRoot`" -I `"$RunDir\headers`" `"$harnessO`" `"$l2RmO`" `"$selWalkO`" -o `"$l2Exe`"" $l2olLog1 $l2olLog2
            if ($l2olExit -ne 0) {
...
Remove-L2-Header: mixa_manager\mixa_remove_l2.h.lm1
Remove-L2-Source: $rmSrc
Remove-L2-Source-Sha256: $RmSourceHash
Harness-Source-Sha256: $((Get-FileHash -LiteralPath (Join-Path $RepoRoot "mixa_manager\tests\mixa_remove_parity_harness.lm1") -Algorithm SHA256).Hash)
Runner-Sha256: $RunnerHash
Oracle-Run-Exit-Code: $oracleRunExit
Oracle-Trace:
$OracleTrace
Remove-Translate-Exit-Code: $RmExit
Remove-Translate-Stdout:
$RmStdoutText
Remove-Translate-Stderr:
$RmStderrText
L2-Trace:
$L2TraceText
...
switch ($Verdict) {
    "EXPECTED_CORE_BARRIER" {
        "EXPECTED_CORE_BARRIER: full mixa_remove.lm2 translation stopped at an already-known barrier. This is NOT a pass -- the integrated frontend is not yet on main. The oracle-side harness DID run (see Oracle-Trace above)."
    }
    "UNEXPECTED_FAILURE" {
    ...
    "PASS" {
        "PASS: full mixa_remove.lm2 translated, built, and ran; its trace is byte-identical to the L1 oracle's own trace."
    }
}
