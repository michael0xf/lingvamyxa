...
# drag_active/drag_grab_offset state -- through the existing headless
# backend with a real temporary fixture directory holding more entries
# than fit in the visible list. No window opens anywhere in this run;
...
$RunDir = ""
$LogDir = ""
$TestSource = Join-Path $RepoRoot "mixa_manager\tests\mixa_app_fmpanel_drag_e2e_selftest.lm1"
...
$BaseDir = Join-Path $RepoRoot "build\mixa\claude\app_fmpanel_drag_e2e"
...
try {
    ...
    $TransOut = Join-Path $RunDir "mixa_app_fmpanel_drag_e2e_selftest.c"
    $TestObj = Join-Path $RunDir "mixa_app_fmpanel_drag_e2e_selftest.o"
    $ExeOut = Join-Path $RunDir "mixa_app_fmpanel_drag_e2e_selftest.exe"
