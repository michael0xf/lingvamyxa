# Real L1-vs-L2 parity gate for mixa_file_win32 (ticket 20260913-114500).
# Same structure as run_mixa_pointer_l2_parity.ps1 (113000): fully self-
# contained, own dedicated ABI probe pair (mixa_file_win32_abi_probe_
# real.c/_l2.c), no other manager module linked. Differs only in
# needing -lkernel32 (real Win32 API calls: CreateFileW/ReadFile/
# WriteFile/CloseHandle/GetFileSize/SetFilePointer/GetLastError/
# SetLastError/MultiByteToWideChar/CreateDirectoryW) on every compiled
# object and the small system-header adapter mixa_file_win32_l2_win.h
# (WIN32_LEAN_AND_MEAN + <windows.h>, not a manager-owned declaration).
#   0. ABI parity gate: dedicated MixaFile sizeof/offsetof probe pair.
#   1. ALWAYS builds and runs the ORACLE-side harness (real mixa_
#      file_win32.lm1, unmodified).
#   2. Attempts to translate the COMPLETE mixa_file_win32.lm2.
...
$BaseDir = Join-Path $RepoRoot "build\mixa\claude\mixa_file_win32_l2_parity"
...
$fwHdrOut = Join-Path $HeaderDir "mixa_file_win32_l2.lm1.h"
$hdrExit = Invoke-Cmd $L1Trans "mixa_manager\mixa_file_win32_l2.h.lm1 `"$fwHdrOut`"" $hdrLog1 $hdrLog2
Pop-Location
if ($hdrExit -ne 0) { Get-Content $hdrLog2; throw "file_win32 header translation failed" }

# ---- Step 0.5: ABI parity gate (dedicated MixaFile probe pair). ----
...
$abiRealExit = Invoke-Cmd "gcc" "$GccStd -I `"$RepoRoot`" `"$RepoRoot\mixa_manager\tests\mixa_file_win32_abi_probe_real.c`" -o `"$abiRealExe`" $LinkLibs" $abiRealLog1 $abiRealLog2
...
$abiL2Exit = Invoke-Cmd "gcc" "$GccStd -I `"$RunDir\headers`" `"$RepoRoot\mixa_manager\tests\mixa_file_win32_abi_probe_l2.c`" -o `"$abiL2Exe`" $LinkLibs" $abiL2Log1 $abiL2Log2
...
$hExit = Invoke-Cmd $L1Trans "mixa_manager\tests\mixa_file_win32_parity_harness.lm1 `"$harnessC`"" $hLog1 $hLog2
Pop-Location
...
$oracleC = Join-Path $RunDir "mixa_file_win32_oracle.c"
$ocLog1 = Join-Path $RunDir "oracle_trans_stdout.log"
$ocLog2 = Join-Path $RunDir "oracle_trans_stderr.log"
Push-Location $RepoRoot
$ocExit = Invoke-Cmd $L1Trans "mixa_manager\mixa_file_win32.lm1 `"$oracleC`"" $ocLog1 $ocLog2
Pop-Location
if ($ocExit -ne 0) { Get-Content $ocLog2; throw "oracle mixa_file_win32.lm1 translation failed" }

$oracleO = Join-Path $RunDir "mixa_file_win32_oracle.o"
...
if ($occExit -ne 0) { Get-Content $occLog2; throw "oracle mixa_file_win32.lm1 compile failed" }
...
# ---- Step 4: attempt the COMPLETE mixa_file_win32.lm2 translation. ----
Push-Location $RepoRoot
$fwSrc = "mixa_manager\mixa_file_win32.lm2"
$fwOut = Join-Path $RunDir "mixa_file_win32_l2.lm1"
...
if ($FwExit -ne 0 -and $KnownBarrier) {
    ...
    Push-Location $RepoRoot
    $l2FwC = Join-Path $RunDir "mixa_file_win32_l2.c"
    $l2fcLog1 = Join-Path $RunDir "l2fw_trans_stdout.log"
    $l2fcLog2 = Join-Path $RunDir "l2fw_trans_stderr.log"
    $l2fcExit = Invoke-Cmd $L1Trans "`"$fwOut`" `"$l2FwC`"" $l2fcLog1 $l2fcLog2
    Pop-Location
    if ($l2fcExit -ne 0) {
        Get-Content $l2fcLog2
        $Verdict = "UNEXPECTED_FAILURE"
        $ExitCode = 1
    } else {
        $l2FwO = Join-Path $RunDir "mixa_file_win32_l2.o"
        ...
        if ($l2occExit -ne 0) {
            ...
            $l2olExit = Invoke-Cmd "gcc" "$GccStd -I `"$RepoRoot`" -I `"$RunDir\headers`" `"$harnessO`" `"$l2FwO`" -o `"$l2Exe`" $LinkLibs" $l2olLog1 $l2olLog2
            if ($l2olExit -ne 0) {
...
FileWin32-L2-Header: mixa_manager\mixa_file_win32_l2.h.lm1 (self-contained, no chain)
FileWin32-L2-Source: $fwSrc
FileWin32-L2-Source-Sha256: $FwSourceHash
Harness-Source-Sha256: $((Get-FileHash -LiteralPath (Join-Path $RepoRoot "mixa_manager\tests\mixa_file_win32_parity_harness.lm1") -Algorithm SHA256).Hash)
Runner-Sha256: $RunnerHash
Oracle-Run-Exit-Code: $oracleRunExit
Oracle-Trace:
$OracleTrace
FileWin32-Translate-Exit-Code: $FwExit
FileWin32-Translate-Stdout:
$FwStdoutText
FileWin32-Translate-Stderr:
$FwStderrText
L2-Trace:
$L2TraceText
...
switch ($Verdict) {
    "EXPECTED_CORE_BARRIER" {
        "EXPECTED_CORE_BARRIER: full mixa_file_win32.lm2 translation stopped at an already-known barrier. This is NOT a pass -- the integrated frontend is not yet on main. The oracle-side harness DID run (see Oracle-Trace above)."
    }
    "UNEXPECTED_FAILURE" {
    ...
    "PASS" {
        "PASS: full mixa_file_win32.lm2 translated, built, and ran; its trace is byte-identical to the L1 oracle's own trace."
    }
}
