# Editable command-line selftest runner (ticket 20260913-002013): the
# in-memory pending-line editing model (insert/backspace/delete/cursor
# movement/extract-to-cursor/by-character wrap arithmetic). Pure
# in-process checks: no file, no process, no window. No user files.
...
$RunDir = ""
$LogDir = ""
$TestSource = Join-Path $RepoRoot "mixa_manager\tests\mixa_cmdline_selftest.lm1"
$TransOut = ""
$ExeOut = ""
$TransStdout = ""
$TransStderr = ""
...
$BaseDir = Join-Path $RepoRoot "build\mixa\claude\cmdline"
...
try {
    ...
    $TransOut = Join-Path $RunDir "mixa_cmdline_selftest.c"
    $ExeOut = Join-Path $RunDir "mixa_cmdline_selftest.exe"
