# Real L1-vs-L2 parity gate for mixa_draw (ticket 20260913-102000).
# Same structure as run_mixa_text_rect_l2_parity.ps1 (100000). Reuses
# mixa_tiles_l2.h.lm1 and its ABI probe pair directly (per this ticket's
# own instruction 4).
#   0. ABI parity gate: reuses mixa_tiles_abi_probe_real.c/_l2.c.
#   1. ALWAYS builds and runs the ORACLE-side harness.
#   2. Attempts to translate the COMPLETE mixa_draw.lm2.
...
#        L2 draw object + the harness + the REAL, UNCHANGED
#        mixa_text_rect.o for mixa_rect_open/release/mixa_cell_at's own
#        real bodies -- the oracle side gets its own copy for free since
#        mixa_draw.lm1 predefs mixa_text_rect.lm1 directly) and diffs
#        its stdout against the oracle trace byte-for-byte. PASS (exit
#        0) only on an exact match, else PARITY_FAILURE (exit 1).
#
# mixa_draw.h/mixa_draw.lm1/mixa_core.h/mixa_text_rect.lm1 are the
...
$BaseDir = Join-Path $RepoRoot "build\mixa\claude\mixa_draw_l2_parity"
...
# ---- Step 2: build the harness object + the real mixa_text_rect.o
# (linked in for mixa_rect_open/release/mixa_cell_at's own real bodies
# on the L2 side). ----
Push-Location $RepoRoot
...
$hExit = Invoke-Cmd $L1Trans "mixa_manager\tests\mixa_draw_parity_harness.lm1 `"$harnessC`"" $hLog1 $hLog2
Pop-Location
...
$oracleC = Join-Path $RunDir "mixa_draw_oracle.c"
$ocLog1 = Join-Path $RunDir "oracle_trans_stdout.log"
$ocLog2 = Join-Path $RunDir "oracle_trans_stderr.log"
Push-Location $RepoRoot
$ocExit = Invoke-Cmd $L1Trans "mixa_manager\mixa_draw.lm1 `"$oracleC`"" $ocLog1 $ocLog2
Pop-Location
if ($ocExit -ne 0) { Get-Content $ocLog2; throw "oracle mixa_draw.lm1 translation failed" }

$oracleO = Join-Path $RunDir "mixa_draw_oracle.o"
...
if ($occExit -ne 0) { Get-Content $occLog2; throw "oracle mixa_draw.lm1 compile failed" }
...
# ---- Step 4: attempt the COMPLETE mixa_draw.lm2 translation. ----
Push-Location $RepoRoot
$drawSrc = "mixa_manager\mixa_draw.lm2"
$drawOut = Join-Path $RunDir "mixa_draw_l2.lm1"
$drawStdout = Join-Path $RunDir "draw_stdout.log"
$drawStderr = Join-Path $RunDir "draw_stderr.log"
$DrawExit = Invoke-Cmd "`"$l2exe`"" "`"$drawSrc`" `"$drawOut`"" $drawStdout $drawStderr
Pop-Location

$DrawStdoutText = if (Test-Path -LiteralPath $drawStdout) { Get-Content -LiteralPath $drawStdout -Raw } else { "" }
$DrawStderrText = if (Test-Path -LiteralPath $drawStderr) { Get-Content -LiteralPath $drawStderr -Raw } else { "" }
$DrawSourceHash = (Get-FileHash -LiteralPath (Join-Path $RepoRoot $drawSrc) -Algorithm SHA256).Hash
...
$KnownBarrier = ($DrawStderrText -match "unknown foreign type") -or ($DrawStderrText -match "incompatible entry signature")

if ($DrawExit -ne 0 -and $KnownBarrier) {
    $Verdict = "EXPECTED_CORE_BARRIER"
    $ExitCode = 2
} elseif ($DrawExit -ne 0) {
    $Verdict = "UNEXPECTED_FAILURE"
    $ExitCode = 1
} else {
    Push-Location $RepoRoot
    $l2DrawC = Join-Path $RunDir "mixa_draw_l2.c"
    $l2dcLog1 = Join-Path $RunDir "l2draw_trans_stdout.log"
    $l2dcLog2 = Join-Path $RunDir "l2draw_trans_stderr.log"
    $l2dcExit = Invoke-Cmd $L1Trans "`"$drawOut`" `"$l2DrawC`"" $l2dcLog1 $l2dcLog2
    Pop-Location
    if ($l2dcExit -ne 0) {
        Get-Content $l2dcLog2
        $Verdict = "UNEXPECTED_FAILURE"
        $ExitCode = 1
    } else {
        $l2DrawO = Join-Path $RunDir "mixa_draw_l2.o"
        $l2occLog1 = Join-Path $RunDir "l2draw_compile_stdout.log"
        $l2occLog2 = Join-Path $RunDir "l2draw_compile_stderr.log"
        $l2occExit = Invoke-Cmd "gcc" "$GccStd -I `"$RepoRoot`" -I `"$RunDir\headers`" -c `"$l2DrawC`" -o `"$l2DrawO`"" $l2occLog1 $l2occLog2
        if ($l2occExit -ne 0) {
            ...
            $l2olLog1 = Join-Path $RunDir "l2draw_link_stdout.log"
            $l2olLog2 = Join-Path $RunDir "l2draw_link_stderr.log"
            $l2olExit = Invoke-Cmd "gcc" "$GccStd -I `"$RepoRoot`" -I `"$RunDir\headers`" `"$harnessO`" `"$l2DrawO`" `"$textRectO`" -o `"$l2Exe`"" $l2olLog1 $l2olLog2
            if ($l2olExit -ne 0) {
...
Draw-L2-Header (reused from 414f7a7f): mixa_manager\mixa_tiles_l2.h.lm1
Draw-L2-Source: $drawSrc
Draw-L2-Source-Sha256: $DrawSourceHash
Harness-Source-Sha256: $((Get-FileHash -LiteralPath (Join-Path $RepoRoot "mixa_manager\tests\mixa_draw_parity_harness.lm1") -Algorithm SHA256).Hash)
Runner-Sha256: $RunnerHash
Oracle-Run-Exit-Code: $oracleRunExit
Oracle-Trace:
$OracleTrace
Draw-Translate-Exit-Code: $DrawExit
Draw-Translate-Stdout:
$DrawStdoutText
Draw-Translate-Stderr:
$DrawStderrText
L2-Trace:
$L2TraceText
...
switch ($Verdict) {
    "EXPECTED_CORE_BARRIER" {
        "EXPECTED_CORE_BARRIER: full mixa_draw.lm2 translation stopped at an already-known barrier. This is NOT a pass -- the integrated frontend is not yet on main. The oracle-side harness DID run (see Oracle-Trace above)."
    }
    "UNEXPECTED_FAILURE" {
    ...
    "PASS" {
        "PASS: full mixa_draw.lm2 translated, built, and ran; its trace is byte-identical to the L1 oracle's own trace."
    }
}
