...
# entrypoint uses -- through the existing headless backend with a real
# temporary fixture directory, exercising mixa_fm_is_selected/mixa_fm_
# select and mixa_fm_refresh directly against the real, already-accepted
# (unmodified) mixa_selection module. No window opens anywhere in this
# run; this is a real, executed test, not a build-only check. Fixtures
# live under this run's own directory. No user files.
...
$RunDir = ""
$LogDir = ""
$TestSource = Join-Path $RepoRoot "mixa_manager\tests\mixa_app_fmpanel_selrefresh_e2e_selftest.lm1"
...
$BaseDir = Join-Path $RepoRoot "build\mixa\claude\app_fmpanel_selrefresh_e2e"
...
try {
    ...
    $TransOut = Join-Path $RunDir "mixa_app_fmpanel_selrefresh_e2e_selftest.c"
    $TestObj = Join-Path $RunDir "mixa_app_fmpanel_selrefresh_e2e_selftest.o"
    $ExeOut = Join-Path $RunDir "mixa_app_fmpanel_selrefresh_e2e_selftest.exe"
