# Real L1-vs-L2 parity gate for mixa_process_marker (ticket
# 20260913-115500). Same structure as run_mixa_file_win32_l2_parity.ps1
# (114500): fully self-contained, own dedicated ABI probe pair (mixa_
# process_marker_abi_probe_real.c/_l2.c -- note both sides are
# generated via l1trans, since this module has no separate plain-C
# production header, unlike mixa_file_win32/mixa_tiles), no other
# manager module linked. Needs -lkernel32 (QueryPerformanceCounter/
# CreateFileW/WriteFile/CloseHandle/DeleteFileW/GetLastError/
# GetTempPathW/GetCurrentProcessId/MultiByteToWideChar/
# WideCharToMultiByte).
#   0. ABI parity gate: dedicated probe pair for all 4 public structs
#      (FaultVTable, Gen, Sink, Scanner).
#   1. ALWAYS builds and runs the ORACLE-side harness (real mixa_
#      process_marker.lm1, unmodified).
#   2. Attempts to translate the COMPLETE mixa_process_marker.lm2.
...
#
# mixa_process_marker.h.lm1/mixa_process_marker.lm1 are the parity
...
$BaseDir = Join-Path $RepoRoot "build\mixa\claude\mixa_process_marker_l2_parity"
...
# ---- Step 0: translate BOTH header units (real, for the ABI probe's
# own real side, and the L2 header). ----
Push-Location $RepoRoot
$hdrLog1 = Join-Path $RunDir "header_trans_stdout.log"
$hdrLog2 = Join-Path $RunDir "header_trans_stderr.log"
$realHdrOut = Join-Path $HeaderDir "mixa_process_marker.lm1.h"
$hdrExit1 = Invoke-Cmd $L1Trans "mixa_manager\mixa_process_marker.h.lm1 `"$realHdrOut`"" $hdrLog1 $hdrLog2
if ($hdrExit1 -ne 0) { Pop-Location; Get-Content $hdrLog2; throw "real process_marker header translation failed" }
$l2HdrOut = Join-Path $HeaderDir "mixa_process_marker_l2.lm1.h"
$hdrExit2 = Invoke-Cmd $L1Trans "mixa_manager\mixa_process_marker_l2.h.lm1 `"$l2HdrOut`"" $hdrLog1 $hdrLog2
Pop-Location
if ($hdrExit2 -ne 0) { Get-Content $hdrLog2; throw "L2 process_marker header translation failed" }

# ---- Step 0.5: ABI parity gate (dedicated 4-struct probe pair). ----
...
$abiRealExit = Invoke-Cmd "gcc" "$GccStd -I `"$RunDir\headers`" `"$RepoRoot\mixa_manager\tests\mixa_process_marker_abi_probe_real.c`" -o `"$abiRealExe`"" $abiRealLog1 $abiRealLog2
...
$abiL2Exit = Invoke-Cmd "gcc" "$GccStd -I `"$RunDir\headers`" `"$RepoRoot\mixa_manager\tests\mixa_process_marker_abi_probe_l2.c`" -o `"$abiL2Exe`"" $abiL2Log1 $abiL2Log2
...
$hExit = Invoke-Cmd $L1Trans "mixa_manager\tests\mixa_process_marker_parity_harness.lm1 `"$harnessC`"" $hLog1 $hLog2
Pop-Location
...
$oracleC = Join-Path $RunDir "mixa_process_marker_oracle.c"
$ocLog1 = Join-Path $RunDir "oracle_trans_stdout.log"
$ocLog2 = Join-Path $RunDir "oracle_trans_stderr.log"
Push-Location $RepoRoot
$ocExit = Invoke-Cmd $L1Trans "mixa_manager\mixa_process_marker.lm1 `"$oracleC`"" $ocLog1 $ocLog2
Pop-Location
if ($ocExit -ne 0) { Get-Content $ocLog2; throw "oracle mixa_process_marker.lm1 translation failed" }

$oracleO = Join-Path $RunDir "mixa_process_marker_oracle.o"
...
if ($occExit -ne 0) { Get-Content $occLog2; throw "oracle mixa_process_marker.lm1 compile failed" }
...
# ---- Step 4: attempt the COMPLETE mixa_process_marker.lm2 translation. ----
Push-Location $RepoRoot
$pmSrc = "mixa_manager\mixa_process_marker.lm2"
$pmOut = Join-Path $RunDir "mixa_process_marker_l2.lm1"
$pmStdout = Join-Path $RunDir "pm_stdout.log"
$pmStderr = Join-Path $RunDir "pm_stderr.log"
$PmExit = Invoke-Cmd "`"$l2exe`"" "`"$pmSrc`" `"$pmOut`"" $pmStdout $pmStderr
Pop-Location

$PmStdoutText = if (Test-Path -LiteralPath $pmStdout) { Get-Content -LiteralPath $pmStdout -Raw } else { "" }
$PmStderrText = if (Test-Path -LiteralPath $pmStderr) { Get-Content -LiteralPath $pmStderr -Raw } else { "" }
$PmSourceHash = (Get-FileHash -LiteralPath (Join-Path $RepoRoot $pmSrc) -Algorithm SHA256).Hash
...
$KnownBarrier = ($PmStderrText -match "unknown foreign type") -or ($PmStderrText -match "incompatible entry signature")

if ($PmExit -ne 0 -and $KnownBarrier) {
    ...
    Push-Location $RepoRoot
    $l2PmC = Join-Path $RunDir "mixa_process_marker_l2.c"
    $l2pcLog1 = Join-Path $RunDir "l2pm_trans_stdout.log"
    $l2pcLog2 = Join-Path $RunDir "l2pm_trans_stderr.log"
    $l2pcExit = Invoke-Cmd $L1Trans "`"$pmOut`" `"$l2PmC`"" $l2pcLog1 $l2pcLog2
    Pop-Location
    if ($l2pcExit -ne 0) {
        Get-Content $l2pcLog2
        $Verdict = "UNEXPECTED_FAILURE"
        $ExitCode = 1
    } else {
        $l2PmO = Join-Path $RunDir "mixa_process_marker_l2.o"
        $l2occLog1 = Join-Path $RunDir "l2pm_compile_stdout.log"
        $l2occLog2 = Join-Path $RunDir "l2pm_compile_stderr.log"
        $l2occExit = Invoke-Cmd "gcc" "$GccStd -I `"$RepoRoot`" -I `"$RunDir\headers`" -c `"$l2PmC`" -o `"$l2PmO`"" $l2occLog1 $l2occLog2
        if ($l2occExit -ne 0) {
            ...
            $l2olLog1 = Join-Path $RunDir "l2pm_link_stdout.log"
            $l2olLog2 = Join-Path $RunDir "l2pm_link_stderr.log"
            $l2olExit = Invoke-Cmd "gcc" "$GccStd -I `"$RepoRoot`" -I `"$RunDir\headers`" `"$harnessO`" `"$l2PmO`" -o `"$l2Exe`" $LinkLibs" $l2olLog1 $l2olLog2
            if ($l2olExit -ne 0) {
...
ProcessMarker-L2-Header: mixa_manager\mixa_process_marker_l2.h.lm1 (self-contained, no chain)
ProcessMarker-L2-Source: $pmSrc
ProcessMarker-L2-Source-Sha256: $PmSourceHash
Harness-Source-Sha256: $((Get-FileHash -LiteralPath (Join-Path $RepoRoot "mixa_manager\tests\mixa_process_marker_parity_harness.lm1") -Algorithm SHA256).Hash)
Runner-Sha256: $RunnerHash
Oracle-Run-Exit-Code: $oracleRunExit
Oracle-Trace:
$OracleTrace
ProcessMarker-Translate-Exit-Code: $PmExit
ProcessMarker-Translate-Stdout:
$PmStdoutText
ProcessMarker-Translate-Stderr:
$PmStderrText
L2-Trace:
$L2TraceText
...
switch ($Verdict) {
    "EXPECTED_CORE_BARRIER" {
        "EXPECTED_CORE_BARRIER: full mixa_process_marker.lm2 translation stopped at an already-known barrier. This is NOT a pass -- the integrated frontend is not yet on main. The oracle-side harness DID run (see Oracle-Trace above)."
    }
    "UNEXPECTED_FAILURE" {
    ...
    "PASS" {
        "PASS: full mixa_process_marker.lm2 translated, built, and ran; its trace is byte-identical to the L1 oracle's own trace."
    }
}
