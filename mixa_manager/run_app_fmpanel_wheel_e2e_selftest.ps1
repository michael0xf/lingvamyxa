...
# entrypoint uses, now also owning MIXA_EVENT_MOUSE_WHEEL routing to
# mixa_app_fmpanel_wheel -- through the existing headless backend with a
...
$RunDir = ""
$LogDir = ""
$TestSource = Join-Path $RepoRoot "mixa_manager\tests\mixa_app_fmpanel_wheel_e2e_selftest.lm1"
...
$BaseDir = Join-Path $RepoRoot "build\mixa\claude\app_fmpanel_wheel_e2e"
...
try {
    ...
    $TransOut = Join-Path $RunDir "mixa_app_fmpanel_wheel_e2e_selftest.c"
    $TestObj = Join-Path $RunDir "mixa_app_fmpanel_wheel_e2e_selftest.o"
    $ExeOut = Join-Path $RunDir "mixa_app_fmpanel_wheel_e2e_selftest.exe"
