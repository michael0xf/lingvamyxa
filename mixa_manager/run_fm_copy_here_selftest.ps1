# Copy Here selftest runner (ticket 20260912-122500): the accepted
# mixa_fm_copy_selection exposed as a real button-shaped action
# (mixa_fm_copy_here_action) whose destination is always the file
# manager's own current directory, triggered through a real,
# dependency-free rectangle hit-test standing in for the accepted
# MixaButtonPanel primitive (unaffordable in this combination -- see
# mixa_fm_copy.h.lm1's own comment for the exact reproducer). Includes a
# poisoning-allocator regression that PROVES the path-lifetime hazard the
# ticket asked about is real, not merely documented. Fixtures live under
# this run's own directory and are rebuilt from scratch each run; every
...
$RunDir = ""
$LogDir = ""
$TestSource = Join-Path $RepoRoot "mixa_manager\tests\mixa_fm_copy_here_selftest.lm1"
$TransOut = ""
$ExeOut = ""
$TransStdout = ""
$TransStderr = ""
...
$BaseDir = Join-Path $RepoRoot "build\mixa\claude\fm_copy_here"
...
try {
    ...
    # all undefined unless linked in too, even though this suite (button-
    # trigger scenarios) never calls any of it. Same minimal headless-only
    # stack run_mixa.ps1 links for mixa_pump_selftest, translated
    # separately here and passed into the same single gcc invocation below.
    $PumpUnits = @(
    ...
    $ExeOut = Join-Path $RunDir "mixa_fm_copy_here_selftest.exe"
