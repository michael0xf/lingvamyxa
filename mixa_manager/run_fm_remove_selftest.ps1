# Remove-into-the-view integration selftest runner (ticket 20260913-
# 021200): mixa_remove_selected_run driven as a real user-triggered
# action through the file-manager view's own borrowed MixaSelection, with
# the view refreshed unconditionally after every run (see mixa_fm_remove.
# h.lm1's own comment for why this differs from copy's conditional
# refresh). Fixtures live under this run's own directory and are rebuilt
# from scratch each run; every mutation this test performs stays inside
# that fixture root, verified before any write. No user files.
...
$RunDir = ""
$LogDir = ""
$TestSource = Join-Path $RepoRoot "mixa_manager\tests\mixa_fm_remove_selftest.lm1"
$TransOut = ""
$ExeOut = ""
$TransStdout = ""
$TransStderr = ""
...
$BaseDir = Join-Path $RepoRoot "build\mixa\claude\fm_remove"
...
try {
    ...
    $TransOut = Join-Path $RunDir "mixa_fm_remove_selftest.c"
    $ExeOut = Join-Path $RunDir "mixa_fm_remove_selftest.exe"
