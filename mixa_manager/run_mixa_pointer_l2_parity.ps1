# Real L1-vs-L2 parity gate for mixa_pointer (ticket 20260913-113000).
# Same structure as run_mixa_highlight_l2_parity.ps1 (104000), but fully
# self-contained: MixaPointer has no dependency on MixaCell/MixaTextRect
# or any other manager struct, so this runner uses its OWN dedicated ABI
# probe pair (mixa_pointer_abi_probe_real.c/_l2.c) rather than reusing
# the tiles probes, and links no other manager module's object file.
#   0. ABI parity gate: dedicated MixaPointer sizeof/offsetof probe pair.
#   1. ALWAYS builds and runs the ORACLE-side harness (real mixa_
#      pointer.lm1, unmodified).
#   2. Attempts to translate the COMPLETE mixa_pointer.lm2.
...
#
# mixa_pointer.h/mixa_pointer.lm1 are the parity oracle and are never
...
$BaseDir = Join-Path $RepoRoot "build\mixa\claude\mixa_pointer_l2_parity"
...
$ptrHdrOut = Join-Path $HeaderDir "mixa_pointer_l2.lm1.h"
$hdrExit = Invoke-Cmd $L1Trans "mixa_manager\mixa_pointer_l2.h.lm1 `"$ptrHdrOut`"" $hdrLog1 $hdrLog2
Pop-Location
if ($hdrExit -ne 0) { Get-Content $hdrLog2; throw "pointer header translation failed" }

# ---- Step 0.5: ABI parity gate (dedicated MixaPointer probe pair). ----
...
$abiRealExit = Invoke-Cmd "gcc" "$GccStd -I `"$RepoRoot`" `"$RepoRoot\mixa_manager\tests\mixa_pointer_abi_probe_real.c`" -o `"$abiRealExe`"" $abiRealLog1 $abiRealLog2
...
$abiL2Exit = Invoke-Cmd "gcc" "$GccStd -I `"$RunDir\headers`" `"$RepoRoot\mixa_manager\tests\mixa_pointer_abi_probe_l2.c`" -o `"$abiL2Exe`"" $abiL2Log1 $abiL2Log2
...
$hExit = Invoke-Cmd $L1Trans "mixa_manager\tests\mixa_pointer_parity_harness.lm1 `"$harnessC`"" $hLog1 $hLog2
Pop-Location
...
$oracleC = Join-Path $RunDir "mixa_pointer_oracle.c"
$ocLog1 = Join-Path $RunDir "oracle_trans_stdout.log"
$ocLog2 = Join-Path $RunDir "oracle_trans_stderr.log"
Push-Location $RepoRoot
$ocExit = Invoke-Cmd $L1Trans "mixa_manager\mixa_pointer.lm1 `"$oracleC`"" $ocLog1 $ocLog2
Pop-Location
if ($ocExit -ne 0) { Get-Content $ocLog2; throw "oracle mixa_pointer.lm1 translation failed" }

$oracleO = Join-Path $RunDir "mixa_pointer_oracle.o"
...
if ($occExit -ne 0) { Get-Content $occLog2; throw "oracle mixa_pointer.lm1 compile failed" }
...
# ---- Step 4: attempt the COMPLETE mixa_pointer.lm2 translation. ----
Push-Location $RepoRoot
$ptrSrc = "mixa_manager\mixa_pointer.lm2"
$ptrOut = Join-Path $RunDir "mixa_pointer_l2.lm1"
$ptrStdout = Join-Path $RunDir "ptr_stdout.log"
$ptrStderr = Join-Path $RunDir "ptr_stderr.log"
$PtrExit = Invoke-Cmd "`"$l2exe`"" "`"$ptrSrc`" `"$ptrOut`"" $ptrStdout $ptrStderr
Pop-Location

$PtrStdoutText = if (Test-Path -LiteralPath $ptrStdout) { Get-Content -LiteralPath $ptrStdout -Raw } else { "" }
$PtrStderrText = if (Test-Path -LiteralPath $ptrStderr) { Get-Content -LiteralPath $ptrStderr -Raw } else { "" }
$PtrSourceHash = (Get-FileHash -LiteralPath (Join-Path $RepoRoot $ptrSrc) -Algorithm SHA256).Hash
...
$KnownBarrier = ($PtrStderrText -match "unknown foreign type") -or ($PtrStderrText -match "incompatible entry signature")

if ($PtrExit -ne 0 -and $KnownBarrier) {
    ...
    Push-Location $RepoRoot
    $l2PtrC = Join-Path $RunDir "mixa_pointer_l2.c"
    $l2pcLog1 = Join-Path $RunDir "l2ptr_trans_stdout.log"
    $l2pcLog2 = Join-Path $RunDir "l2ptr_trans_stderr.log"
    $l2pcExit = Invoke-Cmd $L1Trans "`"$ptrOut`" `"$l2PtrC`"" $l2pcLog1 $l2pcLog2
    Pop-Location
    if ($l2pcExit -ne 0) {
        ...
        $l2PtrO = Join-Path $RunDir "mixa_pointer_l2.o"
        $l2occLog1 = Join-Path $RunDir "l2ptr_compile_stdout.log"
        $l2occLog2 = Join-Path $RunDir "l2ptr_compile_stderr.log"
        $l2occExit = Invoke-Cmd "gcc" "$GccStd -I `"$RepoRoot`" -I `"$RunDir\headers`" -c `"$l2PtrC`" -o `"$l2PtrO`"" $l2occLog1 $l2occLog2
        if ($l2occExit -ne 0) {
            ...
            $l2olLog1 = Join-Path $RunDir "l2ptr_link_stdout.log"
            $l2olLog2 = Join-Path $RunDir "l2ptr_link_stderr.log"
            $l2olExit = Invoke-Cmd "gcc" "$GccStd -I `"$RepoRoot`" -I `"$RunDir\headers`" `"$harnessO`" `"$l2PtrO`" -o `"$l2Exe`"" $l2olLog1 $l2olLog2
            if ($l2olExit -ne 0) {
...
Pointer-L2-Header: mixa_manager\mixa_pointer_l2.h.lm1 (self-contained, no chain)
Pointer-L2-Source: $ptrSrc
Pointer-L2-Source-Sha256: $PtrSourceHash
Harness-Source-Sha256: $((Get-FileHash -LiteralPath (Join-Path $RepoRoot "mixa_manager\tests\mixa_pointer_parity_harness.lm1") -Algorithm SHA256).Hash)
Runner-Sha256: $RunnerHash
Oracle-Run-Exit-Code: $oracleRunExit
Oracle-Trace:
$OracleTrace
Pointer-Translate-Exit-Code: $PtrExit
Pointer-Translate-Stdout:
$PtrStdoutText
Pointer-Translate-Stderr:
$PtrStderrText
L2-Trace:
$L2TraceText
...
switch ($Verdict) {
    "EXPECTED_CORE_BARRIER" {
        "EXPECTED_CORE_BARRIER: full mixa_pointer.lm2 translation stopped at an already-known barrier. This is NOT a pass -- the integrated frontend is not yet on main. The oracle-side harness DID run (see Oracle-Trace above)."
    }
    "UNEXPECTED_FAILURE" {
    ...
    "PASS" {
        "PASS: full mixa_pointer.lm2 translated, built, and ran; its trace is byte-identical to the L1 oracle's own trace."
    }
}
