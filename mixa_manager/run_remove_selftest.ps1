# Remove-selected operation selftest runner (ticket 20260913-021200):
# recursive delete driven over the accepted MixaSelection through
# mixa_selection_walk, files deleted on_file and now-empty directories
# deleted on_dir_done (postorder by construction -- see mixa_remove.h.
# lm1's own comment). Fixtures live under this run's own directory and
# are rebuilt from scratch each run; every mutation this test performs
# stays inside that fixture root, verified before any write. No user
# files.
...
$RunDir = ""
$LogDir = ""
$TestSource = Join-Path $RepoRoot "mixa_manager\tests\mixa_remove_selftest.lm1"
$TransOut = ""
$ExeOut = ""
$TransStdout = ""
$TransStderr = ""
...
$BaseDir = Join-Path $RepoRoot "build\mixa\claude\remove"
...
try {
    ...
    $TransOut = Join-Path $RunDir "mixa_remove_selftest.c"
    $ExeOut = Join-Path $RunDir "mixa_remove_selftest.exe"
