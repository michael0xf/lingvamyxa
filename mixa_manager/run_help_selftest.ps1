# F1 Help selftest runner (ticket 20260912-235847): FIRST_VERSION's own
# "one button on it: F1 help, which writes its text into the console,
# ending with a newline" milestone. Pure headless coverage: a real
# file-backed console via the accepted mixa_file seam, no window, no
# backend, no process. Fixtures live under this run's own directory. No
# user files.
...
$RunDir = ""
$LogDir = ""
$TestSource = Join-Path $RepoRoot "mixa_manager\tests\mixa_help_selftest.lm1"
$TransOut = ""
$ExeOut = ""
$TransStdout = ""
$TransStderr = ""
...
$BaseDir = Join-Path $RepoRoot "build\mixa\claude\help"
...
try {
    ...
    $TransOut = Join-Path $RunDir "mixa_help_selftest.c"
    $ExeOut = Join-Path $RunDir "mixa_help_selftest.exe"
