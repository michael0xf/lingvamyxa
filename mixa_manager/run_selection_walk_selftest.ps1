# Lazy selected-tree walk selftest runner (ticket 20260912-075500): real,
# test-owned directory trees walked from the accepted MixaSelection
# through the accepted mixa_dir seam, with an injected allocator proving
# out-of-memory refusal, exact disposal and laziness at every owned
# allocation step. Fixtures live under this run's own directory and are
# rebuilt from scratch each run, so no scenario depends on leftovers from
# a previous one. The walk only reads directories and calls back; nothing
# outside those fixtures is touched.
...
$RunDir = ""
$LogDir = ""
$TestSource = Join-Path $RepoRoot "mixa_manager\tests\mixa_selection_walk_selftest.lm1"
$TransOut = ""
$ExeOut = ""
$TransStdout = ""
$TransStderr = ""
...
$BaseDir = Join-Path $RepoRoot "build\mixa\claude\selection_walk"
...
try {
    ...
    $TransOut = Join-Path $RunDir "mixa_selection_walk_selftest.c"
    $ExeOut = Join-Path $RunDir "mixa_selection_walk_selftest.exe"
