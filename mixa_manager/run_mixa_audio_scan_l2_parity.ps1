# Real L1-vs-L2 parity gate for mixa_audio_scan (ticket 20260913-232000/
# 231500 Part 2, module 3 of 8). The real mixa_audio_scan.lm1 predefs
# BOTH mixa_dir_win32.lm1 and mixa_audio.lm1's full bodies directly
# (embedded into the same translation unit, same pattern as
# mixa_app_panel predefing mixa_app_window's full body) -- so neither
# side of this harness needs a separate real dependency object; each
# side's own single translated .c already contains everything.
#
#   1. ALWAYS builds and runs the ORACLE-side harness (real, test-owned
#      on-disk fixtures; enumeration proven against an independent
#      oracle walk of the same real directory).
#   2. Attempts to translate the COMPLETE mixa_audio_scan.lm2.
...
#
# mixa_audio_scan.lm1/.h.lm1 and the real, unmodified mixa_dir_win32.lm1
# and mixa_audio.lm1 (embedded via predef) are the parity oracle and are
...
$BaseDir = Join-Path $RepoRoot "build\mixa\claude\mixa_audio_scan_l2_parity"
...
Invoke-HeaderTrans "mixa_manager\mixa_audio_scan.h.lm1" "mixa_audio_scan.lm1.h"
Invoke-HeaderTrans "mixa_manager\mixa_audio_scan_l2.h.lm1" "mixa_audio_scan_l2.lm1.h"
...
$hExit = Invoke-Cmd $L1Trans "mixa_manager\tests\mixa_audio_scan_parity_harness.lm1 `"$harnessC`"" $hLog1 $hLog2
Pop-Location
...
$oracleC = Join-Path $RunDir "mixa_audio_scan_oracle.c"
$ocLog1 = Join-Path $RunDir "oracle_trans_stdout.log"
$ocLog2 = Join-Path $RunDir "oracle_trans_stderr.log"
Push-Location $RepoRoot
$ocExit = Invoke-Cmd $L1Trans "mixa_manager\mixa_audio_scan.lm1 `"$oracleC`"" $ocLog1 $ocLog2
Pop-Location
if ($ocExit -ne 0) { Get-Content $ocLog2; throw "oracle mixa_audio_scan.lm1 translation failed" }

$oracleO = Join-Path $RunDir "mixa_audio_scan_oracle.o"
...
if ($occExit -ne 0) { Get-Content $occLog2; throw "oracle mixa_audio_scan.lm1 compile failed" }
...
# ---- Step 4: attempt the COMPLETE mixa_audio_scan.lm2 translation. ----
Push-Location $RepoRoot
$asSrc = "mixa_manager\mixa_audio_scan.lm2"
...
$AsStdoutText = if (Test-Path -LiteralPath $asStdout) { Get-Content -LiteralPath $asStdout -Raw } else { "" }
$AsStderrText = if (Test-Path -LiteralPath $asStderr) { Get-Content -LiteralPath $asStderr -Raw } else { "" }
$AsSourceHash = (Get-FileHash -LiteralPath (Join-Path $RepoRoot $asSrc) -Algorithm SHA256).Hash
...
$KnownBarrier = ($AsStderrText -match "unknown foreign type") -or ($AsStderrText -match "incompatible entry signature") -or ($AsStderrText -match "unsupported own array declaration")

if ($AsExit -ne 0 -and $KnownBarrier) {
    ...
    Push-Location $RepoRoot
    $l2AsC = Join-Path $RunDir "mixa_audio_scan_l2.c"
    $l2ccLog1 = Join-Path $RunDir "l2as_trans_stdout.log"
    $l2ccLog2 = Join-Path $RunDir "l2as_trans_stderr.log"
    $l2ccExit = Invoke-Cmd $L1Trans "`"$asOut`" `"$l2AsC`"" $l2ccLog1 $l2ccLog2
    Pop-Location
    if ($l2ccExit -ne 0) {
...
AudioScan-L2-Header: mixa_manager\mixa_audio_scan_l2.h.lm1
AudioScan-L2-Source: $asSrc
AudioScan-L2-Source-Sha256: $AsSourceHash
Harness-Source-Sha256: $((Get-FileHash -LiteralPath (Join-Path $RepoRoot "mixa_manager\tests\mixa_audio_scan_parity_harness.lm1") -Algorithm SHA256).Hash)
Runner-Sha256: $RunnerHash
Fixture-Directory: $FixtureDir
Oracle-Run-Exit-Code: $oracleRunExit
Oracle-Trace:
$OracleTrace
AudioScan-Translate-Exit-Code: $AsExit
AudioScan-Translate-Stdout:
$AsStdoutText
AudioScan-Translate-Stderr:
$AsStderrText
L2-Trace:
$L2TraceText
...
switch ($Verdict) {
    "EXPECTED_CORE_BARRIER" {
        "EXPECTED_CORE_BARRIER: full mixa_audio_scan.lm2 translation stopped at an already-known barrier. This is NOT a pass -- the integrated frontend is not yet on main. The oracle-side harness DID run (see Oracle-Trace above)."
    }
    "UNEXPECTED_FAILURE" {
    ...
    "PASS" {
        "PASS: full mixa_audio_scan.lm2 translated, built, and ran; its trace is byte-identical to the L1 oracle's own trace."
    }
}
