# Production file-manager row selection by click (ticket 20260913-
# 040213). Drives the SAME production orchestration -- mixa_app_
# controller.h/.lm1, the identical code the real Win32 entrypoint uses,
# now also owning mixa_app_fmpanel's own entry-row click handling --
# through the existing headless backend with a real temporary fixture
# directory. No window opens anywhere in this run; this is a real,
...
$RunDir = ""
$LogDir = ""
$TestSource = Join-Path $RepoRoot "mixa_manager\tests\mixa_app_fmpanel_select_e2e_selftest.lm1"
...
$BaseDir = Join-Path $RepoRoot "build\mixa\claude\app_fmpanel_select_e2e"
...
try {
    ...
    $ExeOut = Join-Path $RunDir "mixa_app_fmpanel_select_e2e_selftest.exe"
