# Audio seam (ClearShell MP3 button) selftest runner: portable controller +
# scriptable fake backend, deterministic, no platform audio libs required.
# Patterned from run_app_selftest.ps1 with the same compiler hash pin.
...
$RunDir = ""
$LogDir = ""
$TestSource = Join-Path $RepoRoot "mixa_manager\tests\mixa_audio_selftest.lm1"
$TransOut = ""
$ExeOut = ""
$TransStdout = ""
$TransStderr = ""
...
$BaseDir = Join-Path $RepoRoot "build\mixa\opencode\audio_seam"
...
try {
    ...
    # Prepare header directory structure and translate both headers (concrete
    # first: the portable header predefs it).
    ...
    # Translate test (predefs headers + controller; single TU, no win32 module).
    $TransOut = Join-Path $RunDir "mixa_audio_selftest.c"
    $ExeOut = Join-Path $RunDir "mixa_audio_selftest.exe"
    ...
    # Compile test (no extra libs: the fake backend is pure controller logic).
    $GCC = "gcc.exe"
