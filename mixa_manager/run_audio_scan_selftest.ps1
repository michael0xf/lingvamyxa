# MP3 stage 3 selftest runner, scanning half (ticket 20260912-065230):
# real directory-to-playlist feeding over real, test-owned on-disk
# fixtures, enumerated through the accepted mixa_dir seam and filtered
# through the accepted mixa_audio_is_audio_path, with an independent
# mixa_dir oracle proving observed order. Fixtures live under this run's
# own directory and are rebuilt from scratch each run, so a scan never
# depends on leftovers from a previous one.
...
$RunDir = ""
$LogDir = ""
$TestSource = Join-Path $RepoRoot "mixa_manager\tests\mixa_audio_scan_selftest.lm1"
$TransOut = ""
$ExeOut = ""
$TransStdout = ""
$TransStderr = ""
...
$BaseDir = Join-Path $RepoRoot "build\mixa\claude\audio_scan"
...
try {
    ...
    $TransOut = Join-Path $RunDir "mixa_audio_scan_selftest.c"
    $ExeOut = Join-Path $RunDir "mixa_audio_scan_selftest.exe"
