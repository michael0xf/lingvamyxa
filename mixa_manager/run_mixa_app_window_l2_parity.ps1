# Real L1-vs-L2 parity gate for mixa_app_window (ticket 20260913-
# 153000). Own dedicated ABI probe pair (both sides via l1trans-
# generated C headers, since neither the real nor the L2 header is
# hand-written plain C).
#
# Dependency-linking note: mixa_app_window.lm1 predefs the FULL bodies
# of mixa_buttons.lm1 (-> mixa_tiles.lm1) and mixa_draw.lm1 (-> mixa_
# text_rect.lm1) directly -- so the ORACLE side's own translation unit
# already embeds buttons/tiles/draw/text_rect and needs no separate
# objects for them. The L2 header instead predefs ONLY mixa_button_
# dispatch_l2.h.lm1 (header-only, no bodies), so the L2 side separately
# links the real, unmodified mixa_buttons.lm1 and mixa_draw.lm1 (which
# embed tiles/text_rect the same way) for those symbols.
#
#   0. ABI parity gate: dedicated MixaAppWindow/MixaAppWindowStack probe
#      pair.
#   1. ALWAYS builds and runs the ORACLE-side harness (real frame/
#      message/button rendering, real nested focus targeting, real
#      sentinel-preserving negative preflight, no depth cap).
#   2. Attempts to translate the COMPLETE mixa_app_window.lm2.
...
#
# mixa_app_window.h.lm1/.lm1 and every real dependency (mixa_buttons.h/
# .lm1, mixa_draw.h/.lm1, mixa_tiles.h/.lm1, mixa_text_rect.h/.lm1) are
# the parity oracle (or real, unmodified dependencies) and are never
...
$BaseDir = Join-Path $RepoRoot "build\mixa\claude\mixa_app_window_l2_parity"
...
# ---- Step 0: translate the real header + L2 header + their own
# transitive chains. ----
...
Invoke-HeaderTrans "mixa_manager\mixa_app_window_l2.h.lm1" "mixa_app_window_l2.lm1.h"
...
$abiRealExit = Invoke-Cmd "gcc" "$GccStd -I `"$RepoRoot`" -I `"$RunDir\headers`" `"$RepoRoot\mixa_manager\tests\mixa_app_window_abi_probe_real.c`" -o `"$abiRealExe`"" $abiRealLog1 $abiRealLog2
...
$abiL2Exit = Invoke-Cmd "gcc" "$GccStd -I `"$RepoRoot`" -I `"$RunDir\headers`" `"$RepoRoot\mixa_manager\tests\mixa_app_window_abi_probe_l2.c`" -o `"$abiL2Exe`"" $abiL2Log1 $abiL2Log2
...
$hExit = Invoke-Cmd $L1Trans "mixa_manager\tests\mixa_app_window_parity_harness.lm1 `"$harnessC`"" $hLog1 $hLog2
Pop-Location
...
# window.lm1's own translation unit already embeds buttons/tiles/draw/
# text_rect (see top comment) -- no separate real objects needed. ----
$oracleC = Join-Path $RunDir "mixa_app_window_oracle.c"
$ocLog1 = Join-Path $RunDir "oracle_trans_stdout.log"
$ocLog2 = Join-Path $RunDir "oracle_trans_stderr.log"
Push-Location $RepoRoot
$ocExit = Invoke-Cmd $L1Trans "mixa_manager\mixa_app_window.lm1 `"$oracleC`"" $ocLog1 $ocLog2
Pop-Location
if ($ocExit -ne 0) { Get-Content $ocLog2; throw "oracle mixa_app_window.lm1 translation failed" }

$oracleO = Join-Path $RunDir "mixa_app_window_oracle.o"
...
if ($occExit -ne 0) { Get-Content $occLog2; throw "oracle mixa_app_window.lm1 compile failed" }
...
# ---- Step 4: attempt the COMPLETE mixa_app_window.lm2 translation. ----
Push-Location $RepoRoot
$awSrc = "mixa_manager\mixa_app_window.lm2"
$awOut = Join-Path $RunDir "mixa_app_window_l2.lm1"
...
$KnownBarrier = ($AwStderrText -match "unknown foreign type") -or ($AwStderrText -match "incompatible entry signature")
...
if ($AwExit -ne 0 -and $KnownBarrier) {
    ...
    Push-Location $RepoRoot
    $l2AwC = Join-Path $RunDir "mixa_app_window_l2.c"
    ...
    if ($l2ccExit -ne 0) {
        ...
        $l2AwO = Join-Path $RunDir "mixa_app_window_l2.o"
        ...
        if ($l2occExit -ne 0) {
            ...
            $buttonsO = Build-RealDep "mixa_buttons" "mixa_manager\mixa_buttons.lm1"
            ...
            $l2olExit = Invoke-Cmd "gcc" "$GccStd -I `"$RepoRoot`" -I `"$RunDir\headers`" `"$harnessO`" `"$l2AwO`" `"$buttonsO`" `"$drawO`" -o `"$l2Exe`"" $l2olLog1 $l2olLog2
            if ($l2olExit -ne 0) {
...
AppWindow-L2-Header: mixa_manager\mixa_app_window_l2.h.lm1
AppWindow-L2-Source: $awSrc
AppWindow-L2-Source-Sha256: $AwSourceHash
Harness-Source-Sha256: $((Get-FileHash -LiteralPath (Join-Path $RepoRoot "mixa_manager\tests\mixa_app_window_parity_harness.lm1") -Algorithm SHA256).Hash)
Runner-Sha256: $RunnerHash
Oracle-Run-Exit-Code: $oracleRunExit
Oracle-Trace:
$OracleTrace
AppWindow-Translate-Exit-Code: $AwExit
AppWindow-Translate-Stdout:
$AwStdoutText
AppWindow-Translate-Stderr:
$AwStderrText
L2-Trace:
$L2TraceText
...
switch ($Verdict) {
    "EXPECTED_CORE_BARRIER" {
        "EXPECTED_CORE_BARRIER: full mixa_app_window.lm2 translation stopped at an already-known barrier. This is NOT a pass -- the integrated frontend is not yet on main. The oracle-side harness DID run (see Oracle-Trace above)."
    }
    "UNEXPECTED_FAILURE" {
    ...
    "PASS" {
        "PASS: full mixa_app_window.lm2 translated, built, and ran; its trace is byte-identical to the L1 oracle's own trace."
    }
}
