 ...
 * field offset of the REAL MixaAppController struct, as seen through
 * the genuine production header (mixa_manager/mixa_app_controller_
 * impl.h, a plain, hand-written C header). The runner compiles this
 * AND mixa_app_controller_abi_probe_l2.c (which probes the L2 header's
 * own l1trans-generated C header instead) and diffs their output
 * byte-for-byte before ever building or running the actual parity
 * harness.
 */
#include <stdio.h>
#include <stddef.h>
#include "mixa_manager/mixa_app_controller_impl.h"
