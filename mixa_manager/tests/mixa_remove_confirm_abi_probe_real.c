 ...
 * field offset of the REAL MixaRemoveConfirmCtx struct, as seen
 * through the real oracle header's own l1trans-generated C header
 * (mixa_remove_confirm.h.lm1 -> mixa_remove_confirm.lm1.h). The runner
 * compiles this AND mixa_remove_confirm_abi_probe_l2.c (which probes
 * the L2 header's own generated C header instead) and diffs their
 * output byte-for-byte before ever building or running the actual
 * parity harness.
 */
#include <stdio.h>
#include <stddef.h>
#include "mixa_manager/mixa_remove_confirm.lm1.h"
