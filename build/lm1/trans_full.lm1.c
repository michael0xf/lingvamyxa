...
static int lm_trans_trailer_single_atom(const LmP0Trailer *trailer, LmP0Text *out_text) {
    ...
    if (field == NULL || field -> next != NULL || field -> value == NULL || field -> value -> kind != LM_P0_NODE_ATOM) {
        return 0;
    }
...
static const char * lm_trans_symbol_kind_name(LmTransSymbolKind kind) {
    if (kind == LM_TRANS_SYMBOL_VARIABLE) {
        return "variable";
    }
    if (kind == LM_TRANS_SYMBOL_FUNCTION) {
        return "function";
    }
    if (kind == LM_TRANS_SYMBOL_PROCEDURE) {
        return "procedure";
    }
    if (kind == LM_TRANS_SYMBOL_LABEL) {
        return "label";
    }
    if (kind == LM_TRANS_SYMBOL_STRUCTURE) {
        return "structure";
    }
    return "symbol";
}
...
static void lm_trans_namespace_destroy(LmTransNamespace *namespace_) {
    ...
    if (namespace_ != NULL) {
        for (i = 0U; i < namespace_->count; ++i) {
            free(namespace_->items[i].param_names);
            namespace_->items[i].param_names = NULL;
            namespace_->items[i].param_count = 0U;
            namespace_->items[i].has_signature = 0;
        }
        free(namespace_->items);
        free(namespace_->cleanups);
        free(namespace_->loops);
        namespace_->items = NULL;
        namespace_->cleanups = NULL;
        namespace_->loops = NULL;
        namespace_->count = 0U;
        namespace_->capacity = 0U;
        namespace_->depth = 0U;
        namespace_->cleanup_count = 0U;
        namespace_->cleanup_capacity = 0U;
...
static void lm_trans_namespace_leave_scope(LmTransNamespace *namespace_) {
    ...
    while (
        namespace_->count > 0U &&
        namespace_->items[namespace_->count - 1U].depth == namespace_->depth
    ) {
        --namespace_->count;
    }
...
static int lm_trans_cleanup_push(LmTransNamespace *namespace_, unsigned id) {
    ...
    if (namespace_->cleanup_count == namespace_->cleanup_capacity) {
        capacity = namespace_->cleanup_capacity == 0U ? 8U : namespace_->cleanup_capacity * 2U;
        cleanups = (LmTransCleanup *)realloc(namespace_->cleanups, capacity * sizeof(*cleanups));
        if (cleanups == NULL) {
            return 1;
        }
        namespace_->cleanups = cleanups;
        namespace_->cleanup_capacity = capacity;
    }

    namespace_->cleanups[namespace_->cleanup_count].id = id;
...
static int lm_trans_loop_push(LmTransNamespace *namespace_) {
    ...
    if (namespace_->loop_count == namespace_->loop_capacity) {
        capacity = namespace_->loop_capacity == 0U ? 8U : namespace_->loop_capacity * 2U;
        loops = (LmTransLoop *)realloc(namespace_->loops, capacity * sizeof(*loops));
        if (loops == NULL) {
            return 1;
        }
        namespace_->loops = loops;
        namespace_->loop_capacity = capacity;
    }

    namespace_->loops[namespace_->loop_count].cleanup_base = namespace_->cleanup_count;
...
static size_t lm_trans_loop_cleanup_base(const LmTransNamespace *namespace_) {
    if (namespace_ == NULL || namespace_->loop_count == 0U) {
        return namespace_ != NULL ? namespace_->cleanup_count : 0U;
    }

    return namespace_->loops[namespace_->loop_count - 1U].cleanup_base;
}
...
static const LmTransSymbol *lm_trans_namespace_find(
    ...
    while (i > 0U) {
        --i;
        if (lm_trans_symbol_name_same(namespace_->items[i].name, name)) {
            return &namespace_->items[i];
        }
    }
...
static int lm_trans_namespace_declare(
    ...
    if (namespace_->count == namespace_->capacity) {
        capacity = namespace_->capacity == 0U ? 16U : namespace_->capacity * 2U;
        items = (LmTransSymbol *)realloc(namespace_->items, capacity * sizeof(*items));
        if (items == NULL) {
            return 1;
        }
        namespace_->items = items;
        namespace_->capacity = capacity;
    }

    namespace_->items[namespace_->count].name = name;
    namespace_->items[namespace_->count].kind = kind;
    namespace_->items[namespace_->count].depth = namespace_->depth;
    namespace_->items[namespace_->count].has_c_name = 0;
    namespace_->items[namespace_->count].c_name.data = NULL;
    namespace_->items[namespace_->count].c_name.length = 0U;
    namespace_->items[namespace_->count].param_names = NULL;
    namespace_->items[namespace_->count].param_count = 0U;
    namespace_->items[namespace_->count].has_signature = 0;
    ++namespace_->count;
    return 0;
}
...
static int lm_trans_namespace_bind_c_reference(
    ...
    if (symbol == NULL) {
        ...
        symbol = &namespace_->items[namespace_->count - 1U];
...
static int lm_trans_emit_call_args(
    ...
    if (callee != NULL && callee->has_signature) {
        ...
        if (callee->param_count > 0U) {
            ranges = (LmTransArgumentRange *)calloc(callee->param_count, sizeof(*ranges));
            if (ranges == NULL) {
                return 1;
            }
        }
        ...
        while (field != NULL) {
            ...
            if (is_named) {
                ...
                ranges[named_index].first = named_frame->body.first_field;
                ranges[named_index].stop = NULL;
                ranges[named_index].present = 1;
                field = field->next;
                continue;
            }
            ...
            ranges[index].first = field;
            ranges[index].stop = next;
            ranges[index].present = 1;
            ++index;
            field = next;
        }
...
static int lm_trans_emit_cleanups_until(
    ...
    if (namespace_ == NULL || cleanup_base > namespace_->cleanup_count) {
        return 0;
    }

    index = namespace_->cleanup_count;
    while (index > cleanup_base) {
        ...
        if (lm_trans_emit_sync_name(file, namespace_->cleanups[index].id) != 0) {
            return 1;
        }
...
static int lm_trans_emit_return_statement(
    ...
    if (namespace_ != NULL && namespace_->cleanup_count > 0U && namespace_->return_type_node != NULL) {
...
static int lm_trans_emit_else(
    ...
    lm_trans_namespace_enter_scope(namespace_);
    if (lm_trans_emit_statement_list(file, frame->body.first_field, indent + 1U, namespace_) != 0) {
        lm_trans_namespace_leave_scope(namespace_);
        return 1;
    }
    lm_trans_namespace_leave_scope(namespace_);
...
static int lm_trans_emit_statement(
    ...
    if (lm_trans_text_equals(node->as.frame.head, "return")) {
        status = lm_trans_emit_return_statement(file, node->as.frame.body.first_field, indent, namespace_);
    } else if (lm_trans_text_equals(node->as.frame.head, "if")) {
        status = lm_trans_emit_control(file, &node->as.frame, indent, namespace_, "if");
    ...
    } else if (lm_trans_text_equals(node->as.frame.head, "const")) {
        status = lm_trans_emit_const_statement(file, &node->as.frame, indent, namespace_);
    ...
    } else if (lm_trans_frame_looks_label_declaration(&node->as.frame)) {
        status = lm_trans_emit_label_statement(file, &node->as.frame, indent, namespace_);
    } else if (lm_trans_is_c_reference_name(node->as.frame.head)) {
        status = lm_trans_emit_call_statement(file, &node->as.frame, indent, namespace_);
    } else {
        symbol = lm_trans_namespace_find(namespace_, node->as.frame.head);
        if (symbol == NULL) {
            fprintf(
                stderr,
                "trans L2 error: unknown Lingvamyxa name \"%.*s\"; use c.%.*s for ANSI C namespace access\n",
        ...
        if (symbol->kind == LM_TRANS_SYMBOL_VARIABLE) {
            if (symbol->has_c_name) {
                ...
                status = lm_trans_emit_assignment(file, &node->as.frame, indent, namespace_);
            }
        } else if (
            symbol->kind == LM_TRANS_SYMBOL_FUNCTION ||
            symbol->kind == LM_TRANS_SYMBOL_PROCEDURE
    ...
    return lm_trans_emit_trailer_statement(file, node->as.frame.trailer, indent, namespace_);
}
...
static int lm_trans_emit_statement_list(
    ...
    if (
        ...
        field = first->value->as.structure.first_field;
    } else {
        field = first;
    }
    while (field != NULL) {
        if (lm_trans_emit_statement(file, field->value, indent, namespace_) != 0) {
            return 1;
        }
        field = field->next;
    }
