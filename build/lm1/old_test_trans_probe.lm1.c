...
The next helpers in trans.lm2 are still intentionally left for the next pass:
    ...
    namespace functions over pointer arrays, for example namespace\items[index]\name

They require settling ownership/allocation for L2 pointer arrays and the small
C-surface details around file APIs. Until that is nailed down, this file stays
a compiling checkpoint instead of pretending to be the full translator.
*/
