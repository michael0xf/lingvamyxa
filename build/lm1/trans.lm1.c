...
static int lm_trans_registry_row_from_frame(const LmP0Frame *frame) {
    ...
    row->table = lm_trans_registry_atom_copy_cstr(table_field->value->as.atom);
    row->key = lm_trans_registry_atom_copy_cstr(key_field->value->as.atom);
    row->payload = lm_trans_registry_atom_copy_cstr(payload_field->value->as.atom);
...
static int lm_trans_registry_load_rows(const LmP0Structure *structure) {
    ...
    while (field != 0) {
        ...
        if (!lm_trans_node_is_ignored(node)) {
            if (node->kind != LM_P0_NODE_FRAME) {
                fprintf(stderr, "trans registry error: registry body expects row frames\n");
                return 1;
            }
...
static int lm_trans_registry_load_root(const LmP0Node *root) {
    ...
    while (field != 0) {
        ...
        if (!lm_trans_node_is_ignored(node)) {
            if (node->kind != LM_P0_NODE_FRAME) {
                fprintf(stderr, "trans registry error: root fields must be registry or row frames\n");
                return 1;
            }
