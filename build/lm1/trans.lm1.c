...
static int lm_trans_expr_stack_schedule_call_args(
    ...
    if (callee != NULL && callee->has_signature) {
        ranges = NULL;
        if (callee->param_names.count > 0U) {
            ranges = (LmTransArgumentRange *)calloc(callee->param_names.count, sizeof(*ranges));
            if (ranges == NULL) {
                return 1;
            }
        }
        ...
        status = lm_trans_expr_stack_push_argument_ranges(stack, ranges, callee->param_names.count);
        lm_own_delete(ranges, NULL);
        return status;
    }
