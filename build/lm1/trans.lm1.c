...
static int lm_trans_registry_push_table_cell(
    ...
    if (!split_by_column) {
        return lm_trans_registry_push_row_atoms(table_name, key_atom, payload_atom);
    }

    if (
        !lm_trans_registry_atom_payload(table_name, &table_payload) ||
        !lm_trans_registry_atom_payload(column_name, &column_payload)
    ) {
        return -1;
    }
    ...
    status = lm_trans_registry_push_row_atoms(relation_table, key_atom, payload_atom);
    free(relation_name);
    return status;
}
...
static int lm_trans_validate_expr_atom(
    ...
    if (
        ...
        lm_trans_symbol_is(symbol, "structure") ||
...
static int lm_trans_param_descriptor_matches_return(
    ...
    if (param->kind == LM_P0_NODE_ATOM) {
        return
            ...
            lm_trans_text_same(param->as.atom, return_node->as.atom);
    }
    ...
    return lm_trans_text_same(frame->head, return_node->as.atom);
}
...
static int lm_trans_lookup_type_receiver_binding(
    ...
    binding = lm_trans_registry_lookup(key, "type.receiver.emit");
    if (binding != 0) {
        if (!lm_trans_binding_resolve(binding, &resolved) || resolved.type_emit == 0) {
            fprintf(
                stderr,
                "trans registry inconsistency: type.receiver.emit[\"%.*s\"] has unknown type emit binding %s\n",
                (int)key.length,
                key.data,
                binding
            );
    ...
    binding = lm_trans_registry_lookup(key, "type.receiver.structure_value_alloc");
    if (binding != 0) {
        if (!lm_trans_binding_resolve(binding, &resolved) || resolved.type_structure_value_alloc == 0) {
            fprintf(
                stderr,
                "trans registry inconsistency: type.receiver.structure_value_alloc[\"%.*s\"] has unknown type Structure-value alloc binding %s\n",
                (int)key.length,
                key.data,
                binding
            );
    ...
    binding = lm_trans_registry_lookup(key, "type.receiver.structure_value_fill");
    if (binding != 0) {
        if (!lm_trans_binding_resolve(binding, &resolved) || resolved.type_structure_value_fill == 0) {
            fprintf(
                stderr,
                "trans registry inconsistency: type.receiver.structure_value_fill[\"%.*s\"] has unknown type Structure-value fill binding %s\n",
                (int)key.length,
                key.data,
                binding
            );
...
static int lm_trans_emit_array_target_path(
    ...
    if (lm_trans_write_text(file, target_name) != 0) {
        return 1;
    }
    for (i = 0U; i < depth; ++i) {
        if (lm_trans_put(file, "[") != 0) {
...
static int lm_trans_struct_return_field_index(
    ...
    while (field != 0) {
        if (
            ...
            lm_trans_text_same(field_name, name)
        ) {
            *out_index = index;
            return 1;
        }
...
static int lm_trans_emit_struct_value_return_statement(
    ...
    for (index = 0U; index < field_count; ++index) {
        ...
        if (
            ...
            lm_trans_write_text(file, return_name) != 0 ||
            lm_trans_put(file, ".") != 0 ||
            lm_trans_write_text(file, field_name) != 0 ||
            lm_trans_put(file, " = ") != 0 ||
    ...
    if (symbol->has_signature) {
        ...
        for (i = 0U; i < param_names.count; ++i) {
            ...
            if (
                ...
                param_name_ref == 0 ||
                !lm_trans_text_same(*existing_param_name_ref, *param_name_ref)
            ) {
                fprintf(stderr, "trans L2 error: os-imported function signatures must match\n");
                lm_own_ptr_stack_destroy(&param_names);
                return 1;
...
static const LmTransCapture *lm_trans_capture_stack_find(
    ...
    for (i = 0U; i < captures->count; ++i) {
        ...
        if (capture != 0 && lm_trans_text_same(capture->name, name)) {
            return capture;
        }
    }
    return 0;
...
static int lm_trans_top_level_declare_function(
    ...
    if (item->function.is_struct_return) {
        ...
        if (lm_trans_namespace_declare_generated(namespace_, return_type_name, "structure") != 0) {
...
static int lm_trans_top_level_declare_named_structure(
    ...
    return lm_trans_namespace_declare(namespace_, item->frame->head, "structure");
}
...
static int lm_trans_top_level_declare_named_structure_compatible(
    ...
    return lm_trans_namespace_declare_compatible(namespace_, item->frame->head, "structure");
}
...
static int lm_trans_registry_columns_from_frame(
    ...
    if (!lm_trans_registry_atom_payload(first_name, &first_name) || !lm_trans_text_equals(first_name, "class")) {
...
static int lm_trans_registry_push_column_metadata(
    ...
    if (!lm_trans_registry_atom_payload(table_name, &table_payload)) {
        return -1;
    }

    for (index = 0U; index < column_count; ++index) {
        if (!lm_trans_registry_atom_payload(columns[index].name, &column_payload)) {
            return -1;
        }
...
static int lm_trans_registry_load_for_source(const char *source_path) {
    ...
    while (override_enabled || candidate_names[candidate_index] != 0) {
        ...
        } else if (strncmp(candidate_names[candidate_index], "lm2/", 4U) == 0) {
            if (strlen(candidate_names[candidate_index]) >= sizeof(registry_path)) {
                lm_trans_registry_destroy();
                return 1;
            }
            strcpy(registry_path, candidate_names[candidate_index]);
        } else if (
            lm_trans_registry_path_for_source(
                source_path,
