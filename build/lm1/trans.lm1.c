...
static int lm_trans_registry_push_table_cell(
    ...
    if (
        !lm_trans_registry_identifier_value(table_name, &table_payload) ||
        !lm_trans_registry_identifier_value(key_atom, &key_payload)
    ) {
        return -1;
    }
...
static int lm_trans_registry_columns_from_frame(
    ...
    if (!lm_trans_registry_identifier_value(first_name, &first_name) || !lm_trans_text_equals(first_name, "class")) {
