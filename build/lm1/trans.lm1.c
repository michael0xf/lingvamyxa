...
static int lm_trans_emit_l2_os_frame(FILE *output, const LmP0Frame *frame);
...
static int lm_trans_top_level_emit_os(
    ...
    (void)namespace_;
    return lm_trans_emit_l2_os_frame(file, item->frame);
}
...
static int lm_trans_emit_l2_structure(FILE *file, const LmP0Structure *body, const LmP0Frame *explicit_l2) {
    ...
    status = explicit_l2 != 0 ? lm_trans_validate_end_trailer(explicit_l2) : 0;
    lm_trans_namespace_delete(namespace_);
    return status;
}
...
static int lm_trans_emit_l2_os_branch(FILE *output, const LmP0Frame *branch) {
    if (lm_trans_os_branch_looks_l2(&branch->body)) {
        return lm_trans_emit_l2_structure(output, &branch->body, 0);
    }
...
static int lm_trans_emit_l2_os_frame(FILE *output, const LmP0Frame *frame) {
    ...
    while (field != 0) {
        ...
        if (!lm_trans_node_is_ignored(node)) {
            ...
            if (lm_trans_emit_l2_os_branch(output, branch) != 0) {
                return 1;
            }
