...
static void lm_trans_text_ref_array_destroy(LmP0Text **items, size_t count) {
    lm_own_pointer_array_delete((void **)items, count, lm_trans_text_ref_delete_any);
}
...
static void lm_trans_namespace_destroy(LmTransNamespace *namespace_) {
    if (namespace_ != NULL) {
        lm_own_pointer_array_delete((void **)namespace_->items, namespace_->count, lm_trans_symbol_delete_any);
        lm_own_pointer_array_delete((void **)namespace_->cleanups, namespace_->cleanup_count, lm_trans_cleanup_delete_any);
        lm_own_pointer_array_delete((void **)namespace_->loops, namespace_->loop_count, lm_trans_loop_delete_any);
...
static void lm_trans_namespace_leave_scope(LmTransNamespace *namespace_) {
    ...
    while (
        namespace_->count > 0U &&
        namespace_->items[namespace_->count - 1U] != NULL &&
        namespace_->items[namespace_->count - 1U]->depth == namespace_->depth
    ) {
        --namespace_->count;
        lm_trans_symbol_destroy(namespace_->items[namespace_->count]);
        namespace_->items[namespace_->count] = NULL;
    }
...
static const LmTransSymbol *lm_trans_namespace_find(
    ...
    while (i > 0U) {
        ...
        if (
            ...
            lm_trans_symbol_name_same(namespace_->items[i]->name, name)
        ) {
