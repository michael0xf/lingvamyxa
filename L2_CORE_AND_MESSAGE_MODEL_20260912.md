...
Callable по-прежнему не хранит node; размещение дескриптора не меняет аргумент
узла вызывающей Structure. Реализация размещения остаётся работой по интеграции,
а срок жизни и владелец уже определены.
...
- размещение общего METHOD в отдельном immutable массиве первого Message
  вместо arena дочернего Message; владелец и срок жизни уже определены;
...
Реализовать размещение дескрипторов общих методов в отдельном const: immutable
массиве первого Message. Они живут до завершения процесса; это определённый
владелец, а не выбор новой политики lifetime.
