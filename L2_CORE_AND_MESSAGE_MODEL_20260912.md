...
У него находится **массив ссылок на ветки**. Это не один обязательный корень
Lmx, не общее лексическое дерево и не требование сделать ветки полями его
видимой прикладной Structure.
...
Message-заёмщика и его сборка мусора не освобождают ветку. Создание может
использовать runtime-значения; это не только строковые литералы или C static.
Одинаковое содержимое не требует interning, и число вечных веток может расти.
...
удержания может пополняться; изменяет это служебное состояние его владелец,
корневой Message. Получатель одной ветки не приобретает доступ к массиву,
настройкам корня или другим его веткам.
...
Текущая доступность, последнее уточнение пользователя 12 сентября около 11:07:
у Grok usage 98%, **новых задач ему пока не давать**. Документ передан ему для
изучения; вотчеры восстановлены для вопросов и ответов по нему. Его участки
в плане ниже сохраняются как границы ответственности, а не как немедленные
новые назначения. Fable и Codex продолжают доступную работу, сохраняя
незавершённые файлы Grok. Ответ по документу не запускает следующий этап.
...
Отдельно реализовать root-owned ARRAY удержания вечных веток: публикация
runtime-построенной ветки, lifetime всех её ссылок, явная передача E, отсутствие
копирования/перепривязки node, исчезновение заёмщиков и end-turn GC без
освобождения E. Никакой автоматической выдачи массива или настроек корня.
