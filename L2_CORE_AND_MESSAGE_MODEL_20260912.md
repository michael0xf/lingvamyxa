...
typedef struct LmxMsg {
    volatile uint_fast8_t running;       /* 1 при запуске; 0 пишет родитель */
    volatile uint_fast8_t success;       /* 0 при запуске; 1 пишет сам Message */
...
состояние планировщика и обслуживания как такие данные; в C остаются только
нативные примитивы (поток, монитор ящика).
...
себя в ту же минуту: «"то есть записывается в "struct Lmx *root;"" это уже на L2 там лежит код который с этими Message и работает. А на L1 просто тот код который _будет порождаться_.». О форме этого кода: «В точности как имитируется L2 на Си -- функция с аргументами ,скрытми ргументами и узлом графа.».
...
пустом ящике после выхода из turn'а: «"завершается ли поток владельца при пустом ящике после выхода из turn'а" -- нет!!!! L3 Thread точно так же завершается только по success пользовательским кодом. Каким образом вы будете код писать иначе?».
