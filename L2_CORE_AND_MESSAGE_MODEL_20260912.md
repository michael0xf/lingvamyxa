...
процесса ОС, то есть передача от корня означает «запустить процесс ОС»; до
этапа 5 (запуск ОС как корневой Message) это заглушка: дерево над корнем
описано алгоритмически, не реализовано. (5) Усыновление закрывает
...
Михаил может отменить). Три следствия правил (1)–(3), решённые
ревью-чатом 2026-09-14 при построении цепочки (Михаил может отменить):
(i) dispose родителем завершённого неуспешного ребёнка освобождает его и
сбрасывает его хранилище, если родитель не усыновил это хранилище раньше;
усыновление — право родителя (§32, §34: может принять итоговое состояние, а
не обязан), поэтому рантайм не отказывает в dispose неуспешного ребёнка с
хранилищем, и dispose — это решение родителя не усыновлять; (ii)
освобождение не ждёт dispose детей по одному: dispose или усыновление ребёнка
родителем освобождает цепочкой всё его завершённое поддерево, остаются лишь
условия «ребёнок не бежит, handoff-safe, нет native-пользователей»; (iii)
потомок, ещё бегущий в момент освобождения, закрывается цепочкой (флаг
closing, как при любом stop) и освобождает себя на своём end-turn, помеченный
цепочкой как сирота; вызов родителя возвращается сразу, ветвь в это время
стекает, а не удерживается. Найдено 2026-09-14 в
