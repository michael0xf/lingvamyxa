...
— ребёнок R0, отображённый на полосу R0, главный поток; (v) передача надзора
...
графов. Готовность ребёнка — собственный управляющий флаг ребёнка (пишется как
любой флаг: при admission отправителем, снимается ребёнком; позднее чтение
ничего не меняет), который шаг планировщика родителя читает у прямых детей,
выбирая ход. Родитель не держит списка, в который дописывает чужой поток:
...
снятием своего флага готовности, своей почтой кроме admission, своими
...
отправитель — ready получателю при admission; полоса, берущая ход Message
(его собственный рабочий, хостовое отображение или полоса родителя при
последовательном отображении по 19.28.R2.2), на время взятия и есть полоса
этого Message и снимает его флаг готовности; позднее чтение ничего не меняет.
(4) Примитивы: admission почты под её локом, счётчики ссылок, сигналы
ожидания. Что убирает это правило: списки готовых и списки контекстов
...
«Структуры уровня рантайма под локом исполнителя не принадлежат ни одному
Message: список корней, список слотов, очередь retire, собственная очередь
полосы» — его записал 2026-09-14 чат ревью как собственное следствие, это
никогда не были слова Михаила.
...
другого, закрывается цепочкой. При передаче надзора ребёнок сохраняет
идентичность создания (свой path: path создателя плюс порядковый номер у
создателя) и теряет create id (он больше не результат чьего-либо create);
меняется только надзирающий (допущение записано 2026-09-14 ревью-чатом;
Михаил может отменить). Три следствия правил (1)–(3) для
...
Удаление рантайма — это закрытие корневого Message той же цепочкой
(уточнено 2026-09-15, этап 5 (f); Михаил может переиграть): прямые дети
корня получают флаг закрытия, каждый закрывающий turn исполняется на своей
полосе (ребёнок на полосе хоста — шагом обслуживания корня, ребёнок на
собственном контексте — там, ребёнок без обработчика — бухгалтерией
обслуживания), осаженные дети освобождаются цепочкой, и только потом уходят
собственное хранилище корня и рантайм; ничего не освобождается «как слот
рантайма» за спиной Message. Закрытие корня не добавляет собственного пути:
оно помечает детей ровно так, как любой закрывающий конец turn — снимает
running, делает каждого ребёнка готовым, затем запрос закрытия, — и каждый
ребёнок в конце своего turn делает то же со своими детьми (Михаил,
2026-09-15: «выставляется running=0, и в конце своего turn закрывающийся
Message делает то же со своими детьми; это всё»). Закрытие не ждёт ни одного
потока и не освобождает состояние ни одного потока: Message на собственном
контексте заканчивает свой закрывающий turn, осаживается, и затем его поток
освобождает своё и завершается; закрытие корня осаживает ребёнка только
когда ребёнок осажен, и освобождает только то, чем владеет корень (Михаил,
2026-09-15: «исполнитель освобождает свою память сам»; «у каждого нашего L3
Thread свой планировщик и свой сборщик мусора»). После закрытия корня усыновителя больше нет,
поэтому неуспешный сирота, удерживаемый по тайм-ауту удержания,
освобождается закрытием корня, не дожидаясь тайм-аута; хранилище, которым
ещё пользуется нативный код, наоборот, держит закрытие без всякого
тайм-аута (19.29.8). Ребёнок, обслуживаемый полосой UI, чей запрос ещё
ждёт, закрывается без обработчика, и запрос уходит вместе с почтовым ящиком
полосы — пока собственный цикл корня не понесёт адаптер turn'а UI-ребёнка.
Закрытие сначала переводит почтовый ящик корня в отказ новым допускам —
состояние самого ящика под его же замком, ведь внешний код достигает
рантайма только через него; post после возврата удаления — обращение
встраивающего кода к освобождённому дескриптору. Удаление отвергается изнутри
любого turn, включая собственный turn корня, и с любого потока, который не
есть полоса корня; полоса корня вне turn — это поток загрузчика, так что
второй поток ОС не исполняет обслуживание корня одновременно с ним. С этим
хостовые формы вызовов жизненного цикла уходят: dispose и adopt ребёнка —
только из собственного turn родителя, а создание и удаление рантайма —
единственные два вызова не на полосе какого-либо Message.
