...
Mix; он же, 2026-09-15, дословно: «"в конце концов по таймауту сработает корневой поток и тупо выйдет из процесс ОС. " -- то есть я выше просил заглушку "родитель корневого L3 Thread"» — эта заглушка и есть родитель
корневого L3 Thread: она держит родительский таймаут на корень и по его
истечении выходит из процесса ОС). (5) Усыновление закрывает
...
## 2026-09-16, Михаил, о способности и закрытом Message

Из спеки 19.29.7 убрано по указанию Михаила предложение 15 сентября о том, что ящик закрывающегося Message оседает в родителя и живёт там для поздних отправителей («a capability is the target's mailbox handle; a closing Message's mailbox is settled into its parent with the rest of its storage … there is no count of holders»). Его слова, дословно: «У вас проблема без id найти Message?» Следствие: способность — это id цели; Message находится по id; слот и арена закрытого Message освобождаются цепочкой освобождения, как записано 14 сентября в 19.29.6 («freed by that chain, never at process end»); поздняя отправка разрешает id и получает отказ со статусом, если цель не найдена или закрыта; счётчика держателей нет.

Михаил, 2026-09-16, дословно, о разрешении адресов: «я же вам писал что всё надо свести к Message. Какая вообще проблема не заводить специальный монитор, а завести почтовый сервис который будет хранить сортированный список адресов и давать за конечное время (только не перебором по одному) вхождение?!!!!  Тот же самый R0. Даже если запланирована потом миграция на какую-то другую моель полуения-отправки , она все равно будет в рамках MEssage». И на «родитель отвечает отказом из своего списка»: «"родитель отвечает отправителю отказом со статусом (kind REJECTED) из своего списк" -- а если родителя уже нет?». Следствие, записанное в спеку 19.29.7: почтовый сервис — это сам R0 (Message) с сортированным списком адресов как своими данными на своей дорожке; вхождение за ограниченное время, не перебором; всякая отправка разрешается там, отсутствующий адрес получает отказ со статусом от сервиса, а не от родителя (родителя может уже не быть); никакого специального монитора; будущая миграция модели приёма-отправки остаётся в рамках Message.

Михаил, 2026-09-16, дословно: «Не плодите постоянно лишних сущностей. Так как все Message порождаются из root Message (по цепочке)то он может при merge передавать себя как почтовый сервис. В чем проблема?». Записано в спеку 19.29.7: почтовый сервис — не новая сущность; все Message порождаются из корневого по цепочке, и корень при merge передаёт себя как почтовый сервис, так что каждый Message с рождения держит адрес сервиса.

Михаил, 2026-09-16, дословно: «"Отправка — письмо R0:" -- это может быть _любой_ Message которого R0 передаёт по цепочке при создании L1 Thread. В том числе проверку наличия по списку доступных может делать сам почтовый сервис, а конкретно доставку -- специальный почтальон, список почтальонов может держать почтовый сервис  и так далее -- вы можете _всё_ написать и всё оптимизировать в этой парадигме». Записано в спеку 19.29.7: почтовый сервис, передаваемый по цепочке при создании, может быть любым Message, не только корнем; проверка наличия по списку — у сервиса, доставка — у почтальона-Message, список почтальонов — у сервиса; всё пишется и оптимизируется внутри Message. Для S6-2 сервис — сам R0 (зерно: его транспортный слив под монитором его ящика); почтальоны — позднейшая оптимизация в той же парадигме.

Михаил, 2026-09-16, дословно: «В исходном коде Message-у присваивался адрес a.b.c.d... Мы его убрали вчера вместе со всем остальным-- бывает. Мое последнее объяснение можно считать верным-- никто не мешает дрежать почтовый сревис, но надо вернуть строку адреса в виде неограниченного числом индексов, только не плодить вокруг нее больше полей, поле строки которое соержит len и uint*.  При смене родителя ментть и адрес -- составлять из адреса родителя и индекса. Доставку сделать через inbox родителя -- то есть передавать по цепочке и каждый L3 Thread сделать почтальоном наодин шаг -- он уже по адресу решает отдавать Message своему родителю или ребенку. Сделайте пожалуйста так.Это понятно?». Записано в спеку 19.29.7: у каждого Message есть иерархический адрес a.b.c.d... — одно строковое поле (len и uint*), число индексов не ограничено, других полей вокруг него нет; при смене родителя адрес пересоставляется из адреса родителя и индекса; доставка идёт через inbox родителя по цепочке, каждый L3 Thread — почтальон на один шаг и по адресу решает, отдать письмо родителю или ребёнку; почтовый сервис может быть рядом, но для доставки не нужен.

Михаил, 2026-09-16, дословно: «Адресные массивы здесь кажется можно оптимиировать применительно к тому адресу который лежит в самом Message. Так как фактически это просто индекс у родителя. Адрес доставки можно писать и полностью пока -- он просто будет отваливаться на несуществующих узлах. Можете сразу закодить более-менее умно?». Записано в спеку (раздел 2, запись Message; 19.29.7): поле адреса в самом Message — только его индекс у родителя; полный адрес составляется обходом ссылок на родителей, когда нужен; письмо несёт адрес назначения полностью и просто отваливается на несуществующем узле.

Михаил, 2026-09-16, дословно: «То есть сделать метод getAddress который отдаёт адрес проверяя дерево до корня, и пото метод который посылает по какому-то адресу». Записано в спеку 19.29.7: два метода — getAddress, отдающий адрес Message обходом дерева до корня, и отправка по заданному адресу.

Михаил, 2026-09-16, дословно (на «отправитель считает маршрут»): «НЕТ! ЗАЧЕМ? Отправителю достаточно один раз где-то в коде получить адрес и сохранить. Адрес перед отправкой никто не проверяет. Я же описал алгоритм -- письмо дается родителю который решает передать своем родителю или ребенку. Где здесь поиск пути?». Записано в спеку 19.29.7: маршрут не считается, перед отправкой ничего не проверяется; отправитель получает адрес назначения один раз и хранит; письмо отдаётся родителю, и каждый Message, у которого письмо, сам решает — родителю или ребёнку — сравнивая адрес назначения со своим.

Михаил, 2026-09-16, дословно: «в общем смотри, вот этот алгоритм мы будем потом переписывать на Messge: C:\Nyasha_Planet\lingvamyxa\include_languages\com.mtk.map\com\mtk\map\ Так как пока это сложно -- оставьте пока getAddress и второй описнный механизм доставки, но сделайте его дополнением к почтовому сервису ссылка на который передаётся от root. Так как алгоритм выше вы вряд ли сразу легко перепишите на event-driven». Записано в спеку 19.29.7: каждый Message получает от корня при создании ссылку на почтовый сервис; getAddress и доставка по цепочке (письмо родителю, решение по сравнению адресов) — дополнение к сервису, его механизм доставки на сегодня; алгоритм из include_languages/com.mtk.map (хеш-таблицы, массивы, курсоры автора) позже переписывается на Message и занимает это место.

Михаил, 2026-09-16, дословно: «Почтовый сервис будет доставлять сразу по адресу в памяти так как адреса не двигаются». Записано в спеку 19.29.7: почтовый сервис (с картой) доставляет сразу по адресу в памяти цели, адреса не двигаются; цепочка почтальонов — механизм на сегодня.

Михаил, 2026-09-16, дословно: «то есть поле адреса в письме нет, адрес вычисляется динамически по индексам как в том алгоритме -- есть getAddress, а доставку сейчас по адресу памяти делает почтовый сервис, только сслка на него не глобальная где-то лежит, а приходит от родителя»; «то есть "приходит от родителя" значит порождается вместе с Message как его переназначемое поле.»; и «да» на формулировку координатора о списке живых записей у сервиса. Записано в спеку (раздел 2, запись Message: поле `service`; 19.29.7): в письме поля адреса нет; отправитель отдаёт сервису адрес памяти цели, сервис кладёт письмо в inbox цели под её монитором; getAddress вычисляет a.b.c.d динамически по индексам; цепочка почтальонов по строке адреса — дополнение; ссылка на сервис — переназначаемое поле записи Message, рождается вместе с ним и ставится от родителя, не глобальная; сервис держит свой список живых записей (регистрация при создании, снятие при release, его действия на его дорожке или письма ему; вхождение за ограниченное время) и на неизвестный адрес отвечает отказом, не разыменовывая его.

Михаил, 2026-09-16, дословно: «То есть почтовый сервис это не дополнительное поле рядом c running, а поле уже LMX, а вот API надо сделать общим -- чтобы любой L3 Thread мог выступать в этой роли». Записано в спеку (раздел 2: поле `service` убрано из записи ядра; 19.29.7): ссылка на почтовый сервис — поле LMX самого Message, рождается вместе с ним и ставится от родителя, переназначаемое, не глобальное; API сервиса общий, роль может исполнять любой L3 Thread.

Михаил, 2026-09-16, дословно: «то есть общее почтовое API и getAddress это пока просто заготовки,но надо их сделать сразу потому что потом переписывать будет дорого. Сейчас должна быть заглушка -- L3 Thread в роли почтового сервиса (если его используют так) просто смотрит свое поле почтового сервиса и делегирует ему». Записано в спеку 19.29.7: общее почтовое API и getAddress делаются сразу как заготовки (переписывать потом дорого); сегодняшнее тело у любого L3 Thread в роли сервиса — заглушка: смотрит своё поле почтового сервиса и делегирует ему; настоящее тело сегодня у R0.
